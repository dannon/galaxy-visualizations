...
(function () {
    ...
    jQuery.color.parse = function (E) {
        ...
        if (
            (D = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(E))
        ) {
...
(function ($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        plot.getAxes = function () {
            var res = {},
                i;
            for (i = 0; i < xaxes.length; ++i) res["x" + (i ? i + 1 : "") + "axis"] = xaxes[i] || {};
            for (i = 0; i < yaxes.length; ++i) res["y" + (i ? i + 1 : "") + "axis"] = yaxes[i] || {};

            // backwards compatibility - to be removed
            if (!res.x2axis) res.x2axis = { n: 2 };
            if (!res.y2axis) res.y2axis = { n: 2 };
        ...
        function parseOptions(opts) {
            ...
            if (options.y2axis) {
                if (options.y2axis.autoscaleMargin === undefined) options.y2axis.autoscaleMargin = 0.02;
        ...
        function getUsedAxes() {
            ...
            for (i = 0; i < xaxes.length; ++i) {
                axis = xaxes[i];
                if (axis && axis.used) res.push(axis);
            }
            for (i = 0; i < yaxes.length; ++i) {
                axis = yaxes[i];
                if (axis && axis.used) res.push(axis);
        ...
        function processData() {
            ...
            for (i = 0; i < xaxes.length; ++i) initAxis(xaxes[i]);
            for (i = 0; i < yaxes.length; ++i) initAxis(yaxes[i]);
        ...
        function computeAxisBox(axis) {
            if (!axis || (!axis.used && !(axis.labelWidth || axis.labelHeight))) return;
        ...
        function setupGrid() {
            ...
            for (k = 0; k < axes.length; ++k) setRange(axes[k]);
            ...
            if (options.grid.show) {
                ...
                for (j = 0; j < xaxes.length; ++j) measureTickLabels(xaxes[j]);
                for (j = 0; j < yaxes.length; ++j) measureTickLabels(yaxes[j]);

                // compute the axis boxes, start from the outside (reverse order)
                for (j = xaxes.length - 1; j >= 0; --j) computeAxisBox(xaxes[j]);
                for (j = yaxes.length - 1; j >= 0; --j) computeAxisBox(yaxes[j]);
            ...
            for (k = 0; k < axes.length; ++k) setTransformationHelpers(axes[k]);

            if (options.grid.show) {
                for (k = 0; k < axes.length; ++k) fixupAxisBox(axes[k]);
        ...
        function drawGrid() {
            ...
            for (var j = 0; j < axes.length; ++j) {
                ...
                ctx.strokeStyle =
                    axis.options.tickColor || $.color.parse(axis.options.color).scale("a", 0.22).toString();
                ctx.lineWidth = 1;
            ...
            for (var j = 0; j < axes.length; ++j) {
                ...
                for (var i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (axis.direction == "x") {
                        ...
                        if (axis.position == "bottom") pos.top = box.top + box.padding;
                        else pos.bottom = canvasHeight - (box.top + box.height - box.padding);
                    } else {
        ...
        function drawSeries(series) {
            ...
            if (series.peaks.show) drawSeriesPeaks(series);
        ...
        function drawSeriesPeaks(series) {
            function plotPeaks(datapoints, xoffset, yoffset, axisx, axisy) {
                ...
                ctx.font = "11px Arial";
                if (series.labelType == "mz") ctx.font = "11px Arial";
            ...
            function drawLabel(myx, myy2, x, y2, axisx, axisy, l) {
                if (series.labelType != "none") {
                    ...
                    if (y2 == axisy.max) drawLabel = false;
            ...
            if (lw > 0) plotPeaks(series.datapoints, 0, 0, series.xaxis, series.yaxis);
            ctx.restore();
        }
...
(function ($) {
    function init(plot) {
        ...
        plot.hooks.bindEvents.push(function (plot, eventHolder) {
            var o = plot.getOptions();
            if (o.selection.mode != null) eventHolder.mousemove(onMouseMove);

            if (o.selection.mode != null) eventHolder.mousedown(onMouseDown);
        });
...
AminoAcid.R = new AminoAcid("R", "Arg", "Arginine", 156.10111105, 156.18568);
AminoAcid.N = new AminoAcid("N", "Asn", "Asparagine", 114.04292747, 114.10264);
...
AminoAcid.Q = new AminoAcid("Q", "Gln", "Glutamic Acid", 128.05857754, 128.12922);
...
AminoAcid.K = new AminoAcid("K", "Lys", "Lysine", 128.09496305, 128.17228);
...
AminoAcid.W = new AminoAcid("W", "Trp", "Tryptophan", 186.07931298, 186.2099);
...
function _getAA(aaCode) {
    if (AminoAcid.aa[aaCode]) return AminoAcid.aa[aaCode];
    else return new AminoAcid(aaCode, aaCode, 0.0, 0.0);
}
// $LastChangedDate$
// $LastChangedBy$
// $LastChangedRevision$
...
var sortByMz = function (ionArray) {
    ionArray.sort(function (ion1, ion2) {
...
function Ion(t, color, charge, terminus) {
    ...
    if (this.charge > 1) this.label += charge;
    this.label += "+";
    this.term = terminus;
}
...
Ion.getSeriesIon = function _getSeriesIon(ion, peptide, idxInSeq, massType) {
    if (ion.type == "a") return new Ion_A(peptide, idxInSeq, ion.charge, massType);
    if (ion.type == "b") return new Ion_B(peptide, idxInSeq, ion.charge, massType);
    if (ion.type == "c") return new Ion_C(peptide, idxInSeq, ion.charge, massType);
    if (ion.type == "x") return new Ion_X(peptide, idxInSeq, ion.charge, massType);
    if (ion.type == "y") return new Ion_Y(peptide, idxInSeq, ion.charge, massType);
    if (ion.type == "z") return new Ion_Z(peptide, idxInSeq, ion.charge, massType);
    if (ion.type == "mh") return new Ion_MH(peptide, ion.charge, massType);
};

function _makeIonLabel(type, index, charge) {
    var label = type + "" + index;
    for (var i = 1; i <= charge; i += 1) label += "+";
    return label;
}
...
function Ion_A(peptide, endIdxPlusOne, charge, massType) {
    ...
    var mass = 0;
    if (massType == "mono") mass = peptide.getSeqMassMono(endIdxPlusOne, "n") - (MASS_C_12 + MASS_O_16);
    else if (massType == "avg") mass = peptide.getSeqMassAvg(endIdxPlusOne, "n") - (MASS_C + MASS_O);
    this.massType = massType;
...
function Ion_B(peptide, endIdxPlusOne, charge, massType) {
    ...
    var mass = 0;
    if (massType == "mono") mass = peptide.getSeqMassMono(endIdxPlusOne, "n");
    else if (massType == "avg") mass = peptide.getSeqMassAvg(endIdxPlusOne, "n");
    this.massType = massType;
    this.subSequence = peptide.getSubSequence(endIdxPlusOne, "n");
    this.charge = charge;
    this.mz = _getMz(mass, charge);
...
function Ion_C(peptide, endIdxPlusOne, charge, massType) {
    ...
    var mass = 0;
    if (massType == "mono") mass = peptide.getSeqMassMono(endIdxPlusOne, "n") + MASS_H_1 + (MASS_N_14 + 2 * MASS_H_1);
    else if (massType == "avg") mass = peptide.getSeqMassAvg(endIdxPlusOne, "n") + MASS_H + (MASS_N + 2 * MASS_H);
    this.massType = massType;
    this.subSequence = peptide.getSubSequence(endIdxPlusOne, "n");
    this.charge = charge;
    this.mz = _getMz(mass, charge);
...
function Ion_X(peptide, startIdx, charge, massType) {
    ...
    var mass = 0;
    if (massType == "mono") mass = peptide.getSeqMassMono(startIdx, "c") + 2 * MASS_O_16 + MASS_C_12;
    else if (massType == "avg") mass = peptide.getSeqMassAvg(startIdx, "c") + 2 * MASS_O + MASS_C;
    this.massType = massType;
...
function Ion_Y(peptide, startIdx, charge, massType) {
    ...
    var mass = 0;
    if (massType == "mono") mass = peptide.getSeqMassMono(startIdx, "c") + 2 * MASS_H_1 + MASS_O_16;
    else if (massType == "avg") mass = peptide.getSeqMassAvg(startIdx, "c") + 2 * MASS_H + MASS_O;
    this.massType = massType;
    this.subSequence = peptide.getSubSequence(startIdx, "c");
    this.charge = charge;
    this.mz = _getMz(mass, charge);
...
function Ion_Z(peptide, startIdx, charge, massType) {
    ...
    var mass = 0;
    if (massType == "mono") mass = peptide.getSeqMassMono(startIdx, "c") + MASS_O_16 - MASS_N_14;
    else if (massType == "avg") mass = peptide.getSeqMassAvg(startIdx, "c") + MASS_O - MASS_N;
    this.massType = massType;
    this.subSequence = peptide.getSubSequence(startIdx, "c");
    this.charge = charge;
    this.mz = _getMz(mass, charge);
...
function Ion_MH(peptide, charge, massType) {
    var mass = 0;
    if (massType == "mono") mass = peptide.getNeutralMassMono();
    else if (massType == "avg") mass = peptide.getNeutralMassAvg();
    this.massType = massType;
...
function Peptide(seq, staticModifications, varModifications, ntermModification, ctermModification) {
    ...
    function _addResidueModMasses(seqMass, slice, term) {
        var mass = seqMass;
        if (typeof slice == "number") slice = new _Slice(null, slice, term);
    ...
    function _getSeqMass(startIndex, endIndex, term, massType) {
        ...
        if (startIndex == null) mass = _addTerminalModMass(mass, term);
    ...
    function _addTerminalModMass(seqMass, term) {
        ...
        if (term == "n" && ntermMod) mass += ntermMod;
        if (term == "c" && ctermMod) mass += ctermMod;
...
(function ($) {
    ...
    function init(parent_container, options) {
        ...
        } else makeOptionsTable(container, 1);
            ...
            if (precursorMz) {
                ...
                if (options.zoomMs1) {
                    ...
                    for (var j = 0; j < options.ms1peaks.length; j += 1) {
                        ...
                        if (pk[0] < options.precursorMz - 5.0) continue;
                        if (pk[0] > options.precursorMz + 5.0) break;
                        if (pk[1] > maxIntensityInRange) maxIntensityInRange = pk[1];
                    }
    ...
    function storeContainerData(container, options) {
        ...
        var plotOptions = {
            ...
            xaxis: { tickLength: 3, tickColor: "#000", min: xmin - padding, max: xmax + padding },
            yaxis: {
                tickLength: 0,
                tickColor: "#000",
                max: maxInt * 1.1,
    ...
    function createMs1Plot(container) {
        ...
        var data = [
            { data: options.ms1peaks, color: "#bbbbbb", labelType: "none", hoverable: false, clickable: false },
        ];
        if (options.precursorPeaks) {
            if (options.precursorPeakClickFn)
                data.push({ data: options.precursorPeaks, color: "#ff0000", hoverable: true, clickable: true });
            else data.push({ data: options.precursorPeaks, color: "#ff0000", hoverable: false, clickable: false });
        }
        ...
        var ms1plotOptions = {
            ...
            grid: { show: true, hoverable: true, autoHighlight: true, clickable: true, borderWidth: 1, labelMargin: 1 },
        ...
        if (options.precursorMz) {
            ...
            placeholder.append(
                '<div style="position:absolute;left:' +
                    ...
                    "px;top:" +
                    (o.top - 4) +
                    'px;color:#000;font-size:smaller">' +
                    options.precursorMz.toFixed(2) +
                    "</div>"
            );
        }
        ...
        if (options.ms1scanLabel) {
            placeholder.append(
                '<div style="position:absolute;left:' +
                    ...
                    "px;top:" +
                    (o.top + 4) +
                    'px;color:#666;font-size:smaller">MS1 scan: ' +
                    options.ms1scanLabel +
                    "</div>"
            );
        }
        ...
        if (container.data("ms1zoomRange")) {
            placeholder.append(
                '<div id="' +
                    getElementId(container, elementIds.ms1plot_zoom_out) +
                    '" class="zoom_out_link"  style="position:absolute; left:' +
                    (o.left + ms1plot.width() - 40) +
                    "px;top:" +
                    (o.top + 4) +
                    'px;"></div>'
            );
            ...
            placeholder.append(
                '<div id="' +
                    getElementId(container, elementIds.ms1plot_zoom_in) +
                    '" class="zoom_in_link"  style="position:absolute; left:' +
                    (o.left + ms1plot.width() - 20) +
                    "px;top:" +
                    (o.top + 4) +
                    'px;"></div>'
            );
    ...
    function createPlot(container, datasets) {
        ...
        if (!container.data("zoomRange")) {
            plot = $.plot(
                $(getElementSelector(container, elementIds.msmsplot)),
                datasets,
                container.data("plotOptions")
            );
        } else {
            ...
            $(getElementSelector(container, elementIds.msmsplot)).append(
                '<div id="' +
                    getElementId(container, elementIds.ms2plot_zoom_out) +
                    '" class="zoom_out_link" style="position:absolute; left:' +
                    (o.left + plot.width() - 20) +
                    "px;top:" +
                    (o.top + 4) +
                    'px"></div>'
            );
            ...
            if ($(getElementSelector(container, elementIds.enableTooltip) + ":checked").length > 0) {
                if (item) {
                    if (container.data("previousPoint") != item.datapoint) {
                        ...
                        showTooltip(container, item.pageX, item.pageY, "m/z: " + x + "<br>intensity: " + y);
                    }
                } else {
                    $(getElementSelector(container, elementIds.msmstooltip)).remove();
    ...
    function resetAxisZoom(container) {
        ...
        var zoom_x = false;
        var zoom_y = false;
        if ($(getElementSelector(container, elementIds.zoom_x)).is(":checked")) zoom_x = true;
        if ($(getElementSelector(container, elementIds.zoom_y)).is(":checked")) zoom_y = true;
        if (zoom_x && zoom_y) {
    ...
    function getSelectedIonTypes(container) {
        var ions = [];
        var charges = [];
        $(getElementSelector(container, elementIds.ion_choice))
            .find("input:checked")
            .each(function () {
    ...
    function makeAllIonsTable(container) {
        ...
        myTable +=
            '<table id="' +
            getElementId(container, elementIds.allTableLoc) +
            '" cellpadding="2" class="font_small ' +
            elementIds.ionTable +
        ...
        for (var i = 0; i < allIons.length; i++) {
            ...
            */
            myTable +=
                "<td class='seq'>" + label + "</td><td class='" + cls + "' " + style + " >" + round(mz) + "</td>";
        }
        myTable += "</tr>";
        myTable += "</table>";
        return myTable;
    ...
    function makeInternalIonsTable(container) {
        ...
        myTable +=
            '<table id="' +
            getElementId(container, elementIds.internalIonTable) +
            '" cellpadding="2" class="font_small ' +
            elementIds.ionTable +
            '" style="margin-top:5px;">';
        myTable += "<thead>";
    ...
    function getSelectedNtermIons(selectedIonTypes) {
        ...
        for (var i = 0; i < selectedIonTypes.length; i += 1) {
            ...
            if (sion.type == "a" || sion.type == "b" || sion.type == "c") ntermIons.push(sion);
        }
    ...
    function getSelectedCtermIons(selectedIonTypes) {
        ...
        for (var i = 0; i < selectedIonTypes.length; i += 1) {
            var sion = selectedIonTypes[i];
            if (sion.type == "x" || sion.type == "y" || sion.type == "z") ctermIons.push(sion);
        }
    ...
    function calculateTheoreticalSeries(container, selectedIons) {
        if (selectedIons) {
            ...
            for (var i = 0; i < selectedIons.length; i += 1) {
                ...
                if (sion.type == "a") {
                    if (!container.data("massTypeChanged") && ionSeries.a[sion.charge]) continue;
                    // already calculated
                    else {
                ...
                if (sion.type == "b") {
                    if (!container.data("massTypeChanged") && ionSeries.b[sion.charge]) continue;
                    // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "c") {
                    if (!container.data("massTypeChanged") && ionSeries.c[sion.charge]) continue;
                    // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "x") {
                    if (!container.data("massTypeChanged") && ionSeries.x[sion.charge]) continue;
                    // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "y") {
                    if (!container.data("massTypeChanged") && ionSeries.y[sion.charge]) continue;
                    // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "z") {
                    if (!container.data("massTypeChanged") && ionSeries.z[sion.charge]) continue;
                    // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "mh") {
                    if (!container.data("massTypeChanged") && ionSeries.mh[sion.charge]) continue;
                    // already calculated
                    else {
                        todoIonSeries.push(sion);
            ...
            if (container.data("options").sequence) {
                ...
                var massType = container
                    .find("input[name='" + getRadioName(container, "massTypeOpt") + "']:checked")
                    .val();
                ...
                for (var i = 1; i < sequence.length; i += 1) {
                    for (var j = 0; j < todoIonSeries.length; j += 1) {
                        ...
                        if (tion.term == "n")
                            ionSeriesData.push(
                                (sion = Ion.getSeriesIon(tion, container.data("options").peptide, i, massType))
                            );
                        else if (tion.term == "c")
                            ionSeriesData.unshift(
                                (sion = Ion.getSeriesIon(tion, container.data("options").peptide, i, massType))
                            );
                    }
                }
                ...
                for (var j = 0; j < todoIonSeries.length; j += 1) {
                    ...
                    if (tion.term == null) {
                        ionSeriesData.push(
                            (sion = Ion.getSeriesIon(tion, container.data("options").peptide, null, massType))
                        );
                    }
                }
    ...
    function recalculate(container) {
        return (
            container.data("massErrorChanged") ||
    ...
    function getSeriesMatches(container, selectedIonTypes) {
        ...
        var peakAssignmentType = container
            .find("input[name='" + getRadioName(container, "peakAssignOpt") + "']:checked")
            .val();
        var peakLabelType = container
            .find("input[name='" + getRadioName(container, "peakLabelOpt") + "']:checked")
            .val();
        ...
        for (var j = 0; j < selectedIonTypes.length; j += 1) {
            ...
            if (ion.type == "a") {
                if (recalculate(container) || !ionSeriesMatch.a[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var adata = calculateMatchingPeaks(
                        container,
                        ionSeries.a[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
            ...
            if (ion.type == "b") {
                if (recalculate(container) || !ionSeriesMatch.b[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var bdata = calculateMatchingPeaks(
                        container,
                        ionSeries.b[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
            ...
            if (ion.type == "c") {
                if (recalculate(container) || !ionSeriesMatch.c[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var cdata = calculateMatchingPeaks(
                        container,
                        ionSeries.c[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
            ...
            if (ion.type == "x") {
                if (recalculate(container) || !ionSeriesMatch.x[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var xdata = calculateMatchingPeaks(
                        container,
                        ionSeries.x[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
            ...
            if (ion.type == "y") {
                if (recalculate(container) || !ionSeriesMatch.y[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var ydata = calculateMatchingPeaks(
                        container,
                        ionSeries.y[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
            ...
            if (ion.type == "z") {
                if (recalculate(container) || !ionSeriesMatch.z[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var zdata = calculateMatchingPeaks(
                        container,
                        ionSeries.z[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
            ...
            if (ion.type == "mh") {
                if (recalculate(container) || !ionSeriesMatch.mh[ion.charge]) {
                    // re-calculate only if mass error has changed OR
                    // matching peaks for this series have not been calculated
                    // calculated matching peaks
                    var mhdata = calculateMatchingPeaks(
                        container,
                        ionSeries.mh[ion.charge],
                        peaks,
                        massError,
                        peakAssignmentType
                    );
                ...
                dataSeries.push({
                    data: ionSeriesMatch.mh[ion.charge],
                    color: ion.color,
                    labelType: peakLabelType,
                    labels: ionSeriesLabels.mh[ion.charge],
                });
            }
        }
        ...
        if (internalIonsEnabled(container)) {
            ...
            if (recalculate(container) || internalIonsMatch.length == 0) {
                // TODO replace true with actual check mimicing above
                var internalIons = container.data("internalIons");
                var internalIonsMatch = container.data("internalIonsMatch");
                ...
                for (var i = 0; i < internalIons.length; i += 1) {
                    ...
                    for (var n = 0; n < neutralLosses.length; n += 1) {
                        ...
                        if (matchData && matchData.length > 0) {
                            ...
                            dataSeries.push({
                                data: internalIonsMatch[sion.label],
                                color: INTERNAL_ION_COLOR,
                                labelType: peakLabelType,
                                labels: internalIonsLabels[sion.label],
                            });
                        }
                    }
                }
    ...
    function getNeutralLosses(container) {
        var options = container.data("options");
        var neutralLosses = [];
        $(getElementSelector(container, elementIds.nl_choice))
            .find("input:checked")
            .each(function () {
    ...
    function getMatchForIon(sion, matchData, allPeaks, peakIndex, massTolerance, peakAssignmentType, neutralLoss) {
        var bestPeak = null;
        if (!neutralLoss) sion.match = false; // reset;
        ...
        for (var j = peakIndex; j < allPeaks.length; j += 1) {
            ...
            if (peak[0] < ionmz - massTolerance) continue;
    ...
    function initContainer(container) {
        ...
        parentTable +=
            ...
            rowspan +
            '" valign="top" id="' +
            getElementId(container, elementIds.optionsTable) +
            '"> ';
        parentTable += "</td> ";
        ...
        if (options.showSequenceInfo) {
            ...
            parentTable +=
                '<td style="background-color: white; padding:5px; border:1px dotted #cccccc;" valign="bottom" align="center"> ';
        ...
        if (options.showIonTable) {
            ...
            parentTable +=
                ...
                rowspan +
                '" valign="top" id="' +
                getElementId(container, elementIds.ionTableLoc1) +
                '" > ';
            parentTable += '<div id="' + getElementId(container, elementIds.ionTableDiv) + '">';
            parentTable +=
                '<span id="' +
                getElementId(container, elementIds.moveIonTable) +
                '" class="font_small link">[Click]</span> <span class="font_small">to move table</span>';
            // placeholder for modifications
            parentTable +=
                '<div id="' + getElementId(container, elementIds.modInfo) + '" style="margin-top:5px;"></div> ';
            parentTable += "</div> ";
            parentTable += "</td> ";
            parentTable += "</tr> ";
        }
        ...
        parentTable += "<tr> ";
        parentTable +=
            '<td style="background-color: white; padding:5px; border:1px dotted #cccccc;" valign="top" align="center"> ';
        parentTable +=
            '<div id="' +
            getElementId(container, elementIds.msmsplot) +
            '" align="bottom" style="width:' +
            options.width +
            "px;height:" +
            options.height +
            'px;"></div> ';
        ...
        parentTable +=
            '<div id="' +
            getElementId(container, elementIds.viewOptionsDiv) +
            '" align="top" style="margin-top:15px;"></div> ';

        // placeholder for ms1 plot (if data is available)
        if (options.ms1peaks && options.ms1peaks.length > 0) {
            parentTable +=
                '<div id="' +
                getElementId(container, elementIds.msPlot) +
                '" style="width:' +
                options.width +
                'px;height:100px;"></div> ';
        }
        ...
        parentTable += "<tr> ";
        parentTable +=
            '<td colspan="3" class="bar noprint" valign="top" align="center" id="' +
            getElementId(container, elementIds.ionTableLoc2) +
            '" > ';
    ...
    function makeIonTable(container) {
        ...
        myTable +=
            '<table id="' +
            getElementId(container, elementIds.ionTable) +
            '" cellpadding="2" class="font_small ' +
            elementIds.ionTable +
            '">';
        myTable += "<thead>";
        ...
        for (var i = 0; i < options.sequence.length; i += 1) {
            ...
            if (options.peptide.varMods()[i + 1]) myTable += "<td class='seq modified'>" + aaChar + "</td>";
            else myTable += "<td class='seq'>" + aaChar + "</td>";
    ...
    function getCalculatedSeries(ionSeries, ion) {
        if (ion.type == "a") return ionSeries.a[ion.charge];
        if (ion.type == "b") return ionSeries.b[ion.charge];
        if (ion.type == "c") return ionSeries.c[ion.charge];
        if (ion.type == "x") return ionSeries.x[ion.charge];
        if (ion.type == "y") return ionSeries.y[ion.charge];
        if (ion.type == "z") return ionSeries.z[ion.charge];
        if (ion.type == "mh") return ionSeries.mh[ion.charge];
    }

    function makeIonTableMovable(container) {
        $(getElementSelector(container, elementIds.moveIonTable)).hover(function () {
    ...
    function showSequenceInfo(container) {
        ...
        if (options.sequence) {
            ...
            if (options.precursorMassType == "mono") neutralMass = options.peptide.getNeutralMassMono();
            else neutralMass = options.peptide.getNeutralMassAvg();
            ...
            if (mz) specinfo += ", m/z " + mz.toFixed(4);
            specinfo += "</div>";
        }
    ...
    function getModifiedSequence(options) {
        ...
        for (var i = 0; i < options.sequence.length; i += 1) {
            if (options.peptide.varMods()[i + 1])
                modSeq +=
                    '<span style="background-color:yellow;padding:1px;border:1px dotted #CFCFCF;">' +
                    options.sequence.charAt(i) +
                    "</span>";
            else modSeq += options.sequence.charAt(i);
        }
    ...
    function makeViewingOptions(container) {
        ...
        myContent +=
            'X:<input id="' +
            getElementId(container, elementIds.zoom_x) +
            '" type="checkbox" value="X" checked="checked"/> ';
        myContent +=
            '&nbsp;Y:<input id="' + getElementId(container, elementIds.zoom_y) + '" type="checkbox" value="Y" /> ';
        myContent +=
            '&nbsp;<input id="' +
            getElementId(container, elementIds.resetZoom) +
        ...
        myContent += "<nobr> ";
        myContent +=
            '<input id="' + getElementId(container, elementIds.enableTooltip) + '" type="checkbox">Enable tooltip ';
        myContent += "</nobr> ";
    ...
    function makeOptionsTable(container, charge) {
        ...
        if (charge >= 2) myTable += '<input type="checkbox" value="2" id="b_2" checked="checked"/>2<sup>+</sup> ';
        else myTable += '<input type="checkbox" value="2" id="b_2"/>2<sup>+</sup> ';
        if (charge >= 3) myTable += '<input type="checkbox" value="3" id="b_3" checked="checked"/>3<sup>+</sup> ';
        else myTable += '<input type="checkbox" value="3" id="b_3"/>3<sup>+</sup> ';
        myTable += "</nobr> ";
        myTable += "<br/> ";
        myTable += "<!-- c ions --> ";
        myTable += "<nobr> ";
        ...
        if (charge >= 2) myTable += '<input type="checkbox" value="2" id="y_2" checked="checked"/>2<sup>+</sup> ';
        else myTable += '<input type="checkbox" value="2" id="y_2"/>2<sup>+</sup> ';
        if (charge >= 3) myTable += '<input type="checkbox" value="3" id="y_3" checked="checked"/>3<sup>+</sup> ';
        else myTable += '<input type="checkbox" value="3" id="y_3"/>3<sup>+</sup> ';
        myTable += "</nobr> ";
        myTable += "<br/> ";
        myTable += "<!-- z ions --> ";
        myTable += "<nobr> ";
        ...
        if (options.showInternalIonOption) {
            myTable +=
                '<div><span style="font-weight: bold;">Internal</span><input type="checkbox" value="internal" id="internal"/> ';
            myTable += "</nobr><br />";
        }
        myTable +=
            '<span id="' +
            getElementId(container, elementIds.deselectIonsLink) +
            '" style="font-size:8pt;text-decoration: underline; color:sienna;cursor:pointer;">[Deselect All]</span> ';
        myTable += "</div><br /> ";
        ...
        myTable +=
            ...
            getRadioName(container, "peakAssignOpt") +
            '" value="intense" checked="checked"/><span style="font-weight: bold;">Most Intense</span><br/> ';
        myTable +=
            '<input type="radio" name="' +
            getRadioName(container, "peakAssignOpt") +
            '" value="close"/><span style="font-weight: bold;">Nearest Match</span><br/> ';
        ...
        myTable +=
            ' id="' +
            getElementId(container, elementIds.peakDetect) +
            '"/><span style="font-weight:bold;">Peak Detect</span>';
        myTable += "</div> ";
        myTable += "</td> </tr> ";
        ...
        myTable +=
            '<input type="radio" name="' +
            getRadioName(container, "peakLabelOpt") +
            '" value="ion" checked="checked"/><span style="font-weight: bold;">Ion</span>';
        myTable +=
            '<input type="radio" name="' +
            getRadioName(container, "peakLabelOpt") +
            '" value="mz"/><span style="font-weight: bold;">m/z</span><br/>';
        myTable +=
            '<input type="radio" name="' +
            getRadioName(container, "peakLabelOpt") +
            '" value="none"/><span style="font-weight: bold;">None</span> ';
        myTable += "</div> ";
        myTable += "</td> </tr> ";
        ...
        myTable +=
            '<div>Width: <span id="' +
            getElementId(container, elementIds.slider_width_val) +
            '">' +
            options.width +
            "</span></div> ";
        myTable +=
            '<div id="' + getElementId(container, elementIds.slider_width) + '" style="margin-bottom:15px;"></div> ';
        myTable +=
            '<div>Height: <span id="' +
            getElementId(container, elementIds.slider_height_val) +
            '">' +
            options.height +
            "</span></div> ";
