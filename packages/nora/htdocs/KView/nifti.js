...
parse = function (buffer_org) {
    ...
    if (buffer.byteLength < 348) {
        throw new Error(
            "The NIFTI buffer size was " + buffer.byteLength + ". This is not even large enough to contain a header!"
        );
    }

    if ((view.getInt16(0, true) == 540) | (view.getInt16(0, false) == 540)) {
        // nifti-2
        ...
        if (sizeof_hdr !== 348 && (1 > dim[0] || dim[0] > 7)) {
            ...
            console.warn(
                "Size of NIfTI header different from what I expect, but I'll try to do my best anyway (might cause trouble)."
            );
        }
...
*/
        ...
        if (sum < 0.000000001) sform_code = -1;
    ...
    if (vox_offset > 352) {
        if (extension != 0) {
            ...
            for (var k = 0; k < ext.size - 10; k++) {
                ...
                if (c != 0) ext.content += String.fromCharCode(c);
            }
            ret.extension = ext;
    ...
    ret.datascaling = {
        ...
        e: function (v) {
            if (Array.isArray(v)) {
                var ret = [];
                for (var k = 0; k < v.length; k++) ret.push(v[k] * scl_slope + scl_inter);
                return ret;
            } else return v * scl_slope + scl_inter;
        },
        ie: function (v) {
            if (Array.isArray(v)) {
                var ret = [];
                for (var k = 0; k < v.length; k++) ret.push((v[k] - scl_inter) / scl_slope);
                return ret;
            } else return (v - scl_inter) / scl_slope;
        },
    };
    ...
    if (qform_code === 0) {
        ...
        ret.spaceDirections = [
            [
                ...
                pixdim[1] * (2 * b * c + 2 * a * d),
                pixdim[1] * (2 * b * d - 2 * a * c),
            ],
            [
                pixdim[2] * (2 * b * c - 2 * a * d),
                pixdim[2] * (a * a + c * c - b * b - d * d),
                pixdim[2] * (2 * c * d + 2 * a * b),
            ],
            [
                qfac * pixdim[3] * (2 * b * d + 2 * a * c),
                qfac * pixdim[3] * (2 * c * d - 2 * a * b),
                qfac * pixdim[3] * (a * a + d * d - c * c - b * b),
            ],
        ];
    ...
    if (magic === "n+2\0") {
        ...
        ret.buffer = buffer;
        if (datatype !== 0) {
            ret.data = parseNIfTIRawData(
                ret.buffer,
                datatype,
                dim,
                { endianFlag: littleEndian },
                Math.floor(vox_offset)
            );
        }
    } else if (magic === "n+1\0") {
        // nifti1 analysze75
...
function parseNIfTIRawData(buffer, type, dim, options, hdroffs) {
    var i,
        ...
        view,
        totalLen = 1,
        endianFlag = options.endianFlag,
        endianness = endianFlag ? "little" : "big";
    ...
    if (type == "block") {
        ...
        function unevenBufferCorrection16(TypArr) {
            if (totalLen > buffer.byteLength) return new TypArr(buffer, hdroffs);
            else {
                if (hdroffs % 2 == 0) return new TypArr(buffer, hdroffs);
                else {
        ...
        switch (type) {
            ...
            case "int32":
                checkSize(4);
                if (hdroffs % 4 == 0) return new Int32Array(buffer, hdroffs);
                else {
                    var tmp = new Uint8Array(buffer);
...
function parse_mgh(buffer) {
    ...
    var d = nifti.voxSize;
    nifti.spaceDirections = [
        [view.getFloat32(i) * d[0], view.getFloat32(i + 4) * d[0], view.getFloat32(i + 8) * d[0]],
    ...
    if (nifti.type == "UCHAR") {
        ...
        for (var k = 0; k < imgsiz; k++) nifti.data[k] = view.getUint8(k * nifti.nbyte);
    } else if (nifti.type == "SHORT") {
        nifti.nbyte = 2;
        nifti.data = new Uint16Array(buffer, hdroffs, imgsiz);
        for (var k = 0; k < imgsiz; k++) nifti.data[k] = view.getUint16(k * nifti.nbyte);
    } else if (nifti.type == "INT") {
        nifti.nbyte = 4;
        nifti.data = new Int32Array(buffer, hdroffs, imgsiz);
        for (var k = 0; k < imgsiz; k++) nifti.data[k] = view.getInt32(k * nifti.nbyte);
    } else if (nifti.type == "FLOAT") {
        nifti.nbyte = 4;
        nifti.data = new Float32Array(buffer, hdroffs, imgsiz);
        for (var k = 0; k < imgsiz; k++) nifti.data[k] = view.getFloat32(k * nifti.nbyte);
    }
...
function parse_mif(buffer) {
    ...
    for (var k = 0; k < lines.length; k++) {
        ...
        if ((key == "dim") | (key == "transform") | (key == "vox")) val = val.split(",").map(parseFloat);
        if (key == "file") val = parseInt(val.split(" ")[1]);
    ...
    if (obj.datatype[0].search("LE") > -1) nifti.endian = 1;
    //nifti.descrip = obj;
    ...
    if (nifti.type == "UCHAR") {
        ...
        for (var k = 0; k < imgsiz; k++) nifti.data[k] = view.getFloat32(k * nifti.nbyte, nifti.endian);
    } else if (nifti.type == "DOUBLE") {
        nifti.nbyte = 4;
        nifti.data = new Float32Array(imgsiz);
        for (var k = 0; k < imgsiz; k++) nifti.data[k] = view.getFloat64(k * nifti.nbyte * 2, nifti.endian);
    }
...
function NRRD() {
    ...
    nrrd.serialize = function (nrrdOrg) {
        ...
        if (nrrd.sizes === undefined) {
            // 'sizes' should ALWAYS be given
            ...
            switch (nrrd.encoding) {
                case "raw":
                    if (nrrd.type == nativeType && nrrd.endian == systemEndianness) {
                        bufferData = nrrd.data.buffer.slice(
                            nrrd.data.byteOffset,
                            nrrd.data.byteOffset + nrrd.data.byteLength
                        );
                    } else if (nrrd.endian == systemEndianness) {
                        bufferData = castTypedArray(nrrd.data, nrrd.type);
                        bufferData = bufferData.buffer.slice(
    ...
    nrrd.parse = function (buffer) {
        var i,
            ...
            dataStart,
            ret = {
                data: undefined /* parsed data */,
                buffer: undefined /* raw buffer holding data */,
                keys: {},
                version: undefined,
            },
            lines,
        ...
        while (i < buf8.length) {
            if (buf8[i] == 10) {
                ...
                if (buf8[i - 1] == 10 || (buf8[i - 1] == 13 && buf8[i - 2] == 10)) {
                    // Safe because we start at position 2 and never move backwards
                    dataStart = i + 1;
                    break;
                } else {
        ...
        ret.datascaling = {
            ...
            e: function (v) {
                if (Array.isArray(v)) {
                    var ret = [];
                    for (var k = 0; k < v.length; k++) ret.push(v[k] * this.slope + this.offset);
                    return ret;
                } else return v * this.slope + this.offset;
            },
            ie: function (v) {
                if (Array.isArray(v)) {
                    var ret = [];
                    for (var k = 0; k < v.length; k++) ret.push((v[k] - this.offset) / this.slope);
                    return ret;
                } else return (v - this.offset) / this.slope;
            },
        };
        ...
        switch (prop) {
            ...
            case "spaceDimension":
                assert(
                    ...
                    "Field " + prop + " should at least contain a number!"
                );
                line = propNRRD + ": " + value;
                break;
            ...
            case "oldMax":
                assert(
                    typeof value == "number" || value instanceof Number,
                    "Field " + prop + " should contain a number!"
                );
                line = propNRRD + ": " + value;
                break;
            // Vectors
            case "spaceOrigin":
                assert(
                    value.length === spaceDimension,
                    "Field " + prop + " should be a list with length equal to the space dimension!"
                );
                value.forEach(function (val) {
                    assert(
                        typeof val == "number" || val instanceof Number,
                        "Field " + prop + " should be a list of numbers!"
                    );
                });
            ...
            case "spaceUnits":
                assert(
                    value.length !== undefined && value.length == dimension,
                    "Field " + prop + " should be a list with length equal to the dimension!"
                );
                value.forEach(function (val) {
                    assert(
            ...
            case "spaceDirections":
                ...
                value.forEach(function (vec) {
                    assert(
                        vec === null || (vec.length !== undefined && vec.length === spaceDimension),
                        "The elements of field " + prop + " should be lists with length equal to the space dimension!"
                    );
                    if (vec !== null)
                        vec.forEach(function (val) {
                            assert(
                                typeof val == "number" || val instanceof Number,
                                "The elements of field " + prop + " should be lists of numbers!"
                            );
                        });
                });
                line =
            ...
            case "measurementFrame":
                assert(
                    value.length !== undefined && value.length === spaceDimension,
                    "Field " + prop + " should be a list with length equal to the space dimension!"
                );
                value.forEach(function (vec) {
                    assert(
            ...
            case "type":
                assert(
                    ...
                    "Field " + prop + " should contain a string!"
                );
                line = propNRRD + ": " + value;
                break;
            case "encoding":
    ...
    function parseNRRDDataFile(dataFile) {
        ...
        if (match) {
            // We have a format specification
    ...
    function checkNRRD(ret) {
        ...
        if (
            (ret.data === undefined || ret.data.length === 0) &&
            (ret.buffer === undefined || ret.buffer.byteLength === 0) &&
    ...
    function serializeToBuffer(data, type, endian) {
        var i,
            endianFlag,
            view,
            nativeSize = getNRRDTypeSize(type),
            buffer = new ArrayBuffer(data.length * nativeSize);
        switch (endian) {
            case "big":
                endianFlag = false;
                break;
