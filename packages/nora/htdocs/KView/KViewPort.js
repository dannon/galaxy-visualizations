...
function KFreeView(master, $parent) {
    ...
    that.resize = function () {
        ...
        if (that.getCurrentViewer()) that.getCurrentViewer().setInnerLayout();
    };

    that.kill = function () {
        that.panel.close();
        if (that.getCurrentViewer()) that.getCurrentViewer().close();
...
function KPanelView(master, name, options) {
    if (name == undefined || name == "") name = "Viewport";
    ...
    if (options.top != undefined) panel.$container.css("top", options.top);
    if (options.left != undefined) panel.$container.css("left", options.left);
    ...
    if (options.addClass) that.panel.$container.addClass(options.addClass);

    var $resizeTriangle = resizeTriangle(function () {}, that.resize, that.panel.$container).appendTo(that.$container);
...
function KViewPort(master, viewPortID) {
    ...
    that.$LeftViewportCol_resizer = $("<div class='annotion_tool_resizer'><div><div></div></div></div>").on(
        "mousedown",
        function (ev) {
    ...
    function zoomViewPort(ev, hideTable) {
        if (zoomed == 0) {
            if (!KViewer.showInfoBar) KViewer.toggleInfobar();
...
*/
            ...
            for (var k = 0; k < keys.length; k++) {
                if (keys[k] != that.viewPortID && master.viewports[keys[k]].getCurrentViewer() != undefined) {
                    ...
                    if (vi.viewerType != "Manager") {
                        if (vp.$container != undefined) vp.$container.detach();
                        if (vi.toolbar != undefined) vi.toolbar.hide();
                        if (vi.layoutbar != undefined) vi.layoutbar.hide();
                        if (vi.renderOutlines) vi.renderOutlines("close");
                        vi.hiddenHisto = true;
            ...
            if (!TableHidden & hideTable) master.toggleTableHide();
            ...
            if (TableHidden & !KViewer.standalone) master.toggleTableHide();

            if (userinfo.username != guestuser || static_info.public_projects) $("#thetable").show();
            ...
            for (var k = 0; k < keys.length; k++) {
                if (keys[k] != that.viewPortID && master.viewports[keys[k]].getCurrentViewer() != undefined) {
                    ...
                    var vi = vp.getCurrentViewer();
                    if (vi.viewerType != "Manager") {
                        if (vi.toolbar != undefined) vi.toolbar.show();
                        if (vi.layoutbar != undefined) vi.layoutbar.show();
    ...
    that.setCurrentViewer = function (v) {
        ...
        if (v != undefined) {
            if (currentViewer != undefined) currentViewer.$container.detach();
    ...
    that.setInfoText = function (txt) {
        if (currentViewer != undefined) currentViewer.setInfoText(txt);
    };
   ...
   */
    ...
    function setContent(fileObject, params) {
        ...
        if (fileObject.content === false) return;

        if (that.visible != undefined && that.visible == false && fileObject.contentType != "json") return;
        ...
        if (fileObject.contentType == "nii") {
            ...
            if (
                ...
                (fileObject.content.datatype == "rgb24" &&
                    fileObject.content.sizes[2] == 1 &&
                    fileObject.content.sizes.length < 4)
            ) {
                ...
                if (fileObject.fileinfo.Tag) {
                    if (fileObject.fileinfo.Tag.search("/mask/") != -1) {
                        ...
                        if (params.intent == undefined) params.intent = {};
                        if (params.intent.surfcol == undefined) params.intent.ROI = true;
                        medViewer.setContent(fileObject, params);
                    } else medViewer.setContent(fileObject, params);
                } else {
                    medViewer.setContent(fileObject, params);
                }
		  ...
		  */
            ...
            if (!params.intent.asjson) {
                if (
                    (tag.search("FORM") >= 0) |
                    params.intent.patientedit |
                    params.intent.studyedit |
                    ((fileObject.filename || "").search("\\.form\\.json") != -1)
                ) {
        ...
        if (that.onsetContent) {
            setTimeout(function () {
                that.onsetContent(fileObject, params);
            }, 0);
        }
    }
    ...
    setTimeout(
        (function ($container, that) {
            return function () {
                ...
                if (isOrthoDropper) orthodrop = (x) => "<div droptag='ortho" + x + "'> as orthoview </div>";

                that.dragster = {};
                that.dragster.ortho = $("<div droptag='orthoview'>drop image as orthoview</div>").hide();
                if (isOrthoDropper) that.dragster.ortho.appendTo(that.$dropIndicator);
                ...
                that.dragster.fibers = $("<div class='fiberdrop' droptag='fibers'>drop fibers</div>")
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.asjson = $("<div droptag='asjson'> drop as json</div>")
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.switch = $("<div droptag='switch'>switch/drop viewport(s)</div>")
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.obj3D = $("<div>drop 3D object</div>").hide().appendTo(that.$dropIndicator);
                that.dragster.image = $("<div droptag='image'>drop as image</div>")
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.atlas = $(
                    "<div class='atlasdrop' droptag='atlas'>drop  Atlas " + orthodrop("atlas") + " </div>"
                )
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.roi = $("<div class='roidrop' droptag='roi'>drop  ROI " + orthodrop("roi") + " </div>")
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.surfacecol = $("<div class='surfcol'  droptag='surfcol'>drop as surface coloring</div>")
                    .hide()
                    .appendTo(that.$dropIndicator);
                that.dragster.overlay = $(
                    "<div class='ovldrop'  droptag='overlay'>drop  overlay " + orthodrop("overlay") + " </div>"
                )
                    .hide()
                    .appendTo(that.$dropIndicator);
    ...
    function showDropIndicators(e) {
        ...
        if (currentViewer != undefined && currentViewer.viewerType == "Manager") return;

        for (var obj in that.dragster) that.dragster[obj].hide();
        ...
        if (tempObjectInfo != undefined && tempObjectInfo.length > 0) {
            ...
            var atlas = false;
            var any = false;
            if (KViewer.zoomedViewport != -1 && !that.isZoomed()) that.dragster["notpossible"].show();
            ...
            if (tempObjectInfo[0].mime == "nii") {
                ...
                if (
                    ((tempObjectInfo[0].filename || "").search("cosl") > -1) |
            ...
            if (
                (tempObjectInfo[0].mime == "json") |
                (tempObjectInfo[0].mime == "form") |
                (tempObjectInfo[0].mime == "ano")
            ) {
                any = true;
            ...
            if (tempObjectInfo[0].type == "markertemplate") any = true;
            if (!any) that.dragster["default"].show();
        } else if (KToolWindow.dragTool != undefined) that.dragster.tool.show();
        else that.dragster["default"].show();
        $dropIndicator.fadeIn(150);
    }

    function hideDropIndicators(e) {
        if (that.$dropIndicator) that.$dropIndicator.fadeOut(150, undefined, function () {});
    }
...
*/
    ...
    var ondrop = function (ev) {
        ...
        that.$dropIndicator.children().each(function (k, e) {
            $(e).css("background", "");
        });
        ...
        if (ev.dataTransfer && ev.dataTransfer.types && ev.dataTransfer.types[0] == "text/plain") return;
        ...
        if (KViewer.zoomedViewport != -1 && !that.isZoomed()) return;

        if (
            KViewer.zoomedViewport != -1 &&
            tempObjectInfo[0].intent &&
            tempObjectInfo[0].intent.viewport != undefined
        ) {
            zoomViewPort();
        ...
        if (
            ...
            dt.items[0].webkitGetAsEntry().isDirectory
        ) {
        ...
        if (droptag == "fibers" && isFiberVol) {
            ...
            initiateLoadData(params, function () {
                for (var k = 0; k < params.length; k++) params[k].intent = { createFiberTracking: { color: 3 } };
            ...
            function itovl() {
                if (cnt >= params.length) return;
                if (params[cnt].intent == undefined) params[cnt].intent = { overlay: true };
                else params[cnt].intent.overlay = true;
            ...
            if (surfs.length > 1) {
                var menu = KContextMenu(
                    ...
                    function (str, ev) {
                        for (var k = 0; k < params.length; k++) {
                            if (params[k].intent == undefined) params[k].intent = { surfcol: str };
                            else params[k].intent.surfcol = str;
                        }
                        initiateLoadData(params);
                    },
                    false
                ...
                for (var k = 0; k < params.length; k++)
                    if (params[k].intent == undefined) params[k].intent = { surfcol: surfs[0].surf.fileID };
                    else params[k].intent.surfcol = surfs[0].surf.fileID;
                initiateLoadData(params);
            }
        } else if (
            tempObjectInfo &&
            tempObjectInfo[0] &&
            tempObjectInfo[0].mime == "tck" &&
            tempObjectInfo[0].dirvolref
        ) {
            ...
            for (var k = 0; k < params.length; k++) {
                if (params[k].intent == undefined) params[k].intent = { overlay: true };
                else params[k].intent.overlay = true;
            }
            initiateLoadData(params);
        } else if ((droptag == "atlas") | (droptag == "orthoatlas")) {
            for (var k = 0; k < params.length; k++) {
                if (params[k].intent == undefined) params[k].intent = { atlas: true };
                else params[k].intent.atlas = true;
            }
            if (droptag == "orthoatlas") for (var k = 0; k < params.length; k++) loadOrthoview(params[k], viewPortID);
            else initiateLoadData(params);
        } else if ((droptag == "roi") | (droptag == "orthoroi")) {
            for (var k = 0; k < params.length; k++) {
                if (params[k].intent == undefined) params[k].intent = { roi: true };
                else params[k].intent.roi = true;
            }
            if (droptag == "orthoroi") for (var k = 0; k < params.length; k++) loadOrthoview(params[k], viewPortID);
            else initiateLoadData(params);
        } else {
            ...
            var bruker;
            if (BrukerReader) bruker = BrukerReader.checkForBrukerData(params);
            if (bruker !== undefined) {
            ...
            for (var k = 0; k < params.length; k++) {
                if (droptag == "image" && params[k].intent) params[k].intent.overlay = false;
                if (droptag == "asjson" && params[k].intent) params[k].intent.asjson = true;
            }

            if (params[0].intent == undefined) params[0].intent = {};
    ...
    that.$container.on("mouseleave", function (ev) {
        if (KViewer.in_viewport_resizing_phase) return;
        $container.removeClass("resizebar-x resizebar-y");
        $container.css("cursor", "");
    });

    that.$container.on("mousemove", function (ev) {
        if (zoomed || KViewer.in_viewport_resizing_phase || that.viewPortID > 11) return;
    ...
    that.$container.on("mousedown", function (ev) {
        ...
        if (that.right_neighbor == "cartridge") {
            ...
            if (KViewer.viewports[that.right_neighbor] == undefined) return;
    ...
    function initiateLoadData(params, onready) {
        ...
        if (currentViewer != undefined && currentViewer.viewerType == "Manager") return false;
        ...
        function closeit(x) {
            if (str == "this") {
                if (from.currentFileID) {
                    if (x.currentFileID == from.currentFileID) x.close();
                }
                if (from.roi) {
                    if (x.roi.fileID == from.roi.fileID) x.close();
                }
                if (from.fibers) {
                    if (x.fibers.fileID == from.fibers.fileID) x.close();
                }
            } else x.close();
        }
        ...
        KViewer.iterateMedViewers(function (mv) {
            ...
            for (var j = 0; j < objs.length; j++) {
                if (subtype != undefined) {
                    if (objs[j][subtype] != undefined) closeit(objs[j]);
                } else closeit(objs[j]);
            }
        });
    }
    ...
    that.closeContextMenu = function (from) {
        return KContextMenu(
            function () {
                var suffix = "";
                if (from != undefined) suffix = "(" + from.type + ")";

                var $menu = $("<ul class='menu_context'>");

                var filename;
                if (from != undefined) {
                    if (from.fileinfo) filename = from.fileinfo.Filename;
                    if (from.roi) filename = from.roi.filename;
                    if (from.fibers) filename = from.fibers.filename;
                ...
                if ((str == "all") | (str == "this")) {
                    if (from == undefined) {
                        ...
                        if (
                            ...
                            (state.viewer.selectionMode[0] == "w") &
                            (unsaved != "")
                        ) {
                            alertify.confirm(
                                "Are you sure to close all, there are unsaved " + unsaved + "!",
                                function (e) {
                                    if (e) KViewer.closeAll();
                                }
                            );
                        } else {
                        ...
                        return;
                    } else if (from.type == "overlay") delAllObjs("overlays", undefined, from, str);
                    else if (from.type == "surface") delAllObjs("objects3D", "surf", from, str);
                    else if (from.type == "fiber") delAllObjs("objects3D", "fibers", from, str);
                    else if (from.type == "roi") delAllObjs("ROIs", undefined, from, str);
                    else if (from.type == "atlas") delAllObjs("atlas", undefined, from, str);
                } else if (str == "ortho") {
                }
            },
            false
    ...
    that.close = function () {
        if (zoomed) zoomViewPort();
        if (that.getCurrentViewer()) {
            if (that.getCurrentViewer().viewerType != "Manager") that.setCurrentViewer();
        }
    };

    function dummyViewer(which, constr) {
        return {
            setContent: function (c, p) {
...
KViewPort.getViewerByFileID = function (fid) {
    ...
    for (var k = 0; k < KViewer.viewports.length; k++) {
        if (KViewer.viewports[k] != undefined) {
            ...
            if (v != undefined) if (v.currentFileID == fid) vps.push(v);
        }
    }
    return vps;
};
