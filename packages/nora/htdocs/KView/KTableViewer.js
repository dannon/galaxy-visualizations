...
var jsstats = jsstats || {};
!(function (jss) {
    var NormalDistribution = function (t, i) {
        t || (t = 0),
            i || (i = 1),
            (this.mean = t),
            (this.sd = i),
            (this.Sqrt2 = 1.4142135623730951),
            (this.Sqrt2PI = 2.5066282746310007),
            (this.lnconstant = -Math.log(this.Sqrt2PI * i));
    };
    (NormalDistribution.prototype.sample = function () {}),
        (NormalDistribution.prototype.cumulativeProbability = function (t) {
            var i = (t - this.mean) / (this.Sqrt2 * this.sd);
            return 0.5 + 0.5 * this.errorFunc(i);
        }),
        (NormalDistribution.prototype.invCumulativeProbability = function (t) {
            return this.Sqrt2 * this.invErrorFunc(2 * t - 1) * this.sd + this.mean;
        }),
        (NormalDistribution.prototype.errorFunc = function (t) {
            var i = 1 / (1 + 0.5 * Math.abs(t)),
                r =
                    1 -
                    i *
                        Math.exp(
                                                            ...
                                                            (i *
                                                                (0.27886807 +
                                                                    i *
                                                                        (i *
                                                                            (1.48851587 +
                                                                                i * (0.17087277 * i - 0.82215223)) -
                                                                            1.13520398)) -
                                                                0.18628806))))
                        );
            return t >= 0 ? r : -r;
        }),
        (NormalDistribution.prototype.invErrorFunc = function (t) {
            var i, r;
            if (((r = 0 == t ? 0 : t > 0 ? 1 : -1), 0 != t)) {
                var o = Math.log(1 - t * t),
                    a = o / 0.147,
                    s = o / 2 + 2 / (0.147 * Math.PI),
                    e = Math.sqrt(s * s - a);
                i = Math.sqrt(e - s) * r;
            } else i = 0;
            return i;
    ...
    (TDistribution.prototype.LogGamma = function (Z) {
        with (Math)
            var S =
                    ...
                    0.00120858003 / (Z + 4) -
                    536382e-11 / (Z + 5),
                LG = (Z - 0.5) * log(Z + 4.5) - (Z + 4.5) + log(2.50662827465 * S);
        return LG;
    }),
        (TDistribution.prototype.Betinc = function (t, i, r) {
            for (var o, a = 0, s = 1, e = 1, n = 1, u = 0, h = 0; Math.abs((e - h) / e) > 1e-5; )
                (h = e),
                    (s = n + (o = (-(i + u) * (i + r + u) * t) / (i + 2 * u) / (i + 2 * u + 1)) * s),
                    (e = (a = e + o * a) + (o = ((u += 1) * (r - u) * t) / (i + 2 * u - 1) / (i + 2 * u)) * e),
                    (a /= n = s + o * n),
                    (s /= n),
                    (e /= n),
                    (n = 1);
            ...
            with ((df || (df = this.df), Math))
                df <= 0
                    ...
                    : ((A = df / 2),
                      ...
                      (Z = df / (df + X * X)),
                      (BT = exp(
                          this.LogGamma(S) - this.LogGamma(0.5) - this.LogGamma(A) + A * log(Z) + 0.5 * log(1 - Z)
                      )),
                      Z < (A + 1) / (S + 2)
                          ? (betacdf = BT * this.Betinc(Z, A, 0.5))
                          : (betacdf = 1 - BT * this.Betinc(1 - Z, 0.5, A)),
                      (tcdf = X < 0 ? betacdf / 2 : 1 - betacdf / 2)),
                    (tcdf = round(1e5 * tcdf) / 1e5);
            return tcdf;
        }),
        (TDistribution.prototype.invCumulativeProbability = function (t, i) {
            i || (i = this.df);
            if (t >= 0.5) {
                s = 0;
                for (o = 0; o < 100 && !(this.cumulativeProbability(o, i) >= t); o++) s = o;
                for (var r = s, o = 0; o < 100 && !(this.cumulativeProbability(s + o / 100) >= t); o += 1)
                    r = s + o / 100;
                for (var a = r, o = 0; o < 100 && !(this.cumulativeProbability(r + o / 1e4) >= t); o += 1)
                    a = r + o / 1e4;
                return a;
            }
            for (var s = 0, o = 0; o < 100 && !(this.cumulativeProbability(-o, i) <= t); o++) s = o;
            for (var r = s, o = 0; o < 100 && !(this.cumulativeProbability(-s - o / 100) <= t); o += 1) r = s + o / 100;
            for (var a = r, o = 0; o < 100 && !(this.cumulativeProbability(-r - o / 1e4) <= t); o += 1) a = r + o / 1e4;
            return -a;
        }),
        (jss.TDistribution = TDistribution);
    var FDistribution = function (t, i) {
        (this.df1 = t), (this.df2 = i), (this.EPSILON = 1e-10);
    };
    (FDistribution.prototype.L504 = function (t, i, r, o) {
        var a = (t * i) / (t * i + r),
            s = Math.sqrt(a),
            e = Math.log(s),
            n = Math.sqrt(1 - a),
            u = Math.log(n),
            h = 1 - (2 * Math.atan(s / Math.sqrt(-s * s + 1))) / Math.PI,
            f = 0;
        if (1 != r) {
            v = Math.log((2 * s) / Math.PI);
            if (((h -= Math.exp(v + u)), 3 != r))
                for (var l = Math.floor((r - 3) / 2), b = 1; b <= l; b++) {
                    M = 2 * b + 1;
                    (c = (f += Math.log((M - 1) / M)) + u * M + v) > -78.4 && (h -= Math.exp(c));
                }
        }
        if (1 != t) {
            var v = f;
            if (
                (r > 1 && (v += Math.log(r - 1)),
                (v += Math.log(2 / Math.PI) + e + u * r) > -78.4 && (h += Math.exp(v)),
                3 != t)
            ) {
                l = Math.floor((t - 3) / 2);
                f = 0;
        ...
        (FDistribution.prototype.L401 = function (t, i, r, o) {
            var a = (t * i) / (t * i + r),
                s = Math.log(a),
                e = 0,
                n = (Math.log(1 - a) * r) / 2;
            if ((n > -78.4 && (e = Math.exp(n)), 2 != t))
                for (var u = Math.floor(t / 2 - 1), h = 0, f = 1; f <= u; f++) {
                    var l = 2 * f;
                    (h += Math.log(r + l - 2) - Math.log(l) + s) + n > -78.4 && (e += Math.exp(h + n));
                }
            return 1 == o && (e = 1 - e), e;
        }),
        (FDistribution.prototype.ProbF = function (t, i, r) {
            var o = r,
                a = t,
                s = i,
                e = 0;
            if (2 * Math.floor(a / 2) == a) return (n = this.L401(a, o, s, e));
            if (2 * Math.floor(s / 2) != s) {
                var n = this.L504(a, o, s, e);
                return n;
            }
            return (o = 1 / o), (a = i), (s = t), (e = 1), this.L401(a, o, s, e);
        }),
        (FDistribution.prototype.cumulativeProbability = function (t) {
    ...
    var ChiSquareDistribution = function (t) {
        this.df = t;
    };
    (ChiSquareDistribution.prototype.ChiSquaredProbability = function (t) {
        var i,
            r,
            o,
            a,
            ...
            n = 0,
            u = this.df,
            h = Math.log(Math.sqrt(Math.PI)),
            f = 1 / Math.sqrt(Math.PI);
        if (t <= 0 || u < 1) return 1;
        if (
            ((i = 0.5 * t),
            (even = parseInt((u / 2) * 2, 2) == u),
            u > 1 && (n = Math.exp(-i)),
            (r = even ? n : 2 * new jsstats.NormalDistribution(0, 1).cumulativeProbability(-Math.sqrt(t))),
            u > 2)
        ) {
            if (((t = 0.5 * (u - 1)), (s = even ? 1 : 0.5), i > 20)) {
                for (o = even ? 0 : h, a = Math.log(i); s <= t; )
                    (e = a * s - i - (o = Math.log(s) + o)), (r += Math.exp(e)), (s += 1);
                return r;
            }
            for (o = even ? 1 : f / Math.sqrt(i), a = 0; s <= t; ) (a += o *= i / s), (s += 1);
            return a * n + r;
        }
        return r;
    }),
        (ChiSquareDistribution.prototype.cumulativeProbability = function (t) {
            return 1 - this.ChiSquaredProbability(t);
        }),
...
function linRegression(M, N, idx, depidx, ftest) {
    if (depidx == undefined) {
        depidx = [];
        for (var k = 0; k < M.length; k++) if (idx != k) depidx.push(k);
    } else {
        if (!Array.isArray(depidx)) depidx = [0, depidx];
    }
    ...
    for (var s = 0; s < K; s++) {
        ...
        for (var j = 0; j < K; j++) {
            var i = depidx[j];
            a.push(M[i][k]);
            if (math.abs(M[i][k]) > maxi) maxi = math.abs(M[i][k]);
        }
        A.push(a);
    }
...
function regressout(data, stats) {
    ...
    for (var k = 0; k < data[0].length; k++) {
        var p = data[0][k];
        for (var j = 1; j < data.length; j++) p -= data[j][k] * stats.m[j];
        residual[k] = p;
    }
    return residual;
}
...
function pstats(stats, data, idx0, idx1) {
    ...
    var stats = {
        ...
        icc: cop / s,
        r2: cm,
        sdev_x: math.sqrt(s_0),
        sdev_y: math.sqrt(s_1),
        error: math.sqrt(err2),
        expvar: expvar,
        t: t,
        p: p,
        nans: data[0].length - cnt,
    };
    return stats;
}
...
function computeCorrelationMatrix(data, nointerscept) {
    ...
    for (var k = 0; k < data.length + !nointerscept; k++) {
        var m = [];
        for (var j = 0; j < data.length + !nointerscept; j++) m.push(0);
        M.push(m);
    }
    ...
    for (var k = 0; k < data[0].length; k++) {
        ...
        if (nointerscept) {
            for (var j = 0; j < data.length; j++)
                for (var s = 0; s < data.length; s++) M[s][j] += data[s][k] * data[j][k];
        } else
            for (var j = 0; j < data.length + 1; j++)
                for (var s = 0; s < data.length + 1; s++) {
                    if (s == 0 && j == 0) M[s][j] += 1.0;
                    else if (s == 0) M[s][j] += data[j - 1][k];
                    else if (j == 0) M[s][j] += data[s - 1][k];
                    else M[s][j] += data[s - 1][k] * data[j - 1][k];
                }
    }
...
function rankTransform(arr, datatransform) {
    if ((datatransform == undefined) | (datatransform == 0)) return arr;
    var res = [];
    var N = arr.length;
    var r = arr
        .map((i, e) => {
...
function organizeData(sheet, sels, type, datatransform, combicols, keepstring) {
    ...
    for (var j = 0; j < sels.length; j++) {
        ...
        if (combicols != undefined) {
            if (combicols[sels[j]]) combinatorial = true;
        }
        ...
        for (var k = 0; k < data.length; k++) {
            ...
            //if (combinatorial == true )
            {
                ...
                if (uniset[vk] == undefined) {
                    uniset[vk] = true;
                    if (Object.keys(uniset).length > 30) combinatorial = false;
                }
                dn.push(vk);
            }
            var vstr = v;
            if (typeof v == "string") v = parseFloat(v.replace(/,/, "."));
            if (isNaN(v) && keepstring) v = vstr;
            d.push(v);
        ...
        for (var k = 0; k < nk.length; k++)
            if (namekeys[nk[k]] == undefined) namekeys[nk[k]] = 1;
            else namekeys[nk[k]]++;

        if (combinatorial) {
            combinatorial_dataset.push({
                data: dn,
                uniset: uniset,
                name: n,
                name_short: n,
...
function ChartPanel(sheet, title, id) {
    ...
    if (id == undefined) id = "defaultchart";
    ...
    $downloadassvg.click(function () {
        ...
        if (panel.currentChart != undefined) panel.currentChart.destroy();
            ...
            if (activePoints.length > 0) {
                ...
                var cmenu = KContextMenu(
                    ...
                    function (str, ev, eo) {
                        ...
                        if (str == "select") $("input[name='PIZ']")[0].value = value.label;
                        if (str == "auto") {
                            var id = value.label.split("#");
                            if (id[1] != undefined) id[1] = "#" + id[1];
    ...
    $statinfo.on("mouseleave", function () {
        $statinfo.removeClass("visible");
    });
    ...
    function addStatinfo(obj, settarget, txt) {
        ...
        for (var k = 0; k < obj.length; k++) x += " </div> <div> " + obj[k];
        x += "</div></div>";
        var y = $(x);
        ...
        if (txt == "set target")
            y.append(
                $("<div class='tabledeactivate'> <i class='fa fa-close'> </i> </div>").click(function (e) {
                    settarget(e, true);
                })
            );
        $statinfo.append(y);
    ...
    function addOption(which, visname, id, def, type, options) {
        ...
        if (type == "option") {
            ...
            if ((type == "checkbox") & def) $par.find("input").attr("checked", 1);

            $par.find("input").on("change", function (e) {
                if (type == "checkbox") which[id] = e.target.checked;
                else which[id] = e.target.value;
                which.update(e);
            });
        }
    ...
    function scatterPlot(type_, sels_, args, originalTarget) {
        ...
        if (args != undefined) {
            for (var k in args) scatterPlot[k] = args[k];
        }
        ...
        if (sels == undefined) sels = Object.keys(type);
        ...
        var ftest = [];
        var { numerical_data, combinatorial_dataset } = organizeData(
            sheet,
            sels,
            type,
            scatterPlot.datatransform,
            sheet.combicols
        );
        for (var k in type) {
            if (type[k] == tidx) {
            ...
            if ((type[k] == sidx) & (type[k] != 0)) {
                ...
                if (other == -1) other = ot_;
                ftest.push(ot_);
            }
        }
        if (other == -1) other = (target + 1) % 2;
        if (target == -1) target = (other + 1) % 2;
        ...
        var stat = linRegression(
            M,
            N,
            target + !nointerscept,
            undefined,
            ftest.map((x) => x + !nointerscept)
        );
        console.log(stat);
        var txt_target = [
            "sdev: " + fixNumberStr(stat.sdev_y, -3),
        ...
        if (scatterPlot.predplot) {
            ...
            dummyArr.chunk(
                function () {
                    ...
                    totX++;
                    if (stat_scramb.expvar > stat.expvar) n_expv++;
                    if (stat_scramb.error < stat.error) n_err++;

                    $statinfo
                        .find(".KTargetPredP")
                        .html("p(exp.var>" + Math.floor(stat.expvar * 100) + "%|0hyp): " + log10Str(n_expv / totX));
        ...
        function allvsall(params) {
            ...
            var stats = [];
            var Q = ndata.length;
            for (var k = 0; k < Q; k++) stats.push(linRegression(M, N, k + 1));
            ...
            for (var r = 0; r < vars.length; r++) {
                ...
                for (var k = 0; k < Q + 1; k++) {
                    ...
                    var name;
                    if (k == Q) name = "i.cept";
                    else name = numerical_data[k].name.replace(/\./g, " ");
                    var stat = stats[k];
                    ...
                    for (var j = 0; j < Q + 1; j++) {
                        ...
                        if (k == Q) {
                            if (j == Q) v = "";
                            else v = stat[vars[r]][(j - 1 + 1) % Q];
                        } else if (j > k) v = stat[vars[r]][(j - 1 + 1) % Q];
                        else if (j == k) v = "";
                        else v = stat[vars[r]][(j + 1) % Q];
        ...
        for (var k = 0 + !nointerscept; k < stat.p.length; k++) {
            var txt = [
                "beta: " +
                    fixNumberStr((stat.m[k] * stat.sdev_x[k]) / stat.sdev_y, -1) +
                    " m: " +
                    fixNumberStr(stat.m[k], -3) +
                    ", sdev: " +
                    fixNumberStr(stat.sdev_x[k], -3),
                "p: <b>" + log10Str(stat.p[k]) + "</b>, t: " + fixNumberStr(stat.t[k], -1),
            ];
            var n = numerical_data[stat.idx[k] + nointerscept];
            if (type_[n.sel] == 0) txt.name = "<mark style='background:lightblue'>" + n.name + "</mark>";
            else txt.name = "<mark style='background:green; color:white;'>" + n.name + "</mark>";
            var settarget = (function (n) {
                return function (e, deactivate) {
                    if (deactivate) {
                        for (var i in type_) {
                            if (i == n.sel) type_[i] = type_[i] - 10;
                        }
                    } else {
                        var otype = -1;
                        var rtype = -1;
                        for (var i in type_) {
                            if (type_[i] == 2) otype = i;
                            else if (i == n.sel) rtype = i;
                        }
                        var tmp = type_[otype];
        ...
        if (ftest != undefined && ftest.length > 1) {
            ...
            for (var k = 0; k < ftest.length; k++) txt.push(numerical_data[ftest[k]].name + ";");
            txt.name = name;
            addStatinfo(txt);
        }
        ...
        if (stat.p.length == 2 && !nointerscept) {
            ...
            modelfit = [
                { x: min, y: stat.m[1] * min + stat.m[0] },
                { x: max, y: stat.m[1] * max + stat.m[0] },
            ];
            ...
            if (scatterPlot.predplot) {
                ...
                xy = ndata[target].map(function (e, i) {
                    var pred = stat.m[0];
                    for (var k = 1; k < stat.p.length; k++) pred += stat.m[k] * ndata[stat.idx[k]][i];
                ...
                xy = ndata[target].map(function (target_val, i) {
                    ...
                    for (var k = 1; k < stat.p.length; k++) {
                        if (k != other_sidx) cov += stat.m[k] * ndata[stat.idx[k]][i];
                    }
                    var other_val = ndata[other][i] + cov / stat.m[other_sidx];
                    if (isNaN(other_val)) cov;
                    var r = { y: target_val, x: other_val };
                    if (numerical_data[target].ids != undefined) r.label = numerical_data[target].ids[i];
                    return r;
                });
    ...
    function corrMatrix(type) {
        ...
        for (var k in type) if (type[k] > 1) type[k] = 3;
        /*
        var present = {}
        for (var k in type)
            present[type[k]] = 1;
...
*/
        ...
        for (var k = 0; k < sels.length; k++) {
            if (type[sels[k]] == 1) atype.push(k);
            else if (type[sels[k]] == 0) ctype.push(k);
            else btype.push(k);
        }
        if (btype.length == 0) btype = atype;
        ...
        for (var j_ = 0; j_ < atype.length; j_++) {
            ...
            for (var k_ = 0; k_ < btype.length; k_++) {
                ...
                if (j != k) {
                    ...
                    if (!isNaN(lm.p[1])) pcoll.push(lm.p[1]);
                    var logp = -Math.log(lm.p[1] + 0.000001);
                    if (logp > max_p) max_p = logp;
                    if (logp < min_p) min_p = logp;
                    lm.logp = logp;
                    prow.push(lm);
                } else prow.push(undefined);
            }
            pstat.push(prow);
        }

        var $tools = $("<div class='modernbuttongroup'></div>").appendTo(panel.$container);
        $("<div class='modernbutton small green'>p-val</div>")
            .appendTo($tools)
            .click(function () {
                createTable("p");
            });
        $("<div class='modernbutton small green'>t-val</div>")
            .appendTo($tools)
            .click(function () {
                createTable("t");
            });
        $("<div class='modernbutton small green'>slope</div>")
            .appendTo($tools)
            .click(function () {
                createTable("m");
            });
        $("<div class='modernbutton small green'>i.cept</div>")
            .appendTo($tools)
            .click(function () {
                createTable("b");
            });
        $("<div class='modernbutton small green'>r2</div>")
            .appendTo($tools)
            .click(function () {
                createTable("r2");
            });
        $("<div class='modernbutton small green'>expvar</div>")
            .appendTo($tools)
            .click(function () {
                createTable("expvar");
            });
        ...
        function exportResults(csv) {
            ...
            if (csv) initiateDownload(csv_export, projectInfo.name + "_statistics.csv");
            else {
        ...
        $("<div style='float:right' class='modernbutton small '>to PDF</div>")
            .appendTo($tools)
            .click(function () {
                exportResults(false);
            });
        $("<div style='float:right' class='modernbutton small '>to CSV</div>")
            .appendTo($tools)
            .click(function () {
                exportResults(true);
            });
        ...
        var r1 = -1;
        var cVN = 0;
        for (var i = 0; i < V; i++) cVN += 1 / (1 + i);
        for (var i = 0; i < V; i++) {
            if (pcoll[i] < ((i + 1) / V) * alpha) r0 = i;
            if (pcoll[i] < (((i + 1) / V) * alpha) / cVN) r1 = i;
        }
        var fdr0 = +pcoll[r0];
        var fdr1 = +pcoll[r1];
        var fdr2 = +0.05 / V;
        ...
        var covstr = "";
        for (var k = 0; k < ctype.length; k++) covstr += numerical_data[ctype[k]].name + ", ";
        ...
        function createTable(which, append, csv) {
            ...
            if (which == "p") {
                $(
                    "<span class='corrmatheading'> threshold at FDR 5% (Benjamini/Hochberg):" +
                        fdr0.toFixed(5) +
                        "* </span><br>"
                ).appendTo($tablebox);
                $(
                    "<span class='corrmatheading'> threshold at FDR 5% (Benjamini/Yekutieli):" +
                        fdr1.toFixed(5) +
                        "** </span><br>"
                ).appendTo($tablebox);
                $(
                    "<span class='corrmatheading'> threshold at 5% (Bonferoni):" + fdr2.toFixed(5) + "*** </span><br>"
                ).appendTo($tablebox);
            ...
            } else if (which == "t") $("<span class='corrmatheading'>t-value </span><br>").appendTo($tablebox);
            else if (which == "b") $("<span class='corrmatheading'>intercept </span><br>").appendTo($tablebox);
            else if (which == "m") $("<span class='corrmatheading'>slope </span><br>").appendTo($tablebox);
            else if (which == "expvar")
                $("<span class='corrmatheading'>explained variance </span><br>").appendTo($tablebox);
            else if (which == "r2") $("<span class='corrmatheading'>Pearsons r2 </span><br>").appendTo($tablebox);
            ...
            if (csv & (which == "p")) {
                ...
                if (covstr != "") csv_export += "Covariates:" + covstr + "\n";
            }
            csv_export += "\n\n" + which + ";";
            ...
            for (var j_ = 0; j_ < btype.length; j_++) {
                ...
                if (csv) csv_export += numerical_data[j].name + ";";
            }
            ...
            for (var j__ = 0; j__ < atype.length; j__++) {
                ...
                for (var k_ = 0; k_ < btype.length; k_++) {
                    ...
                    if (thestat != undefined) {
                        if (which == "m") v = thestat.m[1];
                        else if (which == "b") v = thestat.m[0];
                        else if ((which == "p") | (which == "t") | (which == "r2")) v = thestat[which][1];
                        else if (thestat[which] != undefined) v = thestat[which];
                        if (thestat.p[1] <= fdr0) star = "*";
                        if (thestat.p[1] <= fdr1) star = "**";
                        if (thestat.p[1] <= fdr2) star = "***";
                    }
                    var span;
                    if (v != undefined) {
                        if (csv) {
                            if ((which != "p") | (v < 0.05)) csv_export += v.toFixed(5) + ";";
                            else csv_export += ";";
                        }
                            ...
                            (function (k, j) {
                                return function () {
                                    ...
                                    if (Object.keys(numerical_data[k].uniset).length == 2) {
                                        ...
                                        var n =
                                            "Linear Model: " +
                                            numerical_data[k].name +
                                            " vs. " +
                                            numerical_data[j].name;
                                        var cpanel = ChartPanel(sheet, n, "chartlin_00");
        ...
        function sort($table, col, idx) {
            ...
            var dir = 1;
            if (idx != undefined && idx["desc"] == 1) dir = -1;
    ...
    function PRgraph(type) {
        ...
        function ROCPR(ndata, cdata, bootstrap) {
            ...
            for (var j = 0; j < L; j++) {
                var k = j;
                if (bootstrap) k = Math.floor(Math.random() * L);

                tup.push({ n: ndata[k], c: parseInt(cdata[k]) });
            }
            //	tup.push({n:Math.random(),c:parseInt(cdata[j])});
            tup = tup.sort(function (x, y) {
                return y.n - x.n;
            });
            var cumsum = tup.map((x) => x.c);
            for (var j = 1; j < cumsum.length; j++) cumsum[j] += cumsum[j - 1];
            ...
            for (var j = 0; j < N; j++) {
                ...
                if (tup[j].n > 0.5) acc_05 = acc;
            }
            AUC += (1 - lastx) * y;
            if (PRgraph.ROC) xy.push({ y: 1, x: 1 });
            else {
                xy.push({ y: 1, x: 0 });
                AUC = 1 - AUC;
            }
        ...
        var combicol = {};
        for (var k in type) if (type[k] == 2) combicol[k] = 1;
        ...
        for (var k = 0; k < numerical_data.length; k++) {
            ...
            if (PRgraph.bootstrap > -1) {
                ...
                AUC_ = AUC_.sort();
                conf =
                    "  (" + AUC_[math.floor(Q * 0.05)].toFixed(2) + ", " + AUC_[math.floor(Q * 0.95)].toFixed(2) + ")";
            }
            var { xy, AUC, acc_max, acc_05, max_th, toton, N } = ROCPR(ndata, cdata);

            var txt = [
                "accuracy@50%: " + acc_05.toFixed(2),
    ...
    function tstats(type) {
        ...
        var combicol = {};
        for (var k in type) if ((type[k] == 2) | sheet.combicols[k]) combicol[k] = 1;
        ...
        for (var k = 0; k < numerical_data_.length; k++) {
            if (numerical_data_[k].covariate) covariate_data.push(numerical_data_[k]);
            else numerical_data.push(numerical_data_[k]);
        }

        if (covariate_data.length > 0) {
            var cdata = covariate_data.map((x) => x.numerical_data);
            var didx = [0];
            for (var k = 0; k < covariate_data.length; k++) didx.push(k + 2);
            for (var k = 0; k < numerical_data.length; k++) {
        ...
        for (var j = 0; j < numerical_data.length; j++) {
            if (numerical_data[j].covariate) continue;

            function compstat(num_data, lab_data, name, min, max, width) {
                var mean = 0;
                var nancnt = 0;
                for (var k = 0; k < num_data.length; k++) {
                    if (isNaN(num_data[k]) | (num_data[k] == Infinity) | (num_data[k] == -Infinity)) nancnt++;
                    else mean += num_data[k];
                }
                ...
                var sorted_index = [];
                for (var k = 0; k < num_data.length; k++)
                    if (!isNaN(num_data[k])) sorted_index.push({ val: num_data[k], psid: lab_data[k] });
                var sorted_index = sorted_index.sort((x, y) => {
                    return x.val - y.val;
                });

                var sorted = sorted_index.map((x) => {
                    return x.val;
                });
                ...
                if (nancnt > 0) nanstr = nanstr + " nans/inf:" + nancnt;
                ...
                var str =
                    ...
                    name +
                    " </div> <div> mean: " +
                    fixNumberStr(mean, order) +
                    " </div> <div> sdev: " +
                    fixNumberStr(sdev, order) +
                    " </div> <div> p%(5,50,95): " +
                    fixNumberStr(p05, order) +
                    ", " +
                    fixNumberStr(p50, order) +
                    ", " +
                    fixNumberStr(p95, order) +
                    nanstr +
                    "</div></div>";
            ...
            if (combinatorial_dataset != undefined && combinatorial_dataset.length > 0) {
                ...
                function rec(k, cond) {
                    ...
                    for (var j in combi.uniset) rec(k + 1, cond.concat([j]));
                }
                function show(conds) {
                    var ndata = [];
                    var ldata = [];

                    OUTERLOOP: for (var s = 0; s < num_data.length; s++) {
                        for (var t = 0; t < conds.length; t++) {
                            if (combinatorial_dataset[t].data[s] != conds[t]) continue OUTERLOOP;
                        }
                ...
                compstat(
                    num_data,
                    numerical_data[j].ids,
                    numerical_data[j].name_short,
                    numerical_data[j].min,
                    numerical_data[j].max,
                    numpoints[j] / totnumpoints
                );
            }
        }
        ...
        for (var k = 0; k < means.length; k++) {
            for (var i = k + 1; i < means.length; i++) {
                var s2 = Math.sqrt(
                    (sdevs[k] * sdevs[k] * (gn[k] - 1) + sdevs[i] * sdevs[i] * (gn[i] - 1)) / (gn[i] + gn[k] - 2)
                );
            ...
            var charty = {
                ...
                options: {
                    ...
                    scales: {
                        yAxes: [
                            {
                                ticks: { beginAtZero: true, min: min, max: max },
                                scaleLabel: {
                                    display: true,
                                    labelString: "value",
                                },
    ...
    function seriesPlot(type) {
        ...
        function doformat(x) {
            var n = parseFloat(x);
            if (!isNaN(n)) return myformat.format(n);
            else return x;
        }
        for (var j = 0; j < sels.length; j++) {
        ...
        for (var j = 0; j < sheet.data.length; j++) {
            if (deselected_rows[j]) continue;
            var sdata = [];
            for (var k = 0; k < sels.length; k++) {
                if (sels[k] != color_col) {
                    var s = sortcrit[k];
                    if (sheet.data[j][s.j] != "") sdata.push({ x: k, y: sheet.data[j][s.j] });
                }
            }
...
function KTableViewer(viewport, master) {
    ...
    that.extendDragObj = function (obj) {
        if (obj.intent == undefined) obj.intent = {};
    ...
    function setContent(ev, intent) {
        ...
        sheet = {
            data: data,
            columns: colnames,
            deselected_rows: deselected_rows,
            rowidx: rowidx,
            combicols: combicols,
        };
        ...
        $div.on("scroll", function (e) {
            if ($div.scrollTop() > $div[0].scrollHeight - $div.height() - 5) renderTableChunk();
        });

        function renderTableChunk() {
            if (chunk_pos == undefined) return;
            ...
            for (var k = start; k < end; k++) {
                ...
                for (var i = 0; i < data[k].length; i++) {
                    ...
                    if (colnames[i] == "PSID") {
                        if (sheet.deselected_rows[k]) $row.addClass("excluded");
                        ...
                        function (str, ev, eo) {
                            if (str == "copy") {
                                ...
                                for (var k in sheet.deselected_rows) {
                                    if (sheet.deselected_rows[k]) str += sheet.data[k][0] + " ";
                                }
                ...
                $row.on("click", function (e) {
                    if (e.ctrlKey) {
                        ...
                        if (e.shiftKey) {
                            var i = $x.attr("row");
                            if (!deselected_rows[i]) $x = $x.prevUntil(".excluded").add($x);
                            else $x = $x.prevUntil(":not(.excluded)").add($x);
                        }
        ...
        lines.chunk(
            function (line, k) {
                if (line == "") return;
                if (aborter.aborted) return false;

                var entries = line.split(new RegExp(delimiter + '(?=(?:(?:[^"]*"){2})*[^"]*$)'));

                var $row = $("<tr></tr>");

                if ((k > 0) & (entries[0] != "")) header = false;
                if (header) {
                    header_lines++;
                    $row.appendTo($head);
                    $row.on(
                        ...
                        KContextMenu(
                            ...
                            function (str, ev, eo) {
                                ...
                                if (str == "deselectrows") {
                                    var cols = getCols();
                                    var str =
                                        "<table class='Ktablecolsel'> <head> <tr> <td> idx </td> <td> name </td> </tr> </head>";
                                    str += "<body>";
                                    ...
                                    alertify.prompt(
                                        "Columns selected:<br>" +
                                            str +
                                            "<br>" +
                                            "<b> Enter an expression. Refer to the value by n(idx). For example, write 'n(0)>0 & n(1)>0' to select all rows that are larger than zero in column 0 and 1. Leave empty to select all valid numbers.</b> ",
                                        function (e, str) {
                                            if (e) {
                                                default_deselect_expression = str;
                                                if (str == "") str = "!isNaN(n(0))";
                                                var type = {};
                                                for (var k in cols) type[cols[k]] = 1;

                                                deselected_rows = {};
                                                sheet.deselected_rows = deselected_rows;

                                                var { numerical_data, combinatorial_dataset, max, min } = organizeData(
                                                    sheet,
                                                    cols,
                                                    type,
                                                    undefined,
                                ...
                                if (str == "del") {
                                    ...
                                    for (var k = 0; k < $tr.length; k++) $($($tr[k]).children()[i]).remove();
                                    colg.remove();
                                    for (var k = 0; k < data.length; k++) data[k].splice(i, 1);
                                        ...
                                        function (e, str) {
                                            if (e) {
                                                ...
                                                for (var k = 0; k < hs.length; k++) {
                                                    ...
                                                    if (cell != undefined && p[k] != p_old[k]) cell.textContent = p[k];
                                                }
                                            }
                                        },
                                        cname
                                ...
                                if (str == "copy") {
                                    ...
                                    for (var k = 0; k < lines.length; k++) {
                                        var r = lines[k].split(";");
                                        var cl = cols.length;
                                        for (var s = 0; s < cl; s++) outstr += r[cols[s]] + (s == cl - 1 ? "" : ";");
                                        outstr += "\n";
                                    }
                                ...
                                if (str == "add") {
                                    ...
                                    alertify.prompt(
                                        "Columns selected:<br>" +
                                            ...
                                            "column from string equality write s(0)=='male'. For a custom name use // after the expression. Use Delta(idx,'str1','str2',..) for creating binary columns.",
                                        function (e, str) {
                                            if (e) {
                ...
                for (var j = 0; j < entries.length; j++) {
                    if (k == 0) {
                        ...
                        if (entries[j] != "") {
                            ...
                            if (!nate[j]) colnames[j] += "." + last;
                        }
                    }

                    if (!header) r.push(entries[j]);
                    else {
                        var entry = entries[j].replace(/\./g, " ");
                        if (entry == "") entry = "&nbsp";
                        entry = "<span>" + entry + "</span>";
                        if (k == 0) $row.append($("<td> " + entry + " <b class='combiflag'> combi. </b></td>"));
                        else $row.append($("<td> " + entry + "</td>"));
                    }
                }
                if (r.length > 0) {
                    data.push(r);
            ...
            function () {
                ...
                for (var k = 0; k < hds.length; k++) {
                    ...
                    if (l < idlen) {
                        for (var j = 0; j < idlen - l; j++) {
                            if (k == 0) $colg.append($("<col colhead=1>"));
                ...
                if (intent.intent && intent.intent.tablestate) that.setState(intent.intent.tablestate);
                else if (tobj && tobj.state) that.setState(tobj.state);
    ...
    that.addColumn = function (cols, str) {
        ...
        var k;
        function n(idx) {
            if (k == undefined) return colnames[cols[idx]];
        ...
        function delta_(ignore, idx, ...args) {
            ...
            if (val != undefined || ignore) {
                var res = [];
                for (var j = 0; j < args.length; j++) res.push(0);
                if (val != undefined) res[val] = 1;
            } else {
                var res = [];
                for (var j = 0; j < args.length; j++) res.push(NaN);
            }
            return res;
        }
        ...
        for (k = 0; k < data.length; k++) {
            ...
            for (var j = 0; j < val.length; j++) {
                if (val[j] === true) val[j] = 1;
                if (val[j] === false) val[j] = 0;
                if (isNaN(val[j])); // val[j] = "";
                data[k].push(val[j]);
            }
        }
        ...
        if (sstr.length == 1) {
            //eval("headname=" + str);
            headname = str;
        } else headname = sstr[1].trim();

        $body.children().remove();

        for (var k = 0; k < val.length; k++) {
            var headnameN;
            if (val.length > 1) headnameN = headname + " (" + k + ")";
            else headnameN = headname;
            var h = $head.children();
            for (var j = 0; j < h.length - 1; j++) $(h[j]).append($("<td> </td>"));
    ...
    that.getState = function () {
        var desel = [];
        for (var k in deselected_rows) if (deselected_rows[k]) desel.push(k);
        return {
            selections: selections,
            derived_cols: derived_cols,
            deselected_rows: desel,
            combicols: sheet.combicols,
        };
    };
    ...
    that.setState = function (state) {
        ...
        if (state.combicols) {
            for (var k in state.combicols) {
                var c = $(cf[k]);
                if (c.length > 0) c.addClass("combicol");
                sheet.combicols[k] = 1;
            }
        }
        ...
        for (var k in state.selections) {
            ...
            if (c.length > 0) {
                ...
                if (selections[k] == 0) c.addClass("tabcolselectedALT2");
                if (selections[k] == 2) c.addClass("tabcolselected");
                if (selections[k] == 1) c.addClass("tabcolselectedALT");
            }
        }
    };
    ...
    function updatetoolbar() {
        ...
        if ((num1 == 3) & (num2 == 1)) toolbar.$createano.removeClass("inactive");
        else toolbar.$createano.addClass("inactive");
    ...
    toolbar.$scatter = $("<div class='KViewPort_tool'><i class='fa fa-area-chart fa-1x'></i></div>")
        .click(function () {
            ...
            for (var k in selections) if (selections[k] == 2) origTarg = k;
    ...
    var contextmenu = KContextMenu(
        function () {
            ...
            if (!(updatetoolbar.index && updatetoolbar.sel)) metaimportinactive = "inactive";

            var saveinactive = "";
            if (tobj == undefined) saveinactive = "inactive";
            var $submenu;
            var $menu = $("<ul class='menu_context'>")
                .append($("<li onchoice='download' > download table </li>"))
                .append(
                    $(
                        "<li onchoice='metaimport' class='" +
                            metaimportinactive +
                            "' > import table as subject specific jsons</li>"
                    )
                )
        ...
        function (str, ev) {
            if (str != undefined) {
                if (str == "save") save();
                else if (str == "metaimport") metaimport();
                else if (str == "refetch") refetch_table();
                else if (str == "fetchdlg") fetch_table_dialog(tobj, state);
                else if (str == "download") initiateDownload(that.content.content, projectInfo.name + "_meta.csv");
                    ...
                    if (res.csv) {
                        var name =
                            "JOIN_" +
                            (that.currentFilename + "_" + tv[0].currentFilename).replace(/\.csv/g, "") +
                            ".csv";
                        KTableViewer.addTableFileObject(res.csv, { name: name });
                        popupView(
                            { content: res.csv, contentType: "tab", filename: name },
    ...
    function refetch_table() {
        ...
        xhr = ajaxRequest("command=exportmeta&json=" + JSON.stringify(tobj), function (e) {
            setContent(
                { content: e.content, contentType: "tab" },
                { intent: { singleview: true, tobj: tobj, tablestate: state } }
            );
        });
    }
    ...
    function fetch_table_dialog(tobj, state) {
        ...
        var ps = tobj.template_json.map(function (x) {
            return {
                URLType: "serverfile",
                fileID: x.fileID,
                callback: handle.updateKeySearch,
                intent: { asjson: true },
            };
        });
        for (var k = 0; k < ps.length; k++) KViewer.dataManager.loadData(ps[k]);

        return;
    }

    function metaimport() {
        if (metaimport.defaultjsonname == undefined) metaimport.defaultjsonname = "misc.json";
        alertify.prompt(
            "Selected columns will be imported as json. Column names will be used as keys in json format. Please enter a name for the resulting json file.",
            function (e, name) {
                if (e) {
                    var outstr = "";
                    var cols = Object.keys(selections);
                    ...
                    var header = "";
                    for (var k = 0; k < cols.length; k++) {
                        if (selections[cols[k]] == "2") header += psidcolname + ";";
                        else header += colnames[cols[k]] + ";";
                    }
                    outstr = header + "\n" + outstr;
                    ...
                    var xhr = ajaxRequest(
                        "command=importcsv&json=" + encodeURIComponent(JSON.stringify(obj)),
                        function (e) {
                            alertify.alert(
                                "Result of import of meta data into " +
                                    name +
                                    " of " +
                                    cols.length +
                                    " column(s) \n\n" +
    ...
    function save() {
        ...
        if (tobj.fileinfo != undefined) defaultfname = [tobj.fileinfo.SubFolder, tobj.fileinfo.studies_id];

        alertify.prompt(
            { msg: "Name of table:", opt: analysis_folders, optMsg: "Analysis folder to save" },
            function (e, val) {
                if (e) {
                    if (val.option != undefined) {
                        var finfo = {
                            patients_id: "ANALYSIS",
                            studies_id: val.option,
                            SubFolder: val.str,
                            tag: "TABCREATOR",
                        };
                        ...
                        uploadJSON("table.json", tobj, finfo, function (e) {
                            uploadBinary(
                                { content: original_content, filename: "content.csv", fileID: "" },
                                finfo,
                                function () {},
    ...
    function createPointset(type) {
        ...
        var defaults = [];
        if (ViewerSettings.createPointset_defaults != undefined) defaults = ViewerSettings.createPointset_defaults;
        else
            defaults = [
                coordexpression.join(","),
                "1",
                "if (" + others[0] + "==1) return cmap[2]; else return cmap[1]",
                "ID",
                that.currentFilename.split(" ")[0].replace(".ano", "").replace(".json", ""),
            ];
        ...
        alertify.prompt(
            [
                {
                    msg:
                        "<b>Create Pointset Annotation</b> " +
                        "<br> define how to map columns to point properties<br>" +
                        str +
                        "<br>" +
            ...
            function (e, ret) {
                if (e) {
                    if (!KViewer.markerTool.enabled) KViewer.markerTool.toggle();
...
function chartjs_errorbars() {
    ...
    const ErrorBarsPlugin = {
        ...
        afterDatasetsDraw(chart, easingValue, options) {
            ...
            const errorBarWidths = (Array.isArray(options.width) ? options.width : [options.width]).map((w) =>
                this._computeWidth(chart, horizontal, w)
            );
            ...
            barchartCoords.forEach((dataset, i) => {
                ...
                dataset.forEach((bar) => {
                    ...
                    if (hasLabelProperty) {
                        ...
                        bar.label &&
                        bar.label.label &&
                        Object.hasOwnProperty.call(cur, bar.label.label)
                    ) {
                    ...
                    errorBars.forEach((errorBar, ei) => {
                        ...
                        const errorBarColor = errorBarColors[ei % errorBarColors.length]
    ...
    if (typeof Chart != "undefined") Chart.pluginService.register(ErrorBarsPlugin);
}
...
function log10Str(x) {
    if (x == 0) return 0;
    if ((x > 0.01) & (x < 10)) return x.toFixed(2);
    var l = Math.log10(x);
    if (x == 0) return "10<sup>-inf</sup>";
    var s = Math.floor(l);
    return (x / Math.pow(10, s)).toFixed(2) + "&middot;10<sup>" + s + "</sup>";
}

function fixNumberStr(x, order) {
    if (isNaN(x) | (x == 0) | isNaN(order)) return x;
    if (0)
        //Math.abs(order) > 3)
        return x.toFixed(1) + "e" + order;
    else {
        if (x == undefined) return "undefined";
        if (order > 0) return "" + x.toFixed(1);
        else return "" + x.toFixed(-order + 1);
    }
}
...
KTableViewer.joinCSVs = function (t1, t2) {
    ...
    for (var k = 0; k < d1.data.length; k++) {
        ...
        if (id == "") continue;
        var tmp = [];
        for (var i = 0; i < ids1.length; i++) tmp.push(d1.data[k][ids1[i]]);
        if (join[id] == undefined) join[id] = [];
        join[id].push(tmp);
    }
    ...
    for (var k = 0; k < d2.data.length; k++) {
        ...
        if (id == "") continue;
        var tmp = [];
        for (var i = 0; i < ids2.length; i++) tmp.push(d2.data[k][ids2[i]]);
        if (join[id] == undefined) join[id] = [Array(ids1.length).fill("")];
        join[id] = join[id].map((x) => x.concat(tmp));
    }

    var newcolnames = [];
    for (var k = 0; k < ids1.length; k++) newcolnames.push(d1.colnames[ids1[k]]);
    for (var k = 0; k < ids2.length; k++) newcolnames.push(d2.colnames[ids2[k]]);
    ...
    for (var k in join) {
        ...
        for (var j = 0; j < rs.length; j++) {
            var jo = rs[j];
            if (jo.length < newcolnames.length) jo = jo.concat(Array(ids2.length).fill(""));
            csv += k + ";" + jo.join(";") + ";\n";
        }
    }
...
KTableViewer.addTableFileObject = function (content, tobj, callback) {
    ...
    hashfun("", JSON.stringify(tobj), function (fid) {
        ...
        if (tobj != undefined) {
            if (tobj.name != undefined) fileObject.filename = tobj.name;
        ...
        if (callback != undefined) callback(fileObject);
    });
};

function pasteCSV() {
    var default_delim = ";";
    if (pasteCSV.last_delim != undefined) default_delim = pasteCSV.last_delim;
    alertify.prompt(
        [{ msg: "Paste CSV/table below. Use as column delimiter:", addon: "<textarea id=KimportCSV> </textarea> " }],
        function (e, str) {
            if (e) {
    ...
    if (pasteCSV.lastContent != undefined) importEl.text(pasteCSV.lastContent);

    importEl[0].addEventListener("keyup", (event) => {
        event.stopPropagation();
    });

    importEl[0].addEventListener(
        "paste",
        (event) => {
