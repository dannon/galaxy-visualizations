   ...
   @function */
    that.loadData = function (params) {
        if (params.URLType === "localfile") {
            ...
            if (f == undefined) loadDataFromLocalFile(params);
            else if (params.callback) params.callback(f);
            ...
            if (params.callback) {
                ...
                if (params.intent && params.intent.shared && params.intent.shared.already_present)
                    ...
                    KViewer.roiTool.pushROI(
                        params.fileID,
                        params.intendedName,
                        undefined,
                        function (ev) {
                            if (params.intent && params.intent.shared) params.intent.shared.already_present = ev;
                            params.callback(ev);
                        },
                        undefined,
                        { donotcheckforexistence: true }
                    );
                return;
            }
    ...
    that.loadProxy = function (params, update) {
        var fobj = {
            proxyev: params,
            fileID: "proxy" + params.fileID.substring(5),
            fileinfo: {},
            filename: params.filename.substring(12),
        };
        if (params.SubFolder) fobj.fileinfo.SubFolder = params.SubFolder;

        setFile(fobj.fileID, fobj);

        params.fileID = fobj.fileID;
        if (update == undefined || update == true) KViewer.cacheManager.update();
    };
    ...
    function getFile(fid, params) {
        if (
            (fid.substring(0, 5) == "proxy") &
            (params != undefined) &
            (files[fid.replace("proxy", "local")] == undefined)
        ) {
            var proxy = files[fid];
            var reader = new FileReader();
            ...
            var fobj;
            if (files[fid.replace("proxy", "local")] != undefined) fobj = files[fid.replace("proxy", "local")];
            else fobj = files[fid];

            if (fobj != undefined)
                fobj.editable =
                    (fid.substring(0, 9) == "localfile") |
                    (fid.substring(0, 3) == "FVS") |
                    (fid.substring(0, 3) == "ROI") |
                    fobj.modified;
    ...
    that.getFileIdByNiiFile = function getFileIdByNiiFile(nii) {
        for (var id in files) {
            if (files[id].content == nii) return id;
        }
        return false;
    };
    ...
    that.refetchAllFiles = function () {
        ...
        function iterate(updated) {
            ...
            if (fobj.fileinfo.ID != undefined) {
                if (fobj.subfolder != undefined) var txt = fobj.subfolder + "/" + fobj.filename;
                else var txt = fobj.filename;

                pbar.progress(0, "checking for " + txt);
                that.refetchFile(
                    fobj.fileinfo,
                    function (p) {
                        if (p != undefined) pbar.progress(p * 100, "updateing " + txt);
                    },
                    iterate,
                    function () {
                        pbar.done();
    ...
    that.refetchFile = function (fileinfo, progress, callback, onerror) {
        ajaxRequest(
            "command=get_fileidentifier&json=" + JSON.stringify({ fileID: fileinfo.ID }),
            function (e) {
    ...
    function delFile(fid, donotRemoveFromViewer) {
            ...
            KViewer.iterateMedViewers(function (medV) {
                ...
                for (var j = 0; j < medV.overlays.length; j++) {
                    if (medV.overlays[j].currentFileID == fid) {
                        if (medV.overlays[j].content.refvisit_params) medV.unlinktck(medV.overlays[j]);
                ...
                if (medV.currentFileID == fid) medV.close();
            });
        ...
        if (files[fid] && files[fid].content && files[fid].content.octreeWorker) files[fid].content.octreeWorker.kill();
    ...
    function getNextIteratedFilename(basename, baselist) {
        ...
        for (var f in baselist) {
            if (baselist[f].filename.substring(0, basename.length) == basename) found = f;
        }
        if (found != undefined) {
            var m = baselist[found].filename.match(/\d+$/);
            if (m != null) basename = basename.substring(0, m.index) + (parseInt(m) + 1).toFixed(0);
            else basename = basename + "0";
        } else basename = basename + "0";
    ...
    function setCoregInfo(psid, coreginfo, del) {
        if (del == undefined) that.coregInfos[psid] = coreginfo;
        else delete that.coregInfos[psid];
    }
    ...
    function loadDataFromURL(params) {
        ...
        if (params.url == undefined && params.fileID != undefined) params.url = params.fileID;
        ...
        xhr.onerror = function (e) {
            alertify.error("url  " + params.url + " not loaded for unknown reasons");
            if (params.callback) params.callback(undefined);
        };
        ...
        xhr.onload = function (e) {
            if (this.status == 200) {
                ...
                alertify.error("url " + params.url + " not loaded (404)");
                if (params.callback) params.callback(undefined);
            }
        };
        xhr.send();
        ...
        client
            ...
            .then((sers) => {
                ...
                TheDicomReader.loadDicoms(
                    params_arr,
                    function (loadparams) {
                        for (var k = 0; k < loadparams.length; k++) KViewer.dataManager.loadData(loadparams[k]);
    ...
    function loadDataFromServer(params) {
        ...
        if (!$.isNumeric(params.fileID)) {
            if (params.fileID.substring(0, 5) == "meta_") {
                ...
                if (fobj != undefined) params.callback(fobj);
                else {
                    ...
                    if (atlas == undefined) {
                        ...
                        KViewer.atlasTool.getROIfromSinglelabel(
                            atlas,
                            ids[2],
                            atlas.content.labels[ids[2]].name,
                            atlas,
                            params.callback,
            ...
            } // fileID seems to be an autoloader pattern
            else {
                params.autoLoaderPattern = params.fileID;
                for (var id in files) {
                    if (files[id].autoLoaderPattern == params.fileID) {
                        if (params.callback) params.callback(files[id]);
                        return;
                    }
                }
            }
        ...
        if (fids == "proxy") {
            ...
            if (false & !electron) {
                that.refetchFile(
                    ...
                    function () {},
                    function () {
                        if (params.callback) params.callback(fids);
                    },
                    function () {
                        if (params.callback) params.callback(fids);
                    }
                );
            } else if (params.callback) params.callback(fids);
            return;
        }
        ...
        function execServerRequest(avoid_storage) {
            ...
            var sigid = signalhandler.attach("close", function () {
                xhr.abort();
                if (params.progressSpinner) params.progressSpinner();
            ...
            if (json == undefined) json = { project: currentModule };
            // projectmode
            else if (json.project == "") json.project = currentModule; // ??
                ...
                storage.getContents(docid).then(function (result) {
                    if ((result == undefined) | (result == ""));
                        ...
                        if (fileinfo.identifier == finfo.identifier) {
                            storage.getAttachment(docid, "blob").then(
                                function (data) {
                                    if (data != undefined) {
                                        ...
                                        if (fileinfo.num_access == undefined) fileinfo.num_access = 0;
                                        fileinfo.num_access++;
    ...
    function processLoadedFile(params, xhr) {
        ...
        function finalize() {
            if (fileObject.content !== false) {
                if (fileObject.fileID.substring(0, 5) != "meta_") {
                    ...
                    //todoMRC
                    if (0) {
                        //params.intent && params.intent.roi)
                        if (params.intent.color != undefined) fileObject.color = params.intent.color;
                        KViewer.roiTool.pushROI(fileObject.fileID, "untitled", "frommaskfile", function (fobj) {
                            if (params.intent.ccanalysis) createConnCompAnalysis(fobj);
                            if (params.callback) params.callback(fobj);
                        });
                        return;
                    }

                    if (fileObject.fileinfo && fileObject.fileinfo.filesize > 5000000) {
                        ...
                        if ((params.URLType == "serverfile") & (storage != undefined)) {
                            ...
                            function tryToSaveInCache() {
                                ...
                                else blob = new Blob([response], { type: "application/octet-binary" });
                                var finfo = JSON.parse(xhr.getResponseHeader("DPX-fileinfo"));
                                if (finfo != null) finfo.timeOfInsertion = new Date().getTime();
                                else {
                                    console.log("DPX-fileinfo missing for " + docid);
                                    return;
                                }
                                storage
                                    .setContents(docid, JSON.stringify(finfo))
                                    .then(function () {
                                        storage
                                            .setAttachment(docid, "blob", blob)
                                            .then(function () {})
                                            .catch(function () {
                                                storage.rm(docid);
                ...
                if (params.callback) params.callback(fileObject);
            }
        }
        ...
        if ((params.URLType === "localfile") | (params.URLType === "foreignurl")) {
            if (xhr.customBuffer !== undefined) xhr.response = xhr.customBuffer;
            else if (xhr.result != undefined) xhr.response = xhr.result;
            ...
            if (params.filename.search("//") > -1) fileObject.filename = params.filename.split("//")[1];
            else fileObject.filename = params.filename;

            fileObject.fileinfo = params.fileinfo;
            if (fileObject.fileinfo == undefined) fileObject.fileinfo = {};
            ...
            if (params.file && params.file.local) {
                ...
                fileObject.fileinfo.SubFolder = params.file.path.substring(
                    0,
                    params.file.path.length - params.file.name.length - 1
                );
            }
            ...
            try {
                // coregistration info for current study
                ...
                var psid;
                if (coreginfo.IDspecific != undefined) psid = fileObject.fileinfo.ID;
                else psid = fileObject.fileinfo.patients_id + fileObject.fileinfo.studies_id;
        ...
        if (params.SubFolder) fileObject.fileinfo.SubFolder = params.SubFolder;
        ...
        if ((fileObject.error !== undefined) & (fileObject.error !== null)) {
            ...
            if (params.onerror != undefined) params.onerror(err);
            else {
                alertify.lazy_error(err, "dberror");

                //also log the content...
                if (response.byteLength < 10000) console.log(ab2str(new Uint8Array(response)));
            }
            if (params.progressSpinner) params.progressSpinner();
            params.callback(undefined);
            return;
        ...
        if (xhr.status === 404) {
            $.notify(
                "Oops. An error occured: 404:\nFile not found.\nThis is most likely due to missing x-sendfile permissions, or a wrong :ROUTE:.",
                "error"
            );
            console.log(xhr.result);
            params.progressSpinner();
        ...
        if (fileObject.fileinfo.fileseries) {
            ...
            if (bruker !== undefined) {
                BrukerReader.vis2dseq2Nifti(
                    bruker.headerfile,
                    bruker.datafile,
                    fileObject.fileinfo.Filename,
                    0,
                    function (fobj) {
                        params.callback(fobj);
                    }
        ...
        if (fileObject.filename.search("\\.gz") > -1)
            if (params.progressSpinner != undefined) params.progressSpinner("unpacking");
        ...
        if ((fileObject.filename.search("\\.gz") > -1) | (fileObject.filename.search("\\.mgz") > -1)) {
            ...
            zip.createReader(
                new zip.BlobReader(new Blob([uint8Response])),
                function (reader) {
                    ...
                    reader.getEntries(function (entries) {
                        ...
                        for (var j = 0; j < entries.length; j++)
                            if (!entries[j].directory) fis[entries[j].filename] = entries[j];
		...
		*/
            ...
            if (
                state.viewer.loadBitmapAsNifti &&
                (fileObject.filename.search("\\.jpeg") > 0) |
                    (fileObject.filename.search("\\.jpg") > 0) |
                    (fileObject.filename.search("\\.png") > 0) |
                    (fileObject.filename.search("\\.tif") > 0) |
                    (fileObject.filename.search("\\.tiff") > 0) |
                    (fileObject.filename.search("\\.bmp") > 0)
            ) {
                fileObject.filename += ".nii";
                fileObject.content = bmpToNIFTI(uint8Response, processfileObject);
            } else processfileObject(uint8Response);
        }

        function processfileObject(uint8Response) {
            if (
                (fileObject.filename.search("\\.nii") > 0) |
                (fileObject.filename.search("\\.mgh") > 0) |
                (fileObject.filename.search("\\.mgz") > 0) |
                (fileObject.filename.search("\\.nrrd") > 0) |
                (fileObject.filename.search("\\.mif") > 0)
            ) {
                ...
                //try
                {
                    ...
                    var niibuf;
                    if (fileObject.filename.search(".nrrd") > 0) niibuf = nrrd.parse(buf);
                    else if (fileObject.filename.search(".mif") > 0) niibuf = parse_mif(buf);
                    else if ((fileObject.filename.search(".mgh") > 0) | (fileObject.filename.search(".mgz") > 0))
                        niibuf = parse_mgh(buf);
                    else niibuf = parse(buf);
			...
			{    
                ...
                else fileObject.content = response;
            } else if (
                (fileObject.filename.search("\\.py") > 0) |
                (fileObject.filename.search("\\.m") > 0) |
                (fileObject.filename.search("\\.txt") > 0) |
                (fileObject.filename.search("\\.yml") > 0) |
                (fileObject.filename.search("\\.bexclude") > 0) |
                (fileObject.filename.search("\\.bval") > 0) |
                (fileObject.filename.search("\\.bvec") > 0) |
                (fileObject.filename.search("\\.time") > 0)
            ) {
                fileObject.contentType = "txt";
                //fileObject.content = ab2str( uint8Response);
                // umlaute did not work so far, with this they do
                if (typeof response == "string") fileObject.content = response;
                else fileObject.content = robust_ab2str(uint8Response);
                ...
                !(params.intent != undefined && params.intent.asjson) &
                ...
                } else sets = markerProxy.loadAnnotations(fileObject, undefined, params, true);
                ...
                if (
                    ...
                    markerProxy.currentSet.markerPanel &&
                    markerProxy.currentSet.markerPanel.panelvisible
                ) {
                    // do nothing ( no show )
                } else {
                ...
                var ps = tobj.template_json.map(function (x) {
                    return { URLType: "serverfile", fileID: x.fileID, callback: handle.updateKeySearch };
                });
            ...
            //      }
                ...
                //fileObject.filename == "melodic.json")
                ...
                if (!KViewer.curveTool.enabled) KViewer.curveool.toggle();
                KViewer.curveTool.setstate("mode", "fMRI");
                KViewer.curveTool.fmri.tmodes = fileObject;
            } else if (
                fileObject.filename.search("\\.json") > 0 ||
                fileObject.filename.search("\\.reading") > 0 ||
                fileObject.filename.search("\\.stats") > 0 ||
                fileObject.filename.search("\\.xml") > 0
            ) {
                fileObject.contentType = "json";
                fileObject.content = robust_ab2str(uint8Response);

                if (fileObject.content == "" && typeof response == "string") fileObject.content = response;
                if (params.URLType == "localfile") {
                    ...
                    if (tmp.content) fileObject.content = JSON.stringify(tmp.content);
                    if (tmp.tag) fileObject.fileinfo.Tag = tmp.tag;
                }
                if (
                    ((fileObject.fileinfo.Tag || "").search("/TCKSEL/") > -1) |
                    (fileObject.filename.search("\\.tck\\.json") > 0)
                ) {
                    var tcksel;
                    ...
                    var fcallback = function (fp) {
                        ...
                        if (fp == undefined) {
                            alertify.error(
                                "Tracts associated with selection not found, link broken. " +
                                    "Load correct associated tck into workspace and try again.",
                                "error"
                            );
                            params.callback(undefined);
                        } else {
                            ...
                            if (params.intent.select == undefined && params.intent.visible !== false) {
                                if (params.intent.drop | params.intent.dblclick) params.intent.select = "all";
                            }
                            if (fibs.content.selections == undefined) fibs.content.selections = [];
                            ...
                            if (fibs.tckjsonref == undefined) fibs.tckjsonref = {};
                            ...
                            for (var k = 0; k < tcksel.selections.length; k++) {
                                ...
                                if (tcksel.selections[k].subset.length == 0) continue;
                                if (fibs.tckjsonref[fileObject.fileID] == undefined) {
                                    var match = current.findIndex((x) => x == tcksel.selections[k].name);
                                    if (match == -1) fibs.content.selections.push(tcksel.selections[k]);
                                    else {
                                        if (
                                            fibs.content.selections[match].subset.length !=
                                                tcksel.selections[k].subset.length ||
                            ...
                            if (params.intent.subset_to_open.length == 0) params.intent.subset_to_open = undefined;

                            if (fibs.tckjsonref[fileObject.fileID] == undefined)
                                fibs.tckjsonref[fileObject.fileID] = fileObject;

                            if (params.callback) params.callback(fibs);
                    ...
                    for (var k = 0; k < files.length; k++) {
                        ...
                        if (
                            (file.content.md5 != undefined && file.content.md5 == tcksel.assoc.md5) ||
                            file.fileID == tcksel.assoc.fileID
                        ) {
                            par.fileID = files[k];
                            found = true;
                            break;
                    ...
                    if (fileObject.fileinfo.permission == "link" && that.getFile(par.fileID) == undefined) {
                        if (fileObject.fileinfo.patients_id == "ANALYSIS") {
                            if (tcksel.assoc.subfolder == "") par.fileID = tcksel.assoc.filename + " PPIZ:ANALYSIS";
                            else par.fileID = tcksel.assoc.subfolder + "/" + tcksel.assoc.filename + " PPIZ:ANALYSIS";
                        }
                    }
                    /*
...
*/
                    ...
                    if (typeof fileObject.content != "object") {
                        KViewer.obj3dTool.prepareConmatData(
                            fileObject,
                            function (txt) {
                    ...
                    ((fileObject.fileinfo.Tag || "").search("workstate") >= 0) |
                    (fileObject.filename.search("\\.workstate\\.json") > 0)
                ) {
                    ...
                    if (!ironSight.isVisible()) ironSight.toggle();
                    setTimeout(function () {
                        ironSight.import(state);
                        params.progressSpinner();
                    }, 1000);
                ...
                KViewer.obj3dTool.prepareSurfaceData(
                    fileObject,
                    uint8Response,
                    function (txt) {
                        params.progressSpinner(txt);
                ...
                KViewer.obj3dTool.prepareFiberData(
                    fileObject,
                    uint8Response,
                    function (txt) {
                        if (params.progressSpinner != undefined) params.progressSpinner(txt);
                    },
                    function () {
                        if (fileObject.content == undefined) {
                ...
                if (typeof TheDicomReader != "object") TheDicomReader = new DicomReader();
    ...
    function prepA75_HdrImg(fis, progressSpinner, fileObject, processfileObject) {
        function progress(str) {
            if (progressSpinner != undefined) progressSpinner("unpacking");
            ...
            function (blob_hdr) {
                fis["img"].getData(
                    ...
                    function (current, total) {
                        progress("unzipping " + math.round((current / total) * 100) + "%");
                    }
                );
            },
            function (current, total) {
    ...
    function clearMemory() {
        if (KViewer.roiTool && KViewer.roiTool.isinstance) KViewer.roiTool.clearAll();
        if (typeof KWMQLPanel != "undefined") {
            for (var k in KWMQLPanel.panels) KWMQLPanel.panels[k].close();
            KWMQLPanel.panels = {};
        }
        ...
        for (var k = 0; k < obs.length; k++) {
            if (files[obs[k]].fileinfo && files[obs[k]].fileinfo.Tag) {
                if (files[obs[k]].fileinfo.Tag.search(/\/static\//) > -1) continue;
            }
    ...
    function cloneAsROI(id, name, lim, params) {
        ...
        if (lim == "frommultiroifile") {
            ...
            for (var k in labels) {
                ...
                if (newobj.fileinfo.Filename != undefined) newobj.fileinfo.Filename = newobj.filename;
            ...
            newobjs = [newobj];
        } // from other image file by thresholding/zero init
        else {
        ...
        function clone(fobj, name, lim) {
            ...
            else var fileObject = cloneNifti(fobj, name, "uint8");
            ...
            if (fobj.intendedROIid != undefined) roiid = fobj.intendedROIid;
            ...
            if (lim) {
                ...
                if (KViewer.roiTool.mode4D) offs = 0;
                var thelim = parseFloat(lim.substring(5));
                if (lim.substring(0, 5) == "lower")
                    for (var k = 0; k < fileObject.content.data.length; k++) {
                        fileObject.content.data[k] =
                            (fobj.content.data[k + offs] < thelim) & (fobj.content.data[k + offs] != 0) ? 1 : 0;
                    }
            ...
            } else for (var k = 0; k < fileObject.content.data.length; k++) fileObject.content.data[k] = 0;
...
function createLoadParamsFileDrop(e, callback, progress) {
    function validFileExt(p) {
        if (p.isdicom | p.isbruker) return false;
        else return true;
    ...
    function syncImport(params, finalcb) {
        ...
        if (bruker != undefined) {
            if (progress) progress("converting bruker data");
            setTimeout(function () {
                BrukerReader.loadBruker(
                    bruker,
                    function () {
                        niiImport();
                        progress();
        ...
        for (var k = 0; k < params.length; k++) {
            if (params[k].filename.search("\\.zip") > -1) {
                ...
                freader.onload = function (e) {
                    ...
                    zip.useWebWorkers = false;
                    zip.createReader(
                        new zip.BlobReader(new Blob([freader.result])),
                        function (reader) {
                            // get all entries from the zip
                            reader.getEntries(function (entries) {
                                reader.close();
                                ...
                                if (loadParamsZip.length > 0) callback(loadParamsZip); // rest goes the normal way
                            });
                        },
                        function (err) {
                            progress();
                ...
                if (str) params[k].SubFolder = str.replace(/^\/|\/$/g, "");
    ...
    if (e.clipboardData != undefined) dt = e.clipboardData;
    ...
    if (
        ...
        var ientries = [];
        for (var k = 0; k < dt.items.length; k++) ientries.push(dt.items[k].webkitGetAsEntry());

        var workpackages = [];
        var to_be_aborted = false;
        progress("reading file information", function () {
            to_be_aborted = true;
        });
            ...
            function runSync() {
                if (workpackages.length > 0) {
                    ...
                    syncImport(p.params_arr, function () {
                        if (p.params_arr.to_be_aborted) return;
                        runSync();
                    });
                }
            }
        ...
        function browseDir(entries, parent, str, cb) {
            ...
            for (var k = 0; k < entries.length; k++) {
                ...
                if (
                    entry_ == "[object DirectoryEntry]" ||
                    entry_ == "[object FileSystemDirectoryEntry]" ||
                    entry.isDir
                ) {
                } else {
            ...
            function iterateEntries() {
                ...
                if (entry.file == undefined) readSingleEntry(entry, iterateEntries);
                else
                    entry.file(
                        function (fileobject) {
