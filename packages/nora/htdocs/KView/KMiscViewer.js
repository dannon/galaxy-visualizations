...
function KPrototypeViewer(viewport, master) {
    ...
    that.toolbar = {};
    that.toolbar.$container = $("<div class='KViewPort_toolbar'></div>")
        .appendTo(that.$topRow)
        .click(function (ev) {
            return false;
        });
    ...
    that.toolbar.$container.on("mousemove", function (e) {
        if (!that.toolbar.issticky) {
            ...
            var trg = $(e.target);
            if (!trg.hasClass("KViewPort_tool")) trg = trg.parent();
            if (
            ...
            if (that.toolbar.stdelay != undefined) clearTimeout(that.toolbar.stdelay);
            that.toolbar.stdelay = setTimeout(function () {
                that.toolbar.show_addons(e, trg);
                that.toolbar.stdelay = undefined;
            }, 150);

            if (that.toolbar.id != undefined) clearTimeout(that.toolbar.id);
    ...
    that.toolbar.$zoom = $("<div class='KViewPort_tool'><i class='fa fa-expand fa-1x'></i></div>")
        ...
        .appendTo(that.toolbar.$container)
        .appendTooltip("zoomviewport");
    that.toolbar.$screenshot = $("<div class='KViewPort_tool'><i class='fa fa-camera fa-1x'></i></div>")
        .on("mousedown", function (e) {
            e.stopPropagation();
        })
        .click(function () {
            if (that.type != "mainview") takeScreenshot();
        })
        .appendTo(that.toolbar.$container)
        .appendTooltip("screenshot");
    ...
    that.toolbar.$close = $(
        "<div  class='KViewPort_tool KViewPort_tool_close'>  <i class='fa fa-close fa-1x'></i></div>"
    )
        .click(function () {
            that.close();
        })
    ...
    that.toolbar.get_addons = function (e, trg) {
        if (e == undefined)
            return that.toolbar.$container
                .find(".KToolbarSep")
                .find(".KViewPort_tool")
                .not("input, .persistent, .caption, .draganddrop, .KViewPort_tool_cmap, .KToolbar_sticky");
        else {
            if (trg == undefined) trg = e.target;
    ...
    that.toolbar.show_addons = function (e, trg) {
        ...
        if (trg == undefined && e != undefined) trg = $(e.target);
        // if (e == undefined)
        {
    ...
    that.toolbar.$stickydiv = $(
        "<div  class='KViewPort_tool KToolbar_sticky'>  <i class='fa fa-fw fa-circle-o fa-1x'></i></div>"
    );
    ...
    if (that.toolbar.issticky) that.toolbar.$stickydiv.find("i").removeClass("fa-circle-o").addClass("fa-circle");
    ...
    for (var k = 0; k < types.length; k++) {
        ...
        if (types[k] != "sticky") {
            that.toolbar[types[k]].$sep = $(
                "<div class='Kmintoolsep'> <i class='fa fa-minus-square-o'></i></div>"
            ).appendTo(that.toolbar[types[k]].$div);
            that.toolbar[types[k]].$sep.hide();
            that.toolbar[types[k]].$sep.on(
                "mousedown",
                (function (t) {
                    return function () {
                        ...
                        if (!a.$div.hasClass("minimized")) {
                            ...
                            if (that.toolbar.issticky) {
                                that.toolbar
                                    .get_addons(a.$div)
                                    .removeClass("KTool_hidden")
                                    .removeClass("KTool_displaynone");
                                that.toolbar.$container.find(".KViewPort_tool.caption").removeClass("equalwidth");
                            } else that.toolbar.$container.find(".KViewPort_tool.caption").addClass("equalwidth");
                        }
                    };
                })(types[k])
            );
    ...
    that.toolbar.$info
        ...
        .on(
            ...
            dragstarter(function () {
                return {
                    fromViewPort: that.viewport.viewPortID,
                    intent: { viewport: that.viewport.viewPortID },
                    type: "file",
                    mime: that.contentType,
                    filename: that.currentFilename,
                    fileID: that.currentFileID,
    ...
    that.toolbar.append = function (divs, type) {
        ...
        for (var k = divs.length - 1; k >= 0; k--) divs[k].appendTo($where);
        //	if (that.toolbar.issticky)
        //	   that.toolbar.show_addons();
        if (that.toolbar[type].$div.find("br").length > 2) that.toolbar[type].$sep.show();
    };

    that.toolbar.update = function (type) {
        if (type == undefined) t = types;
        else t = [type];
        for (var k = 0; k < t.length; k++) {
            if (that.toolbar[t[k]].$sep != undefined) {
                if (that.toolbar[t[k]].$div.find("br").length > 2) that.toolbar[t[k]].$sep.show();
                else that.toolbar[t[k]].$sep.hide();
            }
        }
    };
    ...
    that.toolbar.appendAfter = function (divs, which) {
        ...
        for (var k = 0; k < divs.length; k++) divs[k].insertAfter($after);
        if (that.toolbar.issticky) that.toolbar.show_addons();
        if (which.type != undefined)
            if (that.toolbar[which.type].$div.find("br").length > 2) that.toolbar[which.type].$sep.show();
    };
    ...
    that.layoutbar.$container = $("<div class='KViewPort_layoutbar'></div>")
    ...
    that.layoutbar.$zoomin = $(
        "<div class='KViewPort_tool_layout'><i class='fa fa-search-plus fa-1x'></i></div>"
    ).appendTo(that.layoutbar.$container);
    that.layoutbar.$zoomout = $(
        "<div class='KViewPort_tool_layout'><i class='fa fa-search-minus fa-1x'></i></div>"
    ).appendTo(that.layoutbar.$container);
    /** @function */
    ...
    /** @function */
    that.layoutbar.show = function () {
        that.layoutbar.$container.show();
    };
    /** @function */
    that.layoutbar.hide = function () {
    ...
    that.statusbar.$container = $("<div class='KViewPort_statusbar'> good morning </div>")
        .appendTo(that.$container)
        .click(function (ev) {
            return false;
        })
    ...
    that.statusbar.report = function (string) {
        ...
        if (that.sid != -1) clearTimeout(that.sid);
    ...
    that.setInfoText = function (txt) {
        if (that.$infobar != undefined) $(that.$infobar[0]).html(txt);
    };
    ...
    that.prepViewer = function (ev, params) {
        ...
        if (ev.fileinfo && ev.fileinfo.SubFolder != undefined && ev.fileinfo.SubFolder != "")
            that.toolbar.$info.html(
                ev.fileinfo.patients_id + ev.fileinfo.studies_id + " " + ev.fileinfo.SubFolder + "/" + ev.filename
            );
        else that.toolbar.$info.html(ev.filename);
        if (params && params.hideControls) that.toolbar.hide();
        else that.toolbar.show();
    };
    ...
    function close() {
        if (that.currentFileID != undefined) {
            if (that.customClose != undefined) that.customClose();
            that.toolbar.hide();
    ...
    that.sigid_close = signalhandler.attach("close", function () {
        that.close();
    });
    ...
    that.closeUnreferenced = function () {
        ...
        for (var k in KViewer.viewports) {
            if (KViewer.viewports[k] != that.viewport)
                if (
                    KViewer.viewports[k].getCurrentViewer() != undefined &&
...
function KViewerJS(viewport, master) {
    ...
    function setContent(ev) {
        ...
        else renderPdfBlobToIframe(ev.content.buffer, $div[0]);
    ...
    function setImageLayout() {
        ...
        if (that.pdf) $div.scrollTop(that.scrolltop * ($div.children().height() * that.pdf.numPages));
    }
    ...
    function zoom(fac) {
        ...
        var left;
        if ($ref.width() / wid > 1) left = ($ref.width() - wid * fac * Math.floor($ref.width() / wid)) / 2;
        else left = ($ref.width() - wid * fac) / 2;
...
function KJsonViewer(viewport, master) {
    ...
    function unfold() {
        ...
        for (var k = 0; k < labels.length; k++) {
            if ($(labels[k]).next()[0].checked == s) $(labels[k]).trigger("click");
        }
    }
    ...
    that.getState = function () {
        ...
        for (var k = 0; k < nodes.length; k++) state.push(nodes[k].checked);
    ...
    that.setState = function (s) {
        var nodes = $a.find("input");
        for (var k = 0; k < nodes.length; k++) nodes[k].checked = s.fold[k];
    };

    toolbar.$fold = $("<div class='KViewPort_tool'><i class='fa fa-folder-open fa-1x'></i></div>").click(unfold);

    toolbar.$download = $("<div class='KViewPort_tool'><i class='fa fa-download fa-1x'></i></div>").click(function () {
        var fname = that.content.filename;
        if (fname.search("\\.json") == -1) fname += ".json";
    ...
    toolbar.$save = $("<div class='KViewPort_tool'><i class='fa fa-save fa-1x'></i></div>").click(function () {
        ...
        if (that.currentFileID.substr(0, 4) == "meta") {
            ...
            if (psid.length <= 2) {
                ...
                if (psid.length == 2) {
                    sid = "#" + psid[1];
                } else obj = json_obj["STUDIES"];

                myquery[0].push({
                    command: "save_patientinfo_studyinfo",
                    json: { piz: psid[0], sid: sid, content: obj },
                });
                ajaxRequest(myquery, function (e) {
                    if (e.json[0].success == 1) alertify.success("studyinfo saved");
                    else alertify.error("studyinfo saving failed: " + e.json[0].msg);
                });
            }
        } else {
...
*/
    ...
    function setContent(params, ev) {
        if (ev.obj != undefined) that.currentFileinfo = ev.obj.fileinfo;
        ...
        if (str.content != undefined) d = str.content;
        else {
            try {
        ...
        if (d.formcontent != undefined) d = d.formcontent;

        if (ev && ev.intent && ev.intent.editable != undefined && ev.intent.editable == false) editable = false;

        if (!editable) toolbar.$save.css("display", "none");
        else toolbar.$save.css("display", "inline-block");
        ...
        if (ev != undefined && ev.intent != undefined && ev.intent.field != undefined) {
            ...
            if (Object.keys(dspec).length > 0) d = dspec;
                ...
                if (str) {
                    ...
                    if (str == "delete") {
                        if (path != undefined) {
                            var obj = json_obj;
                            index = path.split(".");
                            for (var k = 0; k < index.length - 1; k++) obj = obj[index[k]];
                    ...
                    if (str == "add") {
                        var $tr = getOnParentPath($(ev2.target), function (x) {
                            return x.attr("path") != undefined;
                        });
                        ...
                        var def;
                        if (index.length > 2) def = index[index.length - 2] + "_" + index[index.length - 1];
                        else def = index[index.length - 1];
                        def = def.replace("\\.", "_");
                        ...
                        alertify.prompt(
                            [
                                { msg: "Create meta index with key sequence:" },
                                {
                                    msg: 'Give a name for the metaindex (no special chars).<br>Start with "/" to share with all users.',
                                    opt: ["FLOAT", "INT", "STRING", "MEDIUMTEXT"],
                                    optMsg: "Datatype of Index",
                                },
                            ],
                            function (e, val) {
                                if (e) {
        ...
        function addChildren($parent, $currentNode, obj, attrarr, editable) {
            ...
            if (isarray & $.isNumeric(obj[0]) & (obj.length > 50)) {
                ...
                if ($parent != undefined) $parent.append($currentNode);
                return $currentNode;
            }

            var keys = Object.keys(obj);
            if (!isarray) keys.sort();
            if (keys.length > 1000) {
            ...
            for (var k = 0; k < keys.length; k++) {
                ...
                if (obj.hasOwnProperty(property)) {
                    ...
                    if (typeof obj[property] == "object" && obj[property] != undefined) {
                        ...
                        if (
                            (property.substr(0, 6) == "[FORM]") |
                            (property.substr(0, 5) == "[SQL]") |
                            (property.substr(0, 6) == "[META]") |
                            (property.substr(0, 7) == "[DICOM]")
                        ) {
                        ...
                        var $newNode = $(
                            "<li><label class=" +
                                (children_editable ? "'jsoncontenteditable'" : "") +
                                " for='" +
                                theid +
                                "_" +
                                property +
                        ...
                        setTimeout(
                            ...
                            })($newNode, $("<ol/>"), children, subattr, children_editable),
                            100
                        );
                        //			  	addChildren($newNode,$("<ol/>"), obj[property],subattr)

                        $newNode.on("contextmenu", function (ev) {
                            ev.preventDefault();
                        });
                    ...
                    if (typeof obj[property] != "object") {
                        ...
                        $currentNode.append(
                            $(
                                "<li  path='" +
                                    subattr.join(".") +
                                    "' class='file'> <span class='key' >" +
                                    property +
                                    "</span> : <span class=" +
                                    (editable ? "'jsoncontenteditable'" : "") +
                                    " contenteditable='" +
                                    (editable ? "true" : "false") +
                                    "' >" +
                                    val +
                                ...
                                .on("dragstart", function (e) {
                                    var $x = $(e.target);
                                    for (var k = 0; k < 3; k++) {
                                        if ($x.attr("path") != undefined) break;
                                        else $x = $x.parent();
                                    }
                                    var p = $x.attr("path");
                                    var str;
                                    if (that.currentFileinfo.SubFolder != "")
                                        str =
                                            that.currentFileinfo.SubFolder +
                                            "/" +
                                            that.currentFileinfo.Filename +
                                            " " +
                                            p;
                                    else str = that.currentFileinfo.Filename + " " + p;
                                    e.originalEvent.dataTransfer.setData("text/plain", str);
                                })
                                .on("keyup", function (e) {
                                    var $txt = $(e.target);
                                    var $tr = getOnParentPath($txt, function (x) {
                                        return x.attr("path") != undefined;
                                    });
    ...
    function addChart($tmp, obj, attr) {
        ...
        var str =
            "<div class=Kjsoninlinechart> <svg preserveAspectRatio='none' " +
            vb +
            ">" +
            poly.join(" ") +
            "</svg></div>";
        ...
        if (that.currentFilename == "melodic.json") {
            ...
            $info.click(function (e) {
                e.preventDefault();
                KViewer.setCurrentTimePoint(tp);
            });
        } else {
            info = "<div> " + info + " </div>";
            var $info = $(info);
...
function KTXTViewer(viewport, master) {
    ...
    toolbar.$search = $("<div class='KViewPort_tool'><i class='fa fa-search fa-1x'></i></div>").click(function () {
        KTXTSearchPanel();
    });
    ...
    function getText() {
        var str;
        if (editor != undefined) str = editor.getValue();
        else str = $div.html();
    ...
    function textFromDivToObject() {
        if (that.currentFileID == "WorkstatePostCode") {
            ...
            else editor.setValue(KViewer.WorkstatePostCode);
        } else {
            var fobj = KViewer.dataManager.getFile(that.currentFileID);
            if (editor != undefined) editor.setValue(fobj.content);
            else $div[0].innerHTML = fobj.content;
        }
    ...
    function setContent(ev, p) {
        ...
        codetype = undefined;
        if (that.currentFileID == "WorkstatePostCode") codetype = "javascript";
        if (that.currentFilename.search("\\.py") > -1) codetype = "python";
        if (that.currentFilename.search("\\.m") > -1) codetype = "octave";
        ...
        textFromDivToObject();
        if (ev.modified == true) that.toolbar.$save.addClass("notsaved");
        else that.toolbar.$save.removeClass("notsaved");
    ...
    function setInnerLayout() {
        ...
        if (editor) {
            if (editor.cid != undefined) clearTimeout(editor.cid);
            editor.cid = setTimeout(function () {
                editor.refresh();
                editor.cid = undefined;
            }, 100);
        }
    }
    ...
    that.customClose = function () {
        ...
        if (!that.toolbar.$save.hasClass("notsaved")) {
            that.closeUnreferenced();
        } else that.content.modified = true;
    };
...
function KBmpViewer(viewport, master) {
            ...
            alertify.prompt(
                ...
                function (e, val) {
                    if (e) {
                        ...
                        var res = createNifti(
                            { edges: edges, voxSize: [psz[0], psz[1], 1], sizes: [im.size[0], im.size[1], 1] },
                            val[1],
                            "uint16",
                            1,
                            {}
                        ...
                        for (var k = 0; k < im.size[0] * im.size[1]; k++) {
                            res.data[im.size[0] * im.size[1] - k] =
                                im.buf[4 * k] + im.buf[4 * k + 1] + im.buf[4 * k + 2];
                        }
                    }
                },
                ["1,1", name]
    ...
    $img.click(function () {
        if (!that.moved) {
            if (viewport.viewPortID != KViewer.zoomedViewport) KViewer.unZoomViewport();
            viewport.zoomViewPort();
    ...
    $img.on("load", function () {
        ...
        } else icontainer.attachEvent("onmousewheel", MouseWheelHandler);
    ...
    function setInnerLayout(force) {
        ...
        if (that.currentFileID != undefined) {
            ...
            //console.log(widoffs);
            $img.css({
                height: math.round(hei) - 2 + "px",
                width: math.round(wid) - 2 + "px",
                top: math.round(heioffs) + "px",
                left: math.round(widoffs) + "px",
            });
