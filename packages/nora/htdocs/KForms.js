...
function KFormViewer(viewport, master) {
    ...
    toolbar.$save = $("<div class='KViewPort_tool'><i class='fa fa-save fa-1x'></i></div>").click(function () {
    ...
    function setPatientStudy(psid) {
        ...
        while (true) {
            if (psid == false) break;
            var sid;
            if (psid.studies_id != undefined) sid = psid.studies_id;
            else {
            ...
            var finfo = "";
            if (psid.filename != undefined) finfo = " (" + psid.filename + ")";
        ...
        if (!found) {
            ...
            $info
                .css("color", "yellow")
                .html("No unique patient/study set for this form.<br>Hold CTRL and drop a study to select.");
        }
    }
    ...
    function setContent(ev, params) {
        ...
        if (!master.formManager.isinstance) {
            master.formManager.updateListFromServer(function () {
                setContent(ev, params);
            });
            return false;
        }

        if (typeof ev.content == "string") {
            // a loaded form content
            var x = JSONparse_lazy(ev.content);
            ev.content = { name: x.KForm_name, content: x };
            if (params.intent.patientedit | (params.fileID == "patientinfo")) ev.content.name = "patientinfo";
            if (params.intent.studyedit | (params.fileID == "studyinfo")) ev.content.name = "studyinfo";
        }
        ...
        else that.formcontent = ev.content;
        ...
        if (that.currentFileID == undefined) that.currentFileID = "NA";
        ...
        if (params.intent.studyedit | (params.fileID == "studyinfo")) that.formcontent.StudyID = ev.fileinfo.studies_id;
    ...
    function onchange(e) {
        ...
        if (state.viewer.forms_autosave) {
            if (that.autosave_cid != -1) clearTimeout(that.autosave_cid);
    ...
    function update() {
        ...
        if ((that.currentFileinfo.studies_id != undefined) & (that.currentFileinfo.patients_id != undefined))
            setPatientStudy({
                ...
                filename: that.currentFileinfo.Filename,
            });
        else setPatientStudy(patientTableMirror.getCurrentUniquePSID());
        that.setInnerLayout();
    }
    that.update = update;
...
KForm.createForm = function (form0, content0, $targetdiv0, onchange, isPreview) {
    ...
    function createForm_rec(form, content, $targetdiv, count) {
        ...
        if ((form.classdef !== undefined) & ($(document.head).find("#KForm_" + form.name).length == 0)) {
            $("<style id='KForm_" + form.name + "' type='text/css'>" + form.classdef + "</style>").appendTo(
                document.head
            );
        }
        ...
        for (var r = 0; r < form.layout.length; r++) {
            var x = form.layout[r];
            if (x.title == undefined) x.title = x.name;

            var thetitle = x.title;
            var shortcutid = "";
            if (form0 && form0.tag == "READING" && (x.type == "option" || x.type == "check") && count < 99) {
                thetitle =
                    " <span style='color:gray;position:relative;top:-0px;font-size:12px; margin-right:2px; background:none;'>" +
                    count +
                    " </span>" +
                    thetitle;
            ...
            var element = "div";
            if (x.element) element = x.element;
            ...
            if (x.type == "css") {
            ...
            } else if ((x.type == "div") | (x.type == "placeholder")) {
                // placeholder div for personal use
                $c = $(
                    " <" +
                        element +
                        " placeholderid='" +
                ...
                $c = x.val(content);
                if ($c != undefined) $c.addClass("KFormItem");
            } else if (x.type == "form") {
                $c = $("<" + element + "></" + element + ">");
                ...
                var createitm = function (cb) {
                    return (function (x, content, $c) {
                        return function (ev) {
                            if (content[x.name] == undefined) content[x.name] = new Array();
                ...
                $(
                    "<div class='modernbutton KFormItem_formarray_tool'> " +
                        x.createbutton +
                ...
                if (x.altcreatebuttons != undefined) {
                    for (var s = 0; s < x.altcreatebuttons.length; s++) {
                        $(
                            "<div class='modernbutton small KFormItem_formarray_tool'> " +
                                x.altcreatebuttons[s].text +
                                " <i class='fa fa-plus'></i></div>"
                        )
                            .click(createitm(x.altcreatebuttons[s].fun))
                            .appendTo($menu);
                    }
                }
                function appendFormArrayItem(x, content, index, $c, slide) {
                    var $innerc = $("<div class='KFormItem_formarray_subform' style='display:none'></div>").appendTo(
                        $c
                    );
                    if (slide) $innerc.fadeIn(170);
                    else $innerc.show();
                    ...
                    if (x.autoID) {
                        if (content[x.name][index].id == undefined) {
                            ...
                            for (var k = 0; k < content[x.name].length; k++) {
                                if (next_valid <= content[x.name][k].id) next_valid = content[x.name][k].id + 1;
                            }
                            content[x.name][index].id = next_valid;
                        }

                        $(
                            "<div class='KFormItem_formarray_tool KFormItem_formarray_autoid'>#" +
                                content[x.name][index].id +
                                "</div>"
                        ).appendTo($innerc);
                    }
                    // create a delete button
                    if (!x.linkToMarkerset) {
                        $("<div class='KFormItem_formarray_tool' style='right:0px'><i class='fa fa-trash'></i></div>")
                            .appendTo($innerc)
                            .click(function () {
                                var index = $innerc.prevAll().length - 1; // must find the index each time from new!!
                                $innerc.slideUp(110, function () {
                                    $(this).remove();
                                });
                ...
                function dummy_closure(x, content, $c) {
                    ...
                    $menu.append("<label>" + thetitle + "</label>").appendTo($c);
                    var $newbutton = $(
                        "<div class='modernbutton small KFormItem_formarray_tool' style='max-width:150px;'><i class='fa fa-plus'></i> New marker </div>"
                    ).appendTo($menu);

                    var $itemcontainer = $("<div class='' style='position:relative'></div>").appendTo($c);
                    var $itemselector = $("<div class='KFormItem_formarray_selectormenu'></div>").appendTo(
                        $itemcontainer
                    );
                    var $itemtopline = $("<div class='KFormItem_formarray_highligterline'></div>").appendTo(
                        $itemcontainer
                    );
                    var mset;
                    ...
                    function appendFormArrayItem(x, content, index, $c, point) {
                        ...
                        var $selItemColor = $("<div class='KFormItem_formarray_colorbox' style=''></div>").appendTo(
                            $selItem
                        );
                        ...
                        else createForm_rec(x, point.formcontent, $innerc, count);
                    ...
                    if (isPreview) markerProxy.delSet(x.name);
                    ...
                    if (markerProxy.markersets[x.name] == undefined) {
                        ...
                        if (isPreview) state.delSetOnPanelClose = 1;

                        $.extend(true, state, x.state);
                        mset = markerProxy.newSet({
                            name: x.name,
                            uuid: x.name,
                            type: x.markersettype || "circles",
                            showPanel: 1,
                            templates: x.templates,
                            state: state,
                        });

                        if (mset.markerPanel && !isPreview) {
                            // hide some buttons, save functionality only via form.
                    ...
                    creation_status = 1;
                    if (isPreview) {
                        // on preview, directly create an element
                        markerProxy.createMarker(undefined, mset);
                    }
                }
                ...
                var $row = point
                    .createMarkerRepresentation(form.name, { delete: 0, coords: 0 })
                    .addClass("KForm_markerpointrow");
                ...
                if (x.form != undefined) {
                    ...
                    if (0) {
                        //x.formAtPoint) // create the form also at the point in eacht viewport
                ...
                point.callbacks.delete.KForm = function () {
                    if ($c.parent().hasClass("KFormItem_formarray_subform")) $c.parent().remove();
                    else $c.remove();
                };
            } else if (x.type == "json") {
                // make a lazy json
                ...
                function myofffocus(ev, x) {
                    ...
                    try {
                        ...
                        $.notify(
                            "Sorry, the JSON intent json\n" +
                                jsonstring +
                                " \ncould not be parsed. Content was not updated.",
                            "error"
                        );
                    }
                ...
                $c = $(
                    "<" +
                        element +
                        " class='KFormItem'><label class='KFormItem_label' for='" +
                        x.name +
                        "' > " +
                        x.title +
                        ": </label></" +
                ...
                var $textarea = $("<textarea name='" + x.name + "' " + attr + "></textarea>")
                    .appendTo($c)
                    .on(
                        "click",
                        (function (x) {
                ...
                if (typeof x.defaultval === "number") x.data = ["number"];
                if (x.data == undefined) x.data = ["text"];
                var attr = "";

                if (x.attribute != undefined) attr = x.attribute;

                var $input;

                if (x.data[0] == "text") {
                    var style = "text";
                    if (x.style == "password") style = "password";
                    if (x.style == "date") style = "date";
                    var changeevent = "keyup";
                    if (x.changeevent != undefined) changeevent = x.changeevent;

                    $input = $("<input type='" + style + "'  name='" + x.name + "' " + attr + ">")
                        .appendTo($c)
                        .val(content[x.name])
                        .on(
                            changeevent,
                    ...
                    if (x.style == "slider") {
                        $input = $(
                            "<input name='" +
                                ...
                                "' max='" +
                                x.data[2] +
                                "' /><span style='position:relative;top:-5px;padding:4px'>" +
                                content[x.name] +
                                "</span>"
                        )
                            .appendTo($c)
                ...
                if (x.ids === undefined) x.ids = x.choices;

                if (content[x.name] == undefined) content[x.name] = x.ids[0];
                ...
                if (x.style.search("radio") >= 0) {
                    ...
                    var $sel = $("  <div style='display:inline-block'> " + optlist + " </div> ")
                        .appendTo($c)
                        .click(
                            (function (i, x) {
                                return function (e) {
                                    ...
                                    for (var j = 0; j < opts.length; j++)
                                        if (opts[j].checked) content[i] = opts[j].value;
                                    content.modified = true;
                                    content.update(x.name);
                                };
                            })(x.name, x)
                    ...
                    for (var i = 0; i < x.choices.length; i++) {
                        var selected = "";
                        if (x.ids[i] == content[x.name]) selected = "selected";
                        optlist += "<option value='" + x.ids[i] + "' " + selected + ">" + x.choices[i] + "</option>";
                    }
                    var $sel = $("  <select f> " + optlist + " </select>")
                        .appendTo($c)
                        .click(
                            (function (i, x) {
                                return function (e) {
                ...
                $c = $(
                    "<" +
                        element +
                        " class='KFormItem' ssstyle='display:flex;'><label class='KFormItem_label' for='" +
                        x.name +
                        "' > " +
                        thetitle +
                        ": </label></" +
                ...
                for (var t = 0; t < x.ratings.length; t++) $head.append($("<td><span>" + x.ratings[t] + "</span></td"));
                var rows = {};
                for (var t = 0; t < x.items.length; t++) {
                    ...
                    for (var i = 0; i < x.ratings.length; i++) {
                        ...
                        if (x.ratings[i] == content[x.name][item]) selected = "checked";
                ...
                var $check = $("<input type='checkbox' name ='" + x.name + "' id='" + cid + "'>")
                    .appendTo($c)
                    .prop("checked", content[x.name])
                    .on(
                        "change",
                        (function (i, x) {
                            return function (e) {
            ...
            if ($c != undefined) {
                ...
                if (x.name) $c.attr("name", x.name);
                if (shortcutid != "") $c.attr("shortcutid", shortcutid);
    ...
    function setCSS($c, css) {
        ...
        if (css) {
            for (var i = 0; i < css.length; i += 2) $c.css(css[i], css[i + 1]);
        }
    }
    ...
    function attachUpdater(obj, onchange) {
        ...
        if (obj.update != undefined) {
            if (onchange != undefined) obj.update.onchange = onchange;
            return;
        }
        ...
        Update.broadcast = function (field, val, varargin) {
            if (val == undefined)
                // undefinded value might cause error in jquery value=
                val = "";
            for (var k = 0; k < Update.funlist.length; k++)
                if (Update.funlist[k].field == field) Update.funlist[k].fun(val, varargin);

            if (Update.onchange) Update.onchange(obj, field, val);
        };
...
KForm.getFormContentHTML = function (form, obj) {
    if (obj == undefined) obj = { KForm_name: form.name, html: true };

    var $page = $(form.html);
    var inputs = $page.find("input,textarea,select");
    for (var k = 0; k < inputs.length; k++)
        if (obj[$(inputs[k]).attr("id")] == undefined) obj[$(inputs[k]).attr("id")] = $(inputs[k]).val();
...
KForm.findFormItem = function (form, how, search) {
    ...
    function find_rec(layout) {
        for (var k = 0; k < layout.length; k++) {
            if (layout[k][how] == search) out.push(layout[k].name);

            if (layout[k].layout !== undefined) find_rec(layout[k].layout);
            else if (layout[k].form !== undefined) find_rec(layout[k].form.layout);
        }
    }
...
KForm.getFormContent = function (form, obj) {
    if (form.html) return KForm.getFormContentHTML(form, obj);

    if (obj == undefined) obj = { KForm_name: form.name };
    for (var k = 0; k < form.layout.length; k++) {
        if (form.layout[k] != undefined && obj[form.layout[k].name] == undefined) {
            if (form.layout[k].type == "markerpoint") {
                ...
                if (form.layout[k].type == "rating") {
                    ...
                    if (dval != undefined) obj[form.layout[k].name] = dval;
                }
                ...
                if (form.layout[k].defaultvalif != undefined) {
                    for (var z = 0; z < form.layout[k].defaultvalif.length - 1; z += 2) {
                        try {
                            ...
                            if (eval(expr)) obj[form.layout[k].name] = form.layout[k].defaultvalif[z + 1];
                        } catch (err) {
                            console.log(err);
        ...
        if (form.layout[k] != undefined && form.layout[k].type == "form") {
            if (obj[form.layout[k].name] == undefined) obj[form.layout[k].name] = {};
    ...
    for (var k = 0; k < form.layout.length; k++) {
        if (obj[form.layout[k].name] == undefined) {
            ...
            if (form.layout[k].ids) {
                ...
                var ind = Math.floor(Math.random() * (numanswers - 0.0001));
                obj[form.layout[k].name] = form.layout[k].ids[ind];
            } else if (form.layout[k].defaultval != undefined) obj[form.layout[k].name] = form.layout[k].defaultval;
        }
...
KForm.checkEmptyFields = function ($form, highlight) {
    ...
    if (nonFilled.length > 0) return false;
    else return true;
};
...
function KFormManager(master) {
    ...
    var that = new KToolWindow(
        master,
        $("<div class='KView_tool '><i class='fa fa-file-text fa-1x'></i></div>").append(
            $("<ul class='KView_tool_menu'></ul>").append($("<li>Forms</li>"))
        )
    );
    ...
    if (state.viewer.forms_autosave == undefined) state.viewer.forms_autosave = true;

    if (state.viewer.forms_user_specific == undefined) state.viewer.forms_user_specific = true;

    var formman = master.formManager;

    var $menu = $("<ul></ul>");
    var sel = state.viewer.forms_autosave ? "-dot" : "";
    var $autosave = $("<li> <a> Autosave</a>  <i class='fa fa" + sel + "-circle-o'></i></li>")
        .click(function () {
    ...
    var $userspecname = $("<li> <a> User specific naming </a> <i class='fa fa" + sel + "-circle-o'></i></li>")
        .click(function () {
	...
	****************************************************************************************/
    that.getFormByID = function (name) {
        for (var k = 0; k < that.forms.length; k++) {
            ...
            if (that.forms[k].id == name || form.name.toLowerCase() == name.toLowerCase()) {
                // comparison by name or id
    ...
    function setDefaultForms() {
        that.forms = [
            { id: "X01", iswritable: false, content: psidforms[0] },
    ...
    that.updateListFromServer = function (callback) {
        window.setTimeout(function () {
            jsonTable_loadFormattedList("form", function (res) {
                ...
                for (var id in res) that.forms.push(res[id]);
                that.update();
	...
	****************************************************************************************/
    ...
    that.$topRow.append(
        $("<li style='fffffloat:right'><a><i class='fa fa-pencil' ></i> New Form </a></li>").click(function () {
            copyElement(genericForm);
        })
    );
	...
	****************************************************************************************/
    function copyElement(fromwhich) {
        if (fromwhich == undefined) fromwhich = genericForm;
	...
	****************************************************************************************/
    that.update = function () {
        ...
        for (var k = 3; k < forms.length; k++) {
            var dragstuff =
                "draggable='true' data-type='file' data-piz='' data-sid='' data-tag='/FORM/' data-fileID='" +
                forms[k].content.name +
                "' data-subfolder='' data-filename='' data-mime='emptyform'";
            ...
            $row.append(
                $("<td> <i " + addstyle + "class='tablebutton fa fa-fw  fa-wrench'></td>").click(
                    (function (k) {
                        return function (e) {
            ...
            ).appendTooltip("changeFormDesign");
        }
    ...
    that.save = function (formtype, content, fileinfo, onsuccess) {
        if ((formtype == "patientinfo") | (formtype == "studyinfo") | (formtype == "patientstudyinfo")) {
            ...
            //  obj = content.content;
            if ((formtype == "studyinfo") | (formtype == "patientstudyinfo")) obj.sid = fileinfo.studies_id;
            ...
            if (content.content) delete content.content.modified;
            var finfo = { subfolder: "", tag: "/FORM/", subfolder: "forms" };
            if (fileinfo) finfo = $.extend(finfo, fileinfo);
            ...
            var name = formtype;
            if (state.viewer.forms_user_specific_naming) name += "." + userinfo.username;
	...
	*/
    ...
    if (obj instanceof Array) {
        ...
        if ((obj.length > 0) & (obj[0] instanceof Object)) {
            ...
            if (level !== 0) str += newline + indentOld + "]";
        } else {
...
function KFormTester() {
    var templateform = {
        ...
        layout: [
            ...
            { type: "input", name: "myinput", title: "some text", defaultval: "good" },
            { type: "input", data: ["integer", 0, 10], name: "myint", title: "some number", defaultval: "410" },
            {
                type: "input",
                data: ["integer", 0, 10],
                style: "slider",
    ...
    var $p1 = dialog_generic();
    $p1.$frame.show().width(300).css("left", 400);
    var $p2 = dialog_generic();
    $p2.$frame.show().width(300).css("left", 10);

    // first form
    KForm.createForm(templateform, KFormTester.content, $p2.$container, function ($c) {
        console.log("Onchange function: p1----------");
        console.log($c);
    });

    // second form
    KForm.createForm(templateform, KFormTester.content, $p1.$container, function ($c) {
        console.log("Onchange function: p2----------");
        console.log($c);
    });
    ...
    var patientForm = {
        ...
        layout: [
            ...
            { type: "title", val: "Patient information", css: ["font-size", "30px", "display", "inline-block"] },
            {
                type: "title",
                element: "span",
                val: "<p class='createbutton'> </p>",
                css: ["font-size", "30px", "display", "inline-block"],
            },
            ...
            { type: "input", name: "PatientID", title: "Patients ID", defaultval: "", attribute: "readonly" },
            { type: "separator", css: ["height", "5px"] },
            ...
            { type: "input", name: "PatientsBirthDate", title: "Birthdate", defaultval: "" },
            { type: "input", name: "PatientsAge", title: "Age", defaultval: "" },
            { type: "input", name: "PatientsWeight", title: "Weight", defaultval: "" },
            { type: "input", name: "PatientsSize", title: "Size", defaultval: "" },
        ],
    };
    var studyForm = {
        name: "studyinfo",
        layout: [
            ...
            { type: "separator", css: ["height", "5px"] },
            { type: "input", name: "StudyID", title: "Studys ID", defaultval: "" },
            { type: "input", name: "StudyDescription", title: "Description", defaultval: "" },
            { type: "input", name: "StudyInstanceUID", title: "Instance UID", defaultval: "" },
            { type: "input", name: "StudyDate", data: ["integer"], title: "Date", defaultval: "" },
            { type: "input", name: "StudyTime", title: "Time", defaultval: "" },
        ],
    };

    psidform = {
        name: "patientstudyinfo",
        layout: patientForm.layout.concat([{ type: "separator", css: ["height", "5px"] }]).concat(studyForm.layout),
    };
...
function jsonTable_Editor(jsonrow, tabs, options) {
	...
	****************************************************************************************/
    if (jsonrow == undefined) jsonrow = { id: "new", type: "none" };

    if (jsonrow.content == undefined) jsonrow.content = { name: "noname", id: jsonrow.id || "new" };
	...
	****************************************************************************************/
    function render_central_tabs() {
        ...
        var $nametab = $("<div class='' style='padding:15px; font-size:18px; font-weight:bold'></div>").appendTo(
            $tools
        );
        ...
        var $delete = $("<div class='modernbutton small red'><i class='fa fa-trash'></i>delete</div>")
            .appendTo($tools)
            .click(function () {
                deleteObject();
            });
		...
		*****************************************************/
        function renderInnerContent(tabid) {
            ...
            if (tabid == "general" || tabid == "raw") {
                ...
                if (subjson[tabid] == undefined && tabobj.defaultcontent) subjson[tabid] = tabobj.defaultcontent;
            }

            // if a formular is given render as form, otherwise, render as raw jsoneditor
            if (tabobj.form_________) {
                KForm.createForm(tabobj.form, subjson[tabid], $tabDIV, function () {
                    json.modified = true;
                });
                ...
                if (tabobj.evalbutton && tabobj.evalbutton.callback) {
                    var $preview = $(
                        "<div class='modernbutton small green' style='position:absolute;top:0;right:10px'> " +
                            (tabobj.evalbutton.buttontitle || "run code") +
                            "<i class='fa fa-arrow-right'></i></div>"
                    );
                    $preview
                        .click(function () {
                            jsoneditor.JSONparse(true);
                            if (tabobj.evalbutton.callback) {
                                tabobj.evalbutton.callback(tabid, jsoneditor);
                            }
                        })
                        .appendTo($tabDIV);
                }
    ...
    function renderTemplateList() {
        ...
        for (var id in templates) {
            ...
            $("<span class='flexright'></span>")
                .appendTo($e)
                .append(
                    $("<i class='fa fa-arrow-right'></i>").click(
                        (function (id) {
                            return function (ev) {
                                template_pasteCode(id);
                                ev.stopPropagation();
                            };
                        })(id)
                    )
    ...
    function deleteObject(id) {
        ...
        function whendone(result) {
            if (result.msg == "") {
                ...
                for (var k = 0; k < l.length; k++) if (l[k].id == jsonrow.id) l.splice(k, 1);
                KViewer.formManager.update();
            } else {
	...
	****************************************************************************************/
    function template_copyCode(id) {
        if (jsoneditor == undefined || jsoneditor.$textarea == undefined) return;
        ...
        var $temp = $("<textarea style='position:absolute; display:block;'>" + str + "</textarea>")
            .appendTo($body)
            .select();
        var successful = document.execCommand("copy");
        $temp.remove();
        var $notification = $(
            "<div style='position:absolute;color:green'>code copied to clipboard. Use ctrl+v to paste.</div>"
        ).appendTo(jsoneditor.$log.empty());
        window.setTimeout(function () {
            $notification.fadeOut(900, function () {
                $notification.remove();
            });
        }, 800);
    ...
    that.ontoggle = function () {
        if (json.modified) {
            alertify.confirm(
                "There might be unsaved changes in your form. <br> If you want to save them press 'cancel' and save manually.",
                function (a) {
                    if (!a) return false;
                    else that.$frame.remove();
                }
            );
        } else {
            that.$frame.remove();
