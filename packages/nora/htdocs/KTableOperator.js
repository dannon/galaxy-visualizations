...
function attachTableOperator($div, initstate, sortable) {
    var $table;
    if ($div.is("table")) $table = $div;
    else $table = $div.find("table");
    ...
    if (sortable) {
        ...
        for (var k = 0; k < $cols.length; k++) {
            if (sortable.length > 0) {
                if (sortable.find((x) => x == k) != undefined) continue;
            }
                ...
                (function (idx) {
                    return function (e) {
                        if (!e.shiftKey && !e.ctrlKey) {
                            ...
                            var donotsort =
                                $(e.target).attr("preventsortable") || $(e.target).parent().attr("preventsortable"); // for check or delete or similar cells;
                            if (!(left | right | donotsort | (e.button == 2))) sortbycolumn($cols[idx], idx);
                        }
                    };
                })(k)
            );
    ...
    function sortbycolumn(target, idx) {
        ...
        for (var k = 0; k < rows.length; k++) {
            ...
            if ($(rows[k]).children().length > 0) var text = $(rows[k]).children().eq(0).text().trim();
            else var text = $(rows[k]).text().trim();
        ...
        vals = vals.sort(function (a, b) {
            ...
            if (typeof a.val != typeof b.val || a.val == b.val) return -1;
            else return a.val > b.val ? sorted : -sorted;
        });

        for (var k = 0; k < vals.length; k++) $body.append($(vals[k].div));
    }
        ...
        for (var k = 0; k < sfields.length; k++) {
            $(sfields[k]).on("keyup", function (e) {
                ...
                for (var j = 0; j < rows.length; j++) {
                    ...
                    if (found) $(rows[j]).parent().show();
                    else $(rows[j]).parent().hide();
                }
            });
        }
    }
    ...
    function initialWidthLayout() {
        ...
        for (var k = 0; k < $cols.length; k++) {
            ...
            if ($col.attr("fixedwidth") != undefined || $col.attr("initwidth") != undefined) {
                if ($col.attr("fixedwidth") != undefined) w = parseInt($col.attr("fixedwidth")) + 2;
                else w = parseInt($col.attr("initwidth")) + 2;

                if ($col.find("i").length > 0) $col.css("text-overflow", "clip");

                $col.width(w);
                widths.push(NaN);
                if (initstate != undefined && initstate.viscol != undefined) viscol_acc(initstate.viscol[k]);
                else viscol_acc(true);
                ...
                if (initstate.viscol != undefined) viscol_acc(initstate.viscol[k]);
            } else {
                $col.width(w);
                twid += w;
                widths.push(NaN);
    ...
    function isOnSeparatorLeft(e) {
        ...
        if (e.type == "mousedown" && $(e.target).hasClass("fa")) return false;

        var target = getTD(e.target);
        var off = e.pageX - $(target).offset().left;
        if (math.abs(off) < 4) return true;
        else return false;
    }
    ...
    function colShowHide(e) {
        ...
        var menu = KContextMenu(
            function (ev) {
                ...
                for (var k = 0; k < $cols.length; k++) {
                    var $icons = $("<span ></span>");
                    if ($($cols[k]).hasClass("nottoggable")) continue;

                    $(" <i  class='fa " + (viscol[k] ? "fa-check-square-o" : "fa-square-o") + " fa-1x'></i>").appendTo(
                        $icons
                    );
                    var colname = $($cols[k]).attr("colkey");
                    if (colname != undefined)
                        if (
                            colname.substring(0, 5) == "META_" &&
                            state.metaindices &&
                            state.metaindices[colname.substring(5)] != undefined
                        ) {
                            $(
                                "<i onchoice='refresh_" +
                                    $($cols[k]).attr("colkey") +
                                    "' class='fa fa-refresh fa-1x'>   </i>"
                            ).appendTo($icons);
                            $(
                                "<i onchoice='del_" + $($cols[k]).attr("colkey") + "' class='fa fa-trash fa-1x'>   </i>"
                            ).appendTo($icons);
                        }
                    if ($($cols[k]).text().trim() != "")
                ...
                if (str) {
                    if (str.substring(0, 4) == "del_") {
                        ...
                        var pbar = KProgressBar(
                            "updateing meta index " + str.substring(13),
                            "fa-submit",
                            function (x) {
                                xhr.abortPHPprocess();
                            },
    ...
    function attachResizer($part) {
        $part.children().on("pointermove", function (e) {
            if (isOnSeparatorLeft(e) | isOnSeparatorRight(e)) {
                var t = getTarget(e);
                if (t.colidx == -1) return;
                var $target = $($head.find("td")[t.colidx]);
                if ($target.attr("fixedwidth") != undefined) return;
        ...
        function getTarget(e) {
            ...
            var $targetcol = $(td);
            if (left) $targetcol = $(td).prevAll(":visible:first");
