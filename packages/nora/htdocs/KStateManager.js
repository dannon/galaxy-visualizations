...
function StateManager() {
    ...
    that.applyState = function (state_new) {
        if (state_new === undefined) return;
        state = state_new;
        ViewerSettings = state.viewer;

        setPatientTableWidth();
        setPatientTableLayout();
        $(".leveltab").removeClass("current");
        $("#modeSelector_" + state.viewer.selectionMode[1]).addClass("current");
    ...
    that.saveCurrentState = function (whendone, force) {
        ...
        if (userinfo.username == guestuser && force == undefined) {
            // | projectInfo.rights.readonly == "on" | projectInfo.rights.readonly === false)
            if (whendone) whendone();
            return;
        }
        ...
        // Save state itself
        if (commandDialog != undefined && commandDialog.getState) state.batchtool = commandDialog.getState();
...
*/
        if (
            markerProxy != undefined &&
            markerProxy.markersetIDs.length > 0 &&
            markerProxy.markersets[markerProxy.markersetIDs[0]] != undefined
        )
        ...
        if (typeof electron != "undefined" && electron) {
            dialog.showSaveDialog(
                { title: "save settings", properties: [], defaultPath: "./noraview.settings.json" },
                function (savename) {
            ...
            var row = {
                ...
                MDB_uniquekey:
                    ' type like "state" AND project like "' +
                    currentModule +
                    '" AND owner like "' +
                    userinfo.username +
                    '"',
...
var form_project_user = {
    ...
    layout: [
        { type: "title", val: "State settings" },
        {
            ...
            type: "option",
            style: "radio vert",
            title: "What happens on fresh startup/login",
            choices: ["take most recent state", "choose favourite preset", "use a standard preset, if exists"],
            ids: ["rememberState", "rememberPreset", "tryUsingStandard"],
            defaultval: "rememberState",
        },
        {
            name: "updateInterval",
            type: "input",
            defaultval: 60,
            title: "Update interval of subject table(in seconds)",
        },
        {
            name: "updateInterval_gridstat",
            type: "input",
            defaultval: 30,
            title: "Update interval of gridengine statistics (in seconds)",
        },
    ],
};
...
var presetForm_autoload = {
    ...
    layout: [
        ...
        {
            name: "autoloaderLevel",
            type: "option",
            style: "",
            choices: ["patient or study", "patient", "study"],
            ids: [0, 1, 2],
            defaultval: 0,
        },
        ...
        { type: "separator" },
        {
            ...
            altcreatebuttons: [
                { text: "Add annotation loader", fun: addanno },
                { text: "Add form loader", fun: addform },
            ],
            layout: [
                {
                    name: "pattern",
                    type: "textarea",
                    defaultval: "FFilename:",
                    class: "autoloaderitem",
                    ondrop: function (e, content) {
                        content.pattern = tempObjectInfo[0].filename;
                        content.update("pattern");
                        content.intent = {};
                        if (tempObjectInfo[0].mime == "form") content.intent = { defaultform: 1 };
                ...
                },
                {
                    ...
                    type: "input",
                    defaultval: "0",
                    class: "autoloaderitem viewportID",
                    element: "span",
                },
...
function variableOptions(x) {
    ...
    var $autoroi = $(
        '<span class="Kautocreateroi"><input type="checkbox" name="autocreateroi" ' +
            (x.intent.autocreateroi != undefined ? "checked" : "") +
            '> <label for="autocreateroi"> autocreate ROI </label></span>'
    );
    $autoroi.on("change", function (e) {
        if ($(e.target).is(":checked")) x.intent.autocreateroi = "0";
        else delete x.intent.autocreateroi;
        x.update("intent");
    });

    var $overlay = $(
        '<span class="notroi"><input type="checkbox" name="overlay" ' +
            (x.intent.overlay == 1 ? "checked" : "") +
            '> <label for="overlay"> overlay </label></span>'
    );
    $overlay.on("change", function (e) {
        if ($(e.target).is(":checked")) x.intent.overlay = 1;
        else delete x.intent.overlay;
        x.update("intent");
    });
    ...
    for (var k = 0; k < colormap.names.length; k++) {
        ...
        else $colmap.append($("<option id='" + k + "' > " + colormap.names[k] + " </option>"));
    }
    ...
    if (x.intent.color != undefined) {
        if (Array.isArray(x.intent.color)) {
            if (typeof x.intent.color[0] == "string") defcol = { color: x.intent.color[0].split(",") };
            else defcol = { color: x.intent.color[0] };
        } else {
            if (typeof x.intent.color == "string") defcol = { color: x.intent.color.split(",") };
            else defcol = { color: x.intent.color };
        }
    }
    ...
    var $colselector = KColorSelector(
        ...
        function (c, i) {
            x.intent.color = i;
            if (x.intent.isosurf) x.intent.isosurf.color = i;
            x.update("intent");
        },
        defcol
    );
...
function intentchange(intent, target, $parent) {
    if (target != undefined) $parent = $(target).parent().parent();
    ...
    if ($x.length > 0) $x[0].checked = intent.overlay > 0;

    $x = $parent.find("select[name=colmaps]");
    if ($x.length > 0) $x[0].selectedIndex = intent.cmap;
}
...
var presetForm_general = {
    ...
    layout: [
        ...
        //,{name:"rights"					, type: 'input',     defaultval:"rw"  }
        {
            ...
            type: "option",
            style: "",
            choices: ["private", "readable", "project standard", "project standard, writable"],
            ids: ["--", "r-", "s-", "sw"],
            defaultval: "--",
        },
        { type: "separator" },
...
var presetForm_viewer_permorder = {
    name: "permOrder",
    type: "option",
    style: "",
    choices: [
    ...
    title: "Viewing mode (swaps/flips)",
    defaultval: "human",
};
...
var presetForm_viewer = {
    ...
    layout: [
        ...
        {
            name: "defaultFOVwidth_mm",
            type: "input",
            defaultval: "",
            title: "Default size of field of view (FOV) in mm, if empty, FOV is automatically computed",
        },
        ...
        { name: "loadBitmapAsNifti", type: "check", defaultval: false, title: "Load bitmap as nifti" },
        {
            ...
            type: "check",
            defaultval: false,
            title: "Prefer quaternions during nifti load (qform)",
        },
        {
            name: "spacedef",
            type: "option",
            style: "",
            title: "Apply space definition on image load",
            choices: [
                "No",
...

        {
            name: "worldVoxelsize",
            type: "input",
            defaultval: "[1.5,1.5,1.5]",
            title: "Voxelsize of worldcoordinates (mm)",
        },
        { name: "defaultsizemarker", type: "input", defaultval: 5, title: "Defaultsize of Marker (mm)" },
        { type: "separator" },
        {
            name: "viewing_timeout",
            type: "input",
            defaultval: 100,
            title: "Drawing timeout in ms (GUI/GFX responsiveness)",
        },
        ...
        { name: "fiberAlpha", type: "check", defaultval: false, title: "Transparency on Fibers" },
        {
            name: "picto3D",
            type: "option",
            title: "Pose pictogram",
            style: "radio vert",
            choices: ["none", "Male Body", "Girl", "Minion", "Head", "Monkey"],
            ids: [-1, 0, 1, 2, 3, 4],
            defaultval: 1,
        },
        { type: "separator" },
        ...
        { type: "separator" },
        { name: "globalCoordinates", type: "check", defaultval: true, title: "Global coordinates" },
        { name: "roiTransparency", type: "input", defaultval: 0.5, title: "Transparency of ROIs" },
        ...
        {
            name: "forms_user_specific_naming",
            type: "check",
            defaultval: true,
            title: "User Specific Naming for Form saving",
        },
    ],
};
...
var presetForm_ptable = {
    ...
    layout: [
...

        {
            ...
            type: "option",
            style: "radio vert",
            choices: ["Expand nothing", "Expand first", "Expand everything"],
        ...
        },
        {
            ...
            type: "option",
            style: "radio vert",
            choices: ["Working/Patient", "Working/Study", "Patient", "Study"],
            ids: ["wp", "ws", "mp", "ms"],
            defaultval: "ms",
        },
        ...
        { type: "title", val: "CSV table export" },
        {
            ...
            style: "radio vert",
            choices: [
                "wide, header full (single line)",
                "wide, header tree (multi line)",
                "wide, header sparse (multi line)",
                "long",
            ],
	...
	****************************************************************************************/
    that.selectDialog = function selectDialog(activeID, subID) {
        if ((activeID == "projectManagement") | (activeID == "presets")) {
            ...
            MList.updateOrSelectByID(MList.activeID);
            if (subID != undefined) projectSettingsDialog.updateOrSelectByID(subID);
        }
    };
...
function PresetManager() {
	...
	****************************************************************************************/
    ...
    var forms_preset = {
        general: [presetForm_general, "content"],
    ...
    var forms_state = {
        general: [form_project_user, "project_user"],
        ptable: [presetForm_ptable, "content"],
        viewer: [presetForm_viewer, "content"],
        autoloader: [presetForm_autoload, "content"],
    };
    ...
    that.setPresetList = function (list) {
        presetList = list;
    };
    that.getPresetList = function () {
        return presetList;
    };
    that.getActivePreset = function () {
        return activePreset;
    };
	...
	****************************************************************************************/
    ...
    that.renderPresetDialog = function () {
        ...
        if (electron) {
            $("<span style='width:80px;' class='modernbutton'> save </span>")
                .appendTo($rightDIV)
                .click(function () {
                    stateManager.saveCurrentState(function () {
                        altertify.success("saved");
                    }, true);
                });
        ...
        if (electron) $tDIV.hide();

        var $tools = $("<div class='modernbuttongroup'></div>").appendTo($tDIV);
        var $apply = $(
            "<div class='modernbutton small green'><i class='fa fa-arrow-left'></i>Apply preset as state</div>"
        )
            .appendTo($tools)
            .click(function () {
                that.applyPreset(that.PList.activeID);
            });
        var $asStandard = $(
            "<div class='modernbutton small yellow'><i class='fa fa-star'></i>set preset as startup default</div>"
        )
            .appendTo($tools)
            .click(function () {
                setStartupPreset(activePreset.id);
            });
        var $delete = $("<div class='modernbutton small red'><i class='fa fa-trash'></i>delete preset</div>")
            .appendTo($tools)
            .click(deletePreset);
        ...
        if (!electron) PList.$ul.appendTo($listDIV);
        else $listDIV.hide();
        ...
        if (!electron) TList.append("ptable", "table", renderPresetSubForm);
        ...
        function renderPresetList() {
            ...
            $("<div class='modernbutton small red' ><i class='fa fa-copy'></i>Paste state from text</div>")
                .click(function () {
                    alertify.prompt("Please insert JSON String from your clipboard and press enter", function (a, b) {
                        if (a) {
                            if (!that.pasteit(b)) alertify.alert("Nora not able to understand your clipboard contents");
                        }
                    });
                })
                .appendTo(PList.$ul);
            ...
            else PList.updateOrSelectByID(); // select first element

            if (that.TList && that.TList.activeID != undefined) TList[that.TList.activeID].trigger("click");
            else {
                if (electron) TList.updateOrSelectByID("viewer");
                // select autoloader tab
                else TList.updateOrSelectByID("autoloader"); // select autoloader tab
            }
        ...
        function selectPreset(arg) {
            if (typeof arg == "object") activePreset = arg;
            else activePreset = presetList[arg];
        ...
        function renderJSONCode() {
            var jsoneditor = new KJSONEditor(activePreset, "content", {
                parseonblur: 1,
                editable: activePreset.iswritable,
                copybutton: 1,
            });
            var $formDIV = $("<div style='position:relative;width:100%;height:95%'></div>").append(
        ...
        function renderPresetSubForm(id) {
            ...
            function onchange(obj, field, val) {
                if (field === "name") {
                    // set the title of this preset in left list
                ...
                if (field == "canvasborder") {
                    if (val) $(".KViewPort_canvas").addClass("KViewPort_canvas_border");
                    else $(".KViewPort_canvas").removeClass("KViewPort_canvas_border");
                }
            }
            ...
            else KForm.createForm(forms_preset[id][0], activePreset[forms_preset[id][1]], $formDIV, onchange);

            if (id == "autoloader") {
                var $snapshot = $(
                    "<div class='modernbutton small' style='position:absolute;right:0px;top:0px;width:210px'><i class='fa fa-camera'></i>take snapshot</div>"
                )
                    .appendTo($formDIV)
                    .click(that.takeautosnapshot);
            }
		...
		****************************************************************************************/
        that.overwritePresetWithState = function (id) {
            if (presetList[id] == undefined) return false;
            if (!presetList[id].iswritable) alert("Preset is not writable");
		...
		****************************************************************************************/
        function copyElement(preset) {
            ...
            for (var k = 0; k < 50; k++) {
                ...
                var found = false;
                for (var id in presetList) {
                    if (presetList[id].content.name == newname) found = true;
                }
                if (!found) break;
            }
	...
	****************************************************************************************/
    that.createpresetListFromAjax = function (ajaxList_in) {
        ...
        if (ajaxList_in == null) return false;

        var ajaxList = [];
        if (!$.isArray(ajaxList_in)) {
            for (var k in ajaxList_in) ajaxList.push(ajaxList_in[k].content);
        } else {
        ...
        for (var k = 0; k < ajaxList.length; k++) {
            ...
            presetList[tt.id] = {
                ...
                iswritable:
                    tt.owner == undefined ||
                    tt.owner == userinfo.username ||
                    (tt.rights !== undefined && tt.rights.substr(1, 1) == "w"),
            };
        }
    };
	...
	****************************************************************************************/
    that.applyPreset = function (id) {
        if (id !== undefined) var preset = presetList[id];
        else var preset = activePreset;
	...
	*/
        ...
        if (preset == undefined) {
            alertify.alert("No valid pjsonTable_savereset selected. Aborting");
            return;
        }
        ...
        if (typeof KViewer !== "undefined") KViewer.applyState();

        if (toolstate) KToolWindow.reestablishToolState(toolstate);
    };
	...
	****************************************************************************************/
    that.applyPresetViewerOnly = function (id) {
        ...
        if (typeof KViewer !== "undefined") KViewer.applyNewViewportLayout();
    };
	...
	****************************************************************************************/
    ...
    that.saveAllPresets = function savePresets() {
        ...
        if (rows.length > 0) jsonTable_save(rows);
    };
    ...
    that.getRowsForSaving = function () {
        ...
        for (var id in presetList) {
            ...
            if ((id !== "0") & preset.iswritable) {
                // do not save the state nor non-writable settings!
	...
	****************************************************************************************/
    function viewportContentToAutoloaders(presetID, notsmart) {
        ...
        else var autoloaders = activePreset.content.autoloaders;

        // clear the loaders first
        //autoloaders.length = 0;
        var oldloaders = autoloaders.splice(0, autoloaders.length);
        var idlist = {};
        for (var k = 0; k < oldloaders.length; k++) idlist[oldloaders[k].id] = k;
        ...
        if (s.viewports.length == 0) {
            ...
            function extendtoArray(dest, src, field) {
                ...
                if (Array.isArray(dest[field])) dest[field].push(src[field]);
                else dest[field] = [dest[field], src[field]];
            }

            var params = {};
            for (var k = 0; k < items.length; k++) {
                if (items[k].intent.gl) items[k].intent.slicing = "gl";
            ...
            for (var k in params) {
                if (params[k]) {
                    ...
                    if (idlist[id] != undefined) {
                        if (
                            oldloaders[idlist[id]].pattern != undefined &&
                            (oldloaders[idlist[id]].pattern.trim() != "") & (notsmart != 2)
                        )
                            loader.pattern = oldloaders[idlist[id]].pattern;
                        else loader.pattern = guessFilePattern(notsmart);
                    } else {
                    ...
                    function guessFilePattern(notsmart) {
                        ...
                        if (fobj == undefined) {
                            if ((params[k].URLType == "form") | (params[k].URLType == "formNA")) {
                                ...
                                else return "FFilename:" + params[k].fileID + ".form.json" + "\nFSubFolder:forms";
                            }
                            return "";
                        }
                        ...
                        if (filename == undefined) filename = fobj.filename + ".nii.gz";

                        if (fobj.fileID.substring(0, 3) == "ROI") {
                            filename = filename + ".nii.gz";
                            if (finfo.Filename != undefined) finfo.Filename = filename;
                        }
                        ...
                        if (
                            (finfo.SubFolder == "forms") |
                            (finfo.Tag != undefined && finfo.Tag.search("/FORM/") > -1)
                        ) {
                        ...
                        if ((subf != "") & (subf != undefined)) pattern += "\nFSubFolder:" + subf.substring(0, 5) + "*";
                        ...
                        if (notsmart) {
                            if (finfo.Filename != undefined) {
                                ...
                                else pattern = finfo.Filename;
                            } else {
                                pattern = "FFilename:" + filename;
                                if (finfo.SubFolder != undefined) pattern += "\nFSubFolder:" + finfo.SubFolder;
                            }
                        }
                    ...
                    for (var j = 0; j < intent_keys.length; j++) {
                        if (params[k].intent[intent_keys[j]] == undefined) delete params[k].intent[intent_keys[j]];
                    }

                    if (params[k].mosaic) params[k].intent.mosaic = params[k].mosaic;
