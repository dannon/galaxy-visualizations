...
function ToolProxy(toolvar, toolObj, toolname) {
    ...
    function buildTool() {
        if (typeof toolObj == "string") toolObj = eval(toolObj);

        var tool = new toolObj(KViewer, toolname);
        KViewer[toolvar] = tool;
        ktoolslist[id] = tool;
        tool.ktoolsindex = id;
        if (hasProxy) KToolWindow.attachToollist(tool);
    ...
    var handler = {
        get: function (target, name, rec) {
            if (name == "isproxy") return true;
                ...
                var tobj = toolObj;
                if (typeof tobj == "string") tobj = eval(tobj);
            ...
            } else if (name == "name") return toolname;
            else if (name == "isinstance" || name == "enabled") {
                if (KViewer[toolvar].isproxy) return false;
                else return KViewer[toolvar][name];
            } else {
...
function attachNameDivHandler(fobj, $captiondiv, cb) {
    if (fobj.namedivs == undefined) fobj.namedivs = [];
    ...
    $captiondiv
        ...
        .keyup(function (ev) {
            ...
            if ((subf != "") | subf_given) fobj.fileinfo.SubFolder = subf;
            fobj.fileinfo.Filename = name;
            if (fobj.namedivs != undefined) {
                fobj.namedivs = fobj.namedivs.filter(function (x) {
                    return document.contains(x[0]);
                });
                for (var i = 0; i < fobj.namedivs.length; i++) {
                    if ($captiondiv != fobj.namedivs[i]) fobj.namedivs[i].text(fobj.filename);
                }
            }
            cb();
        });
...
function resizeTriangle(afterResize, onresize, $frame_) {
    var $resizer = $("<div class='dialog_generic_resizeTriangle'></div>").mousedown(function () {
        if ($frame_ != undefined) $frame = $frame_;
        else $frame = $resizer.parent();
...
function KToolWindow(master, $toggle) {
    ...
    that.onresize = function (ev) {
        if (that.tstate.target != -2) return;
        ...
        var h_upper;
        if (th_upper != undefined) h_upper = th_upper.$container.height();

        $(document.body).on("pointermove", function (ev) {
            //if (s-ev.clientY < maxdelta &
            if (h + s - ev.clientY > 50) {
                if ((th_lower != undefined) | (userinfo.username != guestuser)) th.resize(h + s - ev.clientY);
                if (th_upper != undefined) th_upper.resize(h_upper - s + ev.clientY);
    ...
    var $resizer = $("<div class='annotion_tool_resizer'><div><div></div></div></div>")
        .appendTo($container)
        .on("pointerdown", that.onresize);

    var $topDiv = $("<div>").appendTo($container);

    var $newdragger = $(
        "<div class='KToolsdragger' draggable=true><i class='fa fa-hand-paper-o' > </i></div>"
    ).appendTo($topDiv);
    $newdragger[0].ondragstart = function (e) {
        if (that.name == "Workspace" && userinfo.username == guestuser && guestuser != "") return;
        dragstart(e);
        setTimeout(function () {
    ...
    that.$topRow = $topRow;
    $topRow.append(
        $("<li  style='float:right'> <a><i class='fa fa-close close_button' ></i></a></li>")
            .appendTooltip("closetool")
            .click(function () {
                that.toggle(false);
            })
    );

    var $helper_button = $("<li  style='float:right'> <a><i class='fa fa-question' ></i></a></li>").appendTooltip(
        "help"
    );
    ...
    function dragstart(ev) {
        ...
        setTimeout(function (e) {
            var $undocker = $("<div class='dropindicator_general_vert body_dropindicator' style=''></div>")
                .hide()
                .appendTo($(document.body))
                .fadeIn(150);
            setPixelPosition($undocker, [1, 1, 400, 100], 0);
            var $inner = $(
                "<div style='padding:9px; background:rgba(20, 139, 0, 0.9)'>Drop here to undock tool</div>"
            ).appendTo($undocker);
            $inner.on("dragenter dragover", function (ev) {
                $(this).css("background", "rgba(20, 139, 0, 0.9)");
                ev.preventDefault();
                ev.stopPropagation();
                return false;
            });
    ...
    var $resizeTriangle = resizeTriangle(saveSizesUndocked, function (x) {
        that.resize(x);
    })
        .hide()
        .appendTo($container);
    ...
    function toggledockedstate(forcewhichstate) {
        if (forcewhichstate != undefined) {
            ...
            if (that.tstate.target == -1) {
                // was undocked, so dock into lasttarget
                that.show(that.tstate.lasttarget);
            } // was docked, so undock
            else {
    ...
    function setundocked() {
        ...
        $container
            ...
            .width(that.tstate.udsize[2])
            .height(that.tstate.udsize[3]);
        //setPatientTableLayout();
    ...
    that.toggle = function (forcestate) {
        if (forcestate != undefined) that.enabled = forcestate;
        else {
            if (that.tstate.target >= 0 && that.enabled) {
                ...
                } else that.enabled = false;
            } else that.enabled = !that.enabled;
        }
    ...
    that.show = function (target) {
        ...
        if (target == undefined) {
            ...
            } else target = that.tstate.target;
        }

        // translate vp to vpID
        if (target.viewPortID != undefined) target = target.viewPortID;

        // ******************* check where this item was last

        if (that.tstate.target >= 0 && that.enabled != 0) KViewer.viewports[that.tstate.target].setCurrentViewer();
        ...
        //***********************  ptable at end ******************************
            ...
            var above;
            if (userinfo.username == guestuser) above = childs[childs.length - 1];
            else above = childs[0];
            ...
            if (KViewer.zoomedViewport != -1) KViewer.setSizeLeftViewportCol();
        }
        ...
        //***********************  viewport ******************************
            ...
            if (currview != undefined && currview.close != undefined) vp.getCurrentViewer().close();
    ...
    that.$container.on("drop", function (ev) {
        ...
        if ((that.name == "Files") & (userinfo.username == guestuser)) {
            ...
            if (that == source) {
                ...
                // drop into the ptable: insert new one before
                KToolWindow.dragTool.show(that.$container);
            } else if (that.tstate.target >= -1) {
                // undocked or viewport: full replacement
                that.hide();
...
KToolWindow.findToolbyContainer = function ($cont) {
    for (var k = 0; k < ktoolslist.length; k++) {
        if (
            ktoolslist[k].isinstance &&
            ktoolslist[k].$container != undefined &&
            ktoolslist[k].$container[0] == $cont[0]
        )
            return ktoolslist[k];
    }
    return undefined;
...
KToolWindow.findToolInLowerContainer = function () {
    ...
    for (var k = 0; k < tools.length; k++) {
        ...
        if (t != undefined) incontainer.push(t);
    }
    return incontainer;
};
...
KToolWindow.attachToolSelectors = function ($container) {
    ...
    for (var k = userinfo.username == guestuser && !KViewer.standalone ? 1 : 0; k < ktoolslist.length; k++) {
        if (!hasProxy) KToolWindow.attachToollist(ktoolslist[k]);
...
KToolWindow.attachToollist = function (that) {
    for (
        var k = 0;
        k < ktoolslist.length;
        k++ //    for (var k = (userinfo.username == guestuser  && !KViewer.standalone) ? 1 : 0; k < ktoolslist.length; k++)
    ) {
        var onclick = (function (k) {
            return function () {
                ...
                if (that.tstate.target == -1) {
                    ...
                    else ktoolslist[k].$container.insertAfter($anchor);
        ...
        $("<li><a>" + ktoolslist[k].name + "</a></li>")
            .click(onclick)
            .appendTo(that.$toolselector);

        var toolicons = KToolWindow.toolicons;
        var thei = "";
        if (toolicons[ktoolslist[k].name]) thei = "<i class='fa " + toolicons[ktoolslist[k].name] + "'></i>";
        var enabled = "";
        if (ktoolslist[k] === that) enabled = "KView_tool_enabled";
        $(
            "<div class='KView_tool " +
                enabled +
                " KView_tool_small'>" +
                thei +
                " <ul class='KView_tool_menu'> <li> " +
                ktoolslist[k].name +
                " </li> </ul></div>"
        )
            .click(onclick)
            .appendTo(that.$leftToolistDiv);
    }
};
...
KToolWindow.reestablishToolState = function (toolstate) {
    ...
    for (var k = 0; k < toolstate.length; k++) {
        for (var j = userinfo.username != guestuser || electron ? 0 : 1; j < ktoolslist.length; j++) {
            var tool = undefined;
            if (ktoolslist[j].name == toolstate[k].tool) break;
        }
...
function KTool_enabled() {
    for (var k = 0; k < ktoolslist.length; k++) {
        if (ktoolslist[k].enabled) return k;
    }
    return -1;
}
