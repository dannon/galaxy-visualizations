...
function solvePoissonMC(elecGeometry, params, nii, current, conductivity) {
    ...
    var nx = [n[0], -n[2], n[1]];
    var ny = [n[1] * nx[2] - n[2] * nx[1], n[2] * nx[0] - n[0] * nx[2], n[0] * nx[1] - n[1] * nx[0]];
    ...
    function getDisplacementAniso(C, alpha) {
        ...
        return [C[0] * x + C[3] * y + C[6] * z, C[1] * x + C[4] * y + C[7] * z, C[2] * x + C[5] * y + C[8] * z];
    }
    ...
    function getStartPoint() {
        if (elecGeometry.multipolar.length > 0) {
            ...
            return {
                p: [
                    tip[0] + r * n[0] + x * nx[0] + y * ny[0],
                    tip[1] + r * n[1] + x * nx[1] + y * ny[1],
                    tip[2] + r * n[2] + x * nx[2] + y * ny[2],
                ],
                s: m[k].sign,
            };
        } else {
    ...
    function getStartPointSphere() {
        ...
        return [tip[0] + r * n[0], tip[1] + r * n[1], tip[2] + r * n[2]];
    }
    ...
    function isNeumann(p) {
        ...
        if (proj > 0) {
            ...
            if (dif2 - proj * proj < r2) return true;
        }
        return false;
    }
    ...
    function probround(x) {
        var xi = Math.floor(x);
        if (Math.random() < x - xi) return xi + 1;
        else return xi;
    }
    ...
    function mult4(M, e) {
        return [
            M[0][0] * e[0] + M[0][1] * e[1] + M[0][2] * e[2] + M[0][3] * e[3],
    ...
    var current_sigma;
    var default_sigma = 1;
    var drawDisplacement = function (current_sigma, alpha) {
        return getDisplacement(current_sigma * alpha);
    };
    var doProp = function () {
        return 0;
    };
    ...
    if (params.analytical) {
        if (elecGeometry.multipolar.length > 0) {
            ...
            for (var k = 0; k < m.length; k++) {
                ...
                slist.push([tip[0] + r * n[0], tip[1] + r * n[1], tip[2] + r * n[2], m[k].current * m[k].sign]);
            }
        } else {
            var r = min + (max - min) * 0.5;
            var slist = [[tip[0] + r * n[0], tip[1] + r * n[1], tip[2] + r * n[2], 1]];
        }
    ...
    if (conductivity) {
        if (conductivity.sizes[3] == 1) {
            doProp = function () {
                ...
                if (isNaN(new_sigma) | (new_sigma == 0)) return -1; // break;
                ...
                if (current_sigma != new_sigma) {
                    ...
                    if (R < 1) {
                        if (R < Math.random()) return 1; // continue;
                    }
                }
            ...
            doProp = function () {
                ...
                var e1 = trilinInterp3_signcorrected(
                    conductivity,
                    proposal[0],
                    proposal[1],
                    proposal[2],
                ...
                var e2 = trilinInterp3_signcorrected(
                    conductivity,
                    proposal[0],
                    proposal[1],
                    proposal[2],
                ...
                var e3 = trilinInterp3_signcorrected(
                    conductivity,
                    proposal[0],
                    proposal[1],
                    proposal[2],
                ...
                new_sigma[9] =
                    1 / (new_sigma[0] * new_sigma[0] + new_sigma[1] * new_sigma[1] + new_sigma[2] * new_sigma[2]);
                new_sigma[10] =
                    1 / (new_sigma[3] * new_sigma[3] + new_sigma[4] * new_sigma[4] + new_sigma[5] * new_sigma[5]);
                new_sigma[11] =
                    1 / (new_sigma[6] * new_sigma[6] + new_sigma[7] * new_sigma[7] + new_sigma[8] * new_sigma[8]);
                new_sigma[12] = Math.abs(det3(new_sigma));

                var q = [
                    (new_sigma[0] * dx[0] + new_sigma[1] * dx[1] + new_sigma[2] * dx[2]) * new_sigma[9],
                ...
                var r = [
                    (current_sigma[0] * dx[0] + current_sigma[1] * dx[1] + current_sigma[2] * dx[2]) * current_sigma[9],
                    (current_sigma[3] * dx[0] + current_sigma[4] * dx[1] + current_sigma[5] * dx[2]) *
                        current_sigma[10],
                    (current_sigma[6] * dx[0] + current_sigma[7] * dx[1] + current_sigma[8] * dx[2]) *
                        current_sigma[11],
                ];
    ...
    for (var k = 0; k < num_particles; k++) {
        ...
        var pv_old = [
            e[0][0] * p[0] + e[0][1] * p[1] + e[0][2] * p[2] + e[0][3], // in voxel coordinates of simulation grid
        ...
        while (steps < num_steps) {
            ...
            if (pv_old != undefined) accumulate_interp(pv_old, p_v, sign);
...
function KSimulationPanel(electrode) {
    ...
    var $simulate = $("<a style='width:80px;' class='KViewPort_tool'> Simulate </a>")
        .appendTooltip("dosim")
        .click(function (e) {
            if (swarm == undefined) {
                doSimulation();
    ...
    var $clearsim = $("<a style='width:80px;' class='KViewPort_tool'> Clear </a>")
        .appendTooltip("clearsim")
        .click(function (e) {
            if (swarm != undefined) {
                swarm.kill();
    ...
    var $analytical = $(
        "<div><span class='DBS_paramname'>Analytical</span> <input class='DBS_paraminput' type='checkbox' value='0'> </div>"
    ).appendTo($fileRow2);
    ...
    var $thresholdCurrent = $(
        "<div><span class='DBS_paramname'>Current threshold</span> <input class='DBS_paraminput' type='number' value='0.1' step='0.05' min='0' max='1'> </div>"
    ).appendTo($fileRow2);
    electrode.threshold = 0.1;
    $thresholdCurrent.find("input").on("change", function (e) {
        electrode.threshold = parseFloat($(this).val());
        for (var x in electrode.onupdate) electrode.onupdate[x]();
    });

    var $wrap = $("<div class='annotation_tool_elemDIV' ></div>").appendTo($fileRow2);
    var $dragarea = $(
        "<div dragover='return false'; class='annotation_tool_droparea' style='height:40px !important;'><span>drop nifti to define conductivity</span></div>"
    ).appendTo($wrap);
    panel.$container.on("dragover", function (ev) {
        ev.preventDefault();
        ev.stopPropagation();
        return false;
    });
    panel.$container.on("dragleave", function (ev) {
        ev.preventDefault();
        ev.stopPropagation();
        return false;
    });
    ...
    var $detailparam_head = $("<div class='DBS_simparams_head'> ... more simulation parameters</div>").appendTo(
        $fileRow2
    );
            ...
            if (!params.patient_study_drop) {
                params[k].callback = function (fobj) {
                    if (fobj.contentType == "nii") {
                        ...
                        update_conduct_nii();
                    } else alertify.error("only niftis allowed");
    ...
    function update_conduct_nii() {
        ...
        if (fobj != undefined) {
            ...
            var $close = $("<i class='fa fa-fw fa-close'></i>").click(function () {
                delete panel.conduct_nii;
                update_conduct_nii();
            });
            $("<div> <div " + dragstuff + ">" + fobj.filename + "</div>  </div>")
                .append($close)
                .appendTo($dragarea);
        } else $("<span>drop nifti to define conductivity </span>").appendTo($dragarea);
    }
        ...
        for (var z = 0; z < current.sizes[2]; z++)
            for (var y = 0; y < current.sizes[1]; y++)
                for (var x = 0; x < current.sizes[0]; x++) {
                    ...
                    current.data[idx] = sig(x, y, z) * Math.sqrt(d[0] * d[0] + d[1] * d[1] + d[2] * d[2]);
                }
    }
        ...
        for (var z = 0; z < current.sizes[2]; z++)
            for (var y = 0; y < current.sizes[1]; y++)
                for (var x = 0; x < current.sizes[0]; x++) {
                    ...
                    sigma.data[idx] =
                        (fac * Math.sqrt(i[0] * i[0] + i[1] * i[1] + i[2] * i[2])) /
    ...
    function showSimInfo(e) {
        ...
        var $info = $("<div class='DBS_siminfo'> progress:" + active.pts.progress + str + " </div>").appendTo(
            $(document.body)
        );
    ...
    function doSimulation() {
        function getSimDomain(name, fobj, tdim, $div) {
            ...
            if (fileObject == undefined || fobj.content.sizes[0] != fileObject.content.sizes[0]) {
        ...
        if (elecGeometry.multipolar.length > 0) {
            var m = elecGeometry.multipolar;
            var abssum = 0;
            for (var k = 0; k < m.length; k++) abssum += Math.abs(m[k].vol);
            for (var k = 0; k < m.length; k++) {
        ...
        n[1] /= norm;
        n[2] /= norm;
        center = [tip[0] + active_pos * n[0], tip[1] + active_pos * n[1], tip[2] + active_pos * n[2]];
        ...
        var bbox = {
            max: [center[0] + boxlen, center[1] + boxlen, center[2] + boxlen],
            min: [center[0] - boxlen, center[1] - boxlen, center[2] + boxlen],
        };

        var edges = math.matrix(math.diag(voxsz));
        for (var i = 0; i < 3; i++) edges._data[i][3] = (-sizes[i] / 2) * voxsz[i] + center[i];
        var fobj = {
            content: {
                edges: edges,
                bbox: bbox,
        ...
        if (panel.conduct_nii) fobj.fileinfo = panel.conduct_nii.fileinfo;
        ...
        simParams.num_particles_ = Math.floor(simParams.num_particles / simParams.num_workers / chunks);

        var accumVol_potential = new Float32Array(filePotential.content.data.length);
        for (var k = 0; k < filePotential.content.data.length; k++) accumVol_potential[k] = 0;
        var accumVol_current = new Float32Array(fileCurrent.content.data.length);
        for (var k = 0; k < fileCurrent.content.data.length; k++) accumVol_current[k] = 0;
        ...
        var cid = setInterval(function () {
            ...
            if (!swarm.ishold) {
                ...
                if (worker_cnt >= numworkers) clearInterval(cid);
            }
        }, 500);

        swarm.kill = function () {
            for (var k = 0; k < numworkers; k++) if (swarm.workers[k] != undefined) swarm.workers[k].kill();
        };
        swarm.toggle = function () {
            var laststate;
            for (var k = 0; k < numworkers; k++) {
                if (swarm.workers[k] != undefined) laststate = swarm.workers[k].toggle();
            }
        ...
        swarm.done = function () {
            var alldone = true;
            for (var k = 0; k < numworkers; k++) alldone = alldone && swarm.workers[k] == undefined;
            if (alldone) {
                $container.find(".panel").removeClass("inactive");
                KViewer.markerTool.$container.removeClass("inactive");
        ...
        function inititateWorker(workerid) {
            ...
            worker.addEventListener(
                ...
                function (e) {
                    function updateImages() {
                        if (filePotential.content == undefined || filePotential.content.data == undefined) return;

                        for (var x in electrode.onupdate) electrode.onupdate[x]();

                        filePotential.content.histogram = comphisto(
                            0,
                            1,
                            50,
                            filePotential.content.data,
                            filePotential.content.data.length,
                            5000
                        );
                        fileCurrent.content.histogram = comphisto(
                            0,
                            0.3,
                            50,
                            fileCurrent.content.data,
                            filePotential.content.data.length,
                            5000
                        );
                        fileSigma.content.histogram = comphisto(
                            0,
                            3,
                            50,
                            fileSigma.content.data,
                            fileSigma.content.data.length,
                            5000
                        );
                    ...
                    if (e.msg == "progress") {
                        ...
                        if (this.workerid == 0) updateImages();
            ...
            var sigma;
            if (panel.conduct_nii) sigma = panel.conduct_nii.content;
...
function KPickingPanel(mset) {
    ...
    var $dragarea = $(
        "<div dragover='return false'; class='annotation_tool_droparea' style='height:40px !important;'><span>drop nifti to for sensing</span></div>"
    ).appendTo($wrap);
    panel.$container.on("dragover", function (ev) {
        ev.preventDefault();
        ev.stopPropagation();
                ...
                for (var j = 0; j < points.length; j++) {
                    if ((points[j].p.name != "tip") & (points[j].p.name != "end")) {
                        ...
                        for (var k = 0; k < nii.numTimePoints; k++)
                            vals.push(
                                Math.abs(trilinInterp(nii, point[0], point[1], point[2], invedges._data, volsz * k))
                            );
    ...
    function update_contrasts() {
        if (panel.cid != -1) clearTimeout(panel.cid);
        panel.cid = setTimeout(function () {
            ...
            for (var k = 0; k < contrasts.length; k++) {
                ...
                if (fobj != undefined) {
                    ...
                    $c.append(
                        $("<i class='KPickerPanelDrop fa fa-trash'> </i>").click(
                            (function (id) {
                                return function () {
                                    delete panel.contrasts[id];
                                    update_contrasts();
                                };
                            })(fobj.fileID)
                        )
                    ...
                    for (var j = 0; j < res.length; j++) {
                        ...
                        if (res[j].vals.length == 1) {
                            ...
                            var data = [];
                            for (var l = 0; l < res[j].vals.length; l++) data.push({ x: l, y: res[j].vals[l] });
    ...
    mset.onupdate["pickerpanel"] = function () {
        ...
        for (var k = 0; k < arr.length; k++) arr[k].onupdate["pickerpanel"] = update_contrasts;
    };
