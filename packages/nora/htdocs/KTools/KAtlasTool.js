...
function KAtlasTool(master) {
    ...
    // the top menu
    var $menu = $("<ul></ul>");
    if (static_info != undefined)
        if (static_info.atlas != undefined) {
            var subfolder = {};
            for (var k = 0; k < static_info.atlas.available.length; k++)
                if (static_info.atlas.available[k].Tag.search("atlas") > -1) {
                    ...
                    if (subfolder[subname] == undefined) subfolder[subname] = [];
                    subfolder[subname].push(static_info.atlas.available[k]);
                }
            for (var subs in subfolder) {
                var $menuhead = $(
                    "<li><a> <span>" + subs + " </span><i class='fa fa-caret-right'></i></a></li>"
                ).appendTo($menu);
                            ...
                            $("<li><a>" + name + "</a></li>")
                                .click(function () {
                                    KViewer.dataManager.loadData({
                                        URLtype: "serverfile",
                                        fileID: id,
                                        json: { project: static_info.atlas.project },
    ...
    function extendBySlicerTXT(fobj, dest) {
        ...
        for (var j = 0; j < data.length; j++) {
            ...
            if (isNaN(parseInt(lab[0]))) continue;
            ...
            if (params.length == 2) {
                ...
                loadingQueue.execQueue(params, function () {
                    ...
                    if ((p1.contentType == "txt") & (p2.contentType == "nii")) {
                        ...
                        that.update();
                    } else alertify.error("Select a text and a nifti file for atlas drop");
                });
    ...
    that.prepAtlas = function (fobj) {
        if (fobj.content.extension != undefined && fobj.content.labels == undefined) {
            ...
            for (var k = 0; k < labelshtml.length; k++) {
                var x = labelshtml[k];
                var col = math.round([
                    255 * parseFloat(x.getAttribute("Red")),
                    255 * parseFloat(x.getAttribute("Green")),
                    255 * parseFloat(x.getAttribute("Blue")),
                ]);
                labels[x.getAttribute("Key")] = { key: x.getAttribute("Key"), name: x.textContent, color: col };
            }
            fobj.content.labels = labels;
        } else if (
            (fobj.filename.search("\\.mgz") > -1 || fobj.filename.search("\\.mgh") > -1) &&
            fobj.content.labels == undefined
        ) {
    ...
    that.addAtlas = function (fobj) {
        if (fobj.contentType == "nii") {
            // henriks flatmap
            if (0) {
                //fobj.content.sizes[3] == 4)
                that.invdefField = fobj;
            }

            if ((fobj.content.datatype == "float") & ((fobj.content.sizes[3] == 3) | (fobj.content.sizes[4] == 3))) {
                // a warp field
                that.defField = fobj;
                ...
                if (that.objs[fobj.fileID] != undefined) return that.objs[fobj.fileID];
    ...
    that.toggleAllLabels = function (obj, on) {
        ...
        if (Object.keys(obj.panel.persistentLabels).length > 0 || !on) obj.panel.hideAll();
        else obj.panel.showAll();
    };
    ...
    that.update = function (atpoint) {
        ...
        if (that.defField != undefined) {
            ...
            for (var j = 0; j < 3; j++) {
                ...
                if (tmp != undefined)
                    str[j] = trilinInterp(
                        ...
                        p[2],
                        diag,
                        def.sizes[0] * def.sizes[1] * def.sizes[2] * j
                    ).toFixed(1);
                else str[j] = "?";
            }
            str = "(" + str.join(",") + ")";
            ...
            $row.append(
                $("<td> <i class='fa  fa-close'></td>").click(function () {
                    that.defField = undefined;
                    that.update();
                    signalhandler.send("positionChange");
                })
            );
        ...
        for (var k in that.objs) {
            ...
            if (that.defField == undefined) point = KViewer.currentPoint;

            if (point == undefined) point = math.matrix([0, 0, 0, 1]);
            if (atpoint != undefined) point = atpoint;

            function drophandler(fobj, dropevent) {
                if (fobj.contentType == "txt") {
                    extendBySlicerTXT(fobj, that.objs[k]);
                    that.update();
                } else that.addAtlas(fobj);
            }
            var dragstuff =
                "draggable='true' data-type='file' data-mime='nii' data-tag='/atlas/' data-fileID='" +
                that.objs[k].fileID +
                "' data-filename='" +
            ...
            var $row = $("<tr " + dragstuff + "></tr>")
                .appendTo($tbody)
                .on("drop", loadFromDrop(drophandler));
            $row.append(
            ...
            if (nii.labels != undefined) $row.append($("<td >" + that.objs[k].filename + "</td>"));
            else $row.append($("<td >" + that.objs[k].filename + " (no label info) </td>"));
            $row.append(
                $("<td> <i class='tablebutton fa fa-fw fa-building-o'></td>").click(
                    (function (k_) {
                        return function () {
                            if (that.objs[k_].panel == undefined) that.objs[k_].panel = KAtlasPanel(that.objs[k_]);
                            else that.objs[k_].panel.toggle();
                        };
                    })(k)
                )
            );
            $row.append(
                $("<td> <i class='tablebutton fa fa-fw fa-eye'></td>").click(
                    (function (k_) {
                        return function () {
                    ...
                    $row.append(
                        $(
                            "<td style='background-color:" +
                                RGB2HTML(label.color[0], label.color[1], label.color[2]) +
                                ";'  ></td>"
                        )
                    );
...
*/
            ...
            for (var y = 0; y < sz[1]; y++)
                for (var x = 0; x < sz[0]; x++) {
                    ...
                    if (key > 0) {
                        ...
                        if (b.max[0] < X) b.max[0] = X;
                        if (b.min[0] > X) b.min[0] = X;
                        if (b.max[1] < Y) b.max[1] = Y;
                        if (b.min[1] > Y) b.min[1] = Y;
                        if (b.max[2] < Z) b.max[2] = Z;
                        if (b.min[2] > Z) b.min[2] = Z;
                    }
                }
    ...
    function ready4Clone(x, withbuffer) {
        if (x != undefined) {
            ...
            if (withbuffer) res.buffer = x.buffer;
            return res;
    ...
    function renderROIfromLabel(obj, fobj, val, done) {
        if (val == undefined) val = 1;
            ...
            var labs = {};
            for (var k = 0; k < val.length; k++) labs[val[k]] = true;
            val = labs;
        }
        ...
        if (that.defField) {
            ...
            ((master.navigationTool.movingObjs[obj.fileID] != undefined) & (KViewer.navigationMode == 0)) |
            ...
            if (typeof patientTableMirror != "undefined") {
                ...
                if (finfo != false) fobj.fileinfo = $.extend(fobj.fileinfo, finfo);
                else {
    ...
    function renderROIintoAtlas(obj, fobj, val, typ, done) {
        ...
        executeImageWorker(
            { func: "RoiToAtlas", atlas: ready4Clone(obj.content, true), img: ready4Clone(img), key: val, typ: typ },
            [obj.content.buffer],
            that.progressSpinner,
            function (e) {
    ...
    function createISOfromLabel(atlas, label, ondone) {
        ...
        if (that.defField != undefined) {
            ...
            else if (ondone) ondone(label.surfacereference);
        }
    }
    ...
    that.getROIfromSinglelabel = function (atlas, key, name, bgndcontrast, callback, progress) {
        ...
        if (roi != undefined) {
            function tryit() {
                if (roi.rendering_in_progress) setTimeout(tryit, 100);
                else callback(roi);
            }
            tryit();
        } else {
    ...
    that.createView = function (fobj, viewer, intent) {
        var viewer = viewer;
        var obj = { atlas: fobj, nii: fobj.content, type: "atlas", color: 0 };
        ...
        obj.divs = [
            $("<br style='clear:both' />"),
            $("<div  class='KViewPort_tool atlas persistent'>  <i class='fa fa-close fa-1x'></i></div>")
                .click(close)
                .mousedown(viewer.viewport.closeContextMenu(obj)),
            ($save = $("<div  class='KViewPort_tool atlas'>  <i class='fa fa-save'></i></div>")
                .appendTooltip("save")
                .click(function () {
                    ...
                    } else alertify.error("close first editing mode");
                })),
            ($createIso = $("<div  class='KViewPort_tool atlas'>  <i class='fa fa-1x'><span>3D</span></i></div>")
                .appendTooltip("isosurfatlas")
                .hide()),
            ($createOutlines = $(
                "<div  class='KViewPort_tool atlas'>  <i class='fa fa-1x'><span> <i class='fa fa-lemon-o fa-1x'></i></span></i></div>"
            ).appendTooltip("multioutlines")),
            ($panel = $("<div  class='KViewPort_tool atlas'>  <i class='fa fa-building-o fa-1x'></i></div>")
                .appendTooltip("open panel")
                .click(function () {
                    if (obj.atlas.panel == undefined) obj.atlas.panel = KAtlasPanel(obj.atlas);
                    else obj.atlas.panel.toggle();
                })),
            $("<div  class='KViewPort_tool atlas'>  <i class='fa fa-eye fa-1x'> </div>").click(function () {
                if (obj.atlas.panel == undefined) {
                    obj.atlas.panel = KAtlasPanel(obj.atlas);
                    obj.atlas.panel.toggle();
                }
                if (Object.keys(obj.atlas.panel.persistentLabels).length > 0) obj.atlas.panel.hideAll();
                else obj.atlas.panel.showAll(true);
            }),
            $("<div  class='KViewPort_tool atlas'>  <i class='fa fa-plus fa-1x'> </div>")
                .appendTooltip("create ROI from atlas region")
                .click(
                    (function (viewer) {
                        return function () {
                            ...
                            if (
                                obj.atlas.panel != undefined &&
                                Object.keys(obj.atlas.panel.persistentLabels).length > 0
                            ) {
                            ...
                            if (atlaskeys == -1) {
                                alertify.error(
                                    "no labels selected; select some labels in the panel or point to a label via the crosshair"
                                );
                                return;
                            }

                            that.getROIfromSinglelabel(
                                obj.atlas,
                                atlaskeys,
                                name,
                                viewer.content,
                                function (roi) {
                                    viewer.viewport.setContent(roi, {});
                                },
            ...
            ($editing = $("<div  class='KViewPort_tool atlas'>  <i class='fa fa-pencil fa-1x'> </div>")
                .appendTooltip("edit atlas region")
                .click(
                    (function (viewer) {
                        return function (e) {
                            if (obj.atlas.editing_label != undefined) {
                                            ...
                                            KViewer.iterateMedViewers(function (m) {
                                                for (var k = 0; k < m.atlas.length; k++)
                                                    if (obj.atlas.fileID == m.atlas[k].atlas.fileID) {
                                                        ...
                                                        m.viewport.setContent(roi, {
                                                            intent: { nosave: true, color: label.color },
                                                        });
                                                    }
                                            });
                                ...
                                if (obj.atlas.currentLabel.key <= 0) {
                                    ...
                                    var prompt = {
                                        msg: "Create new or edit existing",
                                        opt: ["NEW"].concat(labels),
                                        optMsg: "Labels:",
                                    };
                                    alertify.prompt(
                                        prompt,
        ...
        $dragdiv.on(
            ...
            dragstarter({
                ...
                tag: "/mask/",
                mime: "nii",
                filename: obj.atlas.filename,
                intent: "atlas:true",
                close: close,
                fileID: obj.atlas.fileID,
            })
        );
                    ...
                    KViewer.iterateMedViewers(function (m) {
                        for (var k = 0; k < m.atlas.length; k++)
                            if (obj.atlas.fileID == m.atlas[k].atlas.fileID) {
                                ...
                                if (str != "cancel") m.atlas[k].$savediv.addClass("notsaved");
                            }
                    });
        ...
        function update() {
            if (obj.atlas.currentLabel != undefined)
                $captiondiv.html(
                    objname +
                        ": <b>" +
                        obj.atlas.currentLabel.name +
                        " (" +
                        (obj.atlas.currentLabel.key == -1 ? 0 : obj.atlas.currentLabel.key) +
                        ")</b>"
                );
        }
        ...
        obj.clearMultiOutlines = function () {
            if (obj.multi_outlines != undefined) {
                for (var j = 0; j < obj.multi_outlines.length; j++) obj.multi_outlines[j].close();
            }
        ...
        obj.draw_multioutline = function (ev) {
            if (obj.multi_outlines) obj.multi_outline_state = (obj.multi_outline_state + 1) % 2;
            ...
            for (var k in vis_labels) {
                ...
                if (obj.multi_outline_state == 1) m.no_labels = true;
                else m.no_labels = false;
                m.update(viewer);
        ...
        $createIso.click(function (ev) {
            var keys = [];
            if (obj.atlas.panel) keys = Object.keys(obj.atlas.panel.persistentLabels);
        ...
        function saveAtlas(atlas, done) {
            if (electron | (userinfo.username == guestuser)) {
                if (atlas.modified) {
                    ...
                    KViewer.cacheManager.update();
                } else alertify.error("no changes, not saving ...");
                return;
            }

            var zipped = true;

            if (atlas.fileinfo && atlas.fileinfo.Filename) {
                if (atlas.fileinfo.Filename.search("\\.gz") == -1) zipped = false;
                ...
                else that.lastSaveName = atlas.fileinfo.Filename;
            }
            if (atlas.notzipped) zipped = false;
            ...
            saveDialog(
                ...
                function (name, finfo) {
                    ...
                    uploadUnregisteredBinary(
                        ...
                        zipped,
                        { dontremoveoldinstance: true, askonOverwrite: true }
                    );
                },
                that.lastSaveName,
                atlas.fileinfo,
        ...
        function updateGetPixelFunction(nii, R) {
            var Labels = {};
            if (obj.atlas.panel) Labels = $.extend(false, Labels, obj.atlas.panel.persistentLabels);
            if (obj.atlas.currentLabel)
                if (obj.atlas.currentLabel.key != "0") Labels[obj.atlas.currentLabel.key] = obj.atlas.currentLabel;
            if (obj.atlas.editing_label != undefined) {
                Labels[obj.atlas.editing_label.key] = undefined;
            }

            if (Object.keys(Labels).length == 0)
                obj.getPixel = function () {
                    return [0, 0, 0, 0];
                };
            else {
                if (that.defField != undefined)
                    obj.getPixel = KAtlasTool.updateGetPixelFun(
                        obj.atlas.content,
                        nii,
                        Labels,
                        R,
                        that.defField.content
                    );
                else if (
                    KViewer.navigationTool.isinstance &&
                    ((master.navigationTool.movingObjs[obj.atlas.fileID] != undefined) &
                    ...
                    if (KViewer.reorientationMatrix.deffield)
                        obj.getPixel = KAtlasTool.updateGetPixelFun(
                            ...
                            Labels,
                            R,
                            KViewer.reorientationMatrix.deffield
                        );
                    else obj.getPixel = KAtlasTool.updateGetPixelFun(obj.atlas.content, nii, Labels, R);
                } else obj.getPixel = KAtlasTool.updateGetPixelFun(obj.atlas.content, nii, Labels, undefined);
            }
            return obj.getPixel;
        }
        ...
        function close(force) {
            ...
            if (obj.clearMultiOutlines) obj.clearMultiOutlines();
    ...
    that.$topRow.append(
        $(
            "<li style='margin-left:5px; padding-left:5px; border-left:1px solid gray'><a>Statistics</a> </i></li>"
        ).click(function () {
            that.statdlg.toggle();
        ...
        function dostats_(which) {
            ...
            for (var kk = 0; kk < ats.length; kk++) {
                ...
                for (var j = 0; j < imgs.length; j++) {
                    var name;
                    if (_imgs[imgs[j]].currentFilename != undefined) name = _imgs[imgs[j]].currentFilename;
                    else name = _imgs[imgs[j]].filename;
                    ...
                    var nii;
                    if (_imgs[imgs[j]].nii != undefined) nii = _imgs[imgs[j]].niiOriginal;
                    else nii = _imgs[imgs[j]].content;
                    ...
                    for (var k = 0; k < keys.length; k++) {
                        ...
                        var s = stats[keys[k]];
                        if (s.label) var $span = $("<span> " + s.label.name + "</span>");
                        else var $span = $("<span> undefined</span>");
                        $row.append($("<td></td>").append($span));
                        ...
                        if (!rois[imgs[j]]) {
                            var $span = $(
                                "<span> " +
                                    niceFormatNumber(s.mean) +
                                    " / " +
                                    niceFormatNumber(scale(s.mean)) +
                                    "</span>"
                            );
                            $row.append($("<td></td>").append($span));
                            var $span = $(
                                "<span> " + niceFormatNumber(s.std) + " / " + niceFormatNumber(scale(s.std)) + "</span>"
                            );
                            $row.append($("<td></td>").append($span));
                            ...
                            $row.append($("<td></td>").append($span));
                            var $span = $(
                                "<span> " + niceFormatNumber((0.5 * s.mean) / (s.cnt + s.totcnt)) + "</span>"
                            );
                            $row.append($("<td></td>").append($span));
                        }
                    }
...
KAtlasTool.isAtlas = function (fobj) {
    if (fobj.content.extension != undefined && fobj.content.datatype != "float") {
        if (
            (fobj.content.extension.content.contentType == "application/xml") |
            (fobj.content.extension.content.contentType == "text/xml")
        ) {
            if (fobj.content.extension.content.getElementsByTagName("Label").length > 0) return true;
        }
    }

    if (fobj.fileinfo && fobj.fileinfo.Tag && fobj.fileinfo.Tag.search("/atlas/") > -1) return true;
    return false;
};
...
KAtlasTool.isFlatMap = function (fobj) {
    if (fobj.content.extension != undefined) {
        if (
            ...
            if (fobj.content.extension.content.getElementsByTagName("FlatMap").length > 0) return true;
        }
    }

    if (fobj.fileinfo && fobj.fileinfo.Tag && fobj.fileinfo.Tag.search("/flatmap/") > -1) return true;
    return false;
};
...
KAtlasTool.updateGetPixelFun = function (atlas_nii, nii, Labels, R, def, fromWorld) {
	...
	*/
    ...
    var edges;
    if (fromWorld) edges = math.diag([1, 1, 1, 1]);
    else edges = nii.edges;
    ...
    if (def != undefined) {
        var A;
        if (def.invedges == undefined) def.invedges = math.inv(def.edges);

        if (R == undefined) A = math.multiply(def.invedges, edges)._data;
        else
            A = math.multiply(
                math.multiply(math.multiply(def.invedges, KViewer.reorientationMatrix.matrix), edges),
                R
            )._data;

        var B;
        if (atlas_nii.invedges) B = atlas_nii.invedges._data;
        else B = math.inv(atlas_nii.edges)._data;
        ...
        if (R == undefined) {
            ...
            A = math.multiply(
                math.multiply(math.multiply(math.inv(atlas_nii.edges), KViewer.reorientationMatrix.matrix), edges),
                R
            )._data;

        if (isIdentity(A)) A = math.eye(4)._data;
...
function KAtlasPanel(atlas, state) {
    var panel = KPanel(
        $(document.body),
        atlas.fileID,
        "Atlas: " + atlas.filename.replace(".nii", "").replace(".gz", "")
    );
    ...
    panel.computeCentroids = function (nii) {
        ...
        for (var x in nii.present_labels) {
            if (nii.labels[x].centroid != undefined) {
                ...
                if (
                    nii.data[
                        nii.labels[x].centroid[0] +
                            nii.wid * nii.labels[x].centroid[1] +
                            nii.widhei * nii.labels[x].centroid[2]
                    ] != x
                )
    ...
    panel.showAll = function (all) {
        ...
        $body.find("tr").map(function () {
            ...
            if ($(this).is(":visible") | all) panel.persistentLabels[key] = atlas.content.labels[key];
        });
        updatePersistentLabels(false);
        signalhandler.send("updateImage", { id: atlas.fileID });
    };
    ...
    var $showAll = $("<a class='KViewPort_tool'><span> Show all </span></a>")
        .appendTooltip("show all regions permanently")
        .click(function (e) {
            panel.showAll();
        });
    $fileRow.append($showAll).append($("<i class='flexspacer'></i>"));

    var $hideAll = $("<a class='KViewPort_tool'><span> Hide all </span></a>")
        .appendTooltip("hide all regions")
        .click(function (e) {
            panel.persistentLabels = {};
            updatePersistentLabels(false);
            //		    	signalhandler.send('positionChange');
    ...
    var $outlines = $("<a class='KViewPort_tool'><span> Outlines </span></a>")
        .appendTooltip("show outlines and labeling")
        .click(function (e) {
            signalhandler.send("updateImage", { id: atlas.fileID, outlines: true });
        });
    $fileRow.append($outlines).append($("<i class='flexspacer'></i>"));

    var $center = $("<a class='KViewPort_tool'><span> center </span></a>")
        .appendTooltip("center atlas")
        .click(function (e) {
            KAtlasTool.center(atlas);
        });
    $fileRow.append($center).append($("<i class='flexspacer'></i>"));

    var $flipLR = $("<a class='KViewPort_tool'><span> flipLR </span></a>").click(function (e) {
        var np = {};
        var map = {};
        var a = [["_L", "_R"]];
        ...
        for (var k in panel.persistentLabels) {
            ...
            if (map[s] != undefined) np[map[s].key] = map[s];
        }
    ...
    */
    ...
    if (state != undefined) {
        if (state.alpha != undefined) atlas.content.alpha = state.alpha;
    }

    var $stats = $("<a class='KViewPort_tool'><span> Stats </span></a>")
        .appendTooltip("compute statistics over contrasts")
        .click(function () {
    ...
    var $alpha = $(
        "<a class='KViewPort_tool'><span> alpha </span> <input style='height:15px;width:40px' min=0.1 max=1 step=0.1 type='number' value='" +
            atlas.content.alpha +
            "'> </a>"
    );
            ...
            if ($row.length > 0 && scrollTo && panel.currentLabel != k) {
                ...
                if ($row.position().top != 0)
                    $row.parent()
                        .parent()
                        .parent()
                        .scrollTop($row.position().top - 65);
            } else panel.currentLabel = k;
        }
    }
    ...
    function updatePersistentLabels(updateWMQLquery) {
        ...
        if (typeof KWMQLPanel != "undefined") {
            ...
            keys = keys.filter(function (x) {
                return x.search(atlas.fileID + "_") > -1;
            });
    ...
    panel.update = function () {
        ...
        for (var k in atlas.content.present_labels) {
            ...
            var dragstuff =
                "class='atlasdrag' draggable='true' data-type='file' data-mime='nii' data-tag='/roi/' data-intent='labelname:\"" +
                label.name +
                '",atlaskey:' +
                label.key +
                "'  data-fileID='" +
            ...
            $row.click(
                (function (k, $row) {
                    return function (e) {
                        if (e.ctrlKey) {
                            if (panel.persistentLabels[k]) delete panel.persistentLabels[k];
                            else panel.persistentLabels[k] = atlas.content.labels[k];
                            updatePersistentLabels(true);
                            signalhandler.send("updateImage", { id: atlas.fileID });

                            //	    	signalhandler.send('positionChange');
                        } else if (e.shiftKey) {
                            $row.prevUntil(".selected")
                                .addBack()
                                .each(function (x, y) {
                                    var a = $(y).attr("value");
                                    if ($(y).is(":visible")) {
                                        if (panel.persistentLabels[a]) delete panel.persistentLabels[a];
                                        else panel.persistentLabels[a] = atlas.content.labels[a];
                                    }
                                });
                            updatePersistentLabels(true);
                            signalhandler.send("updateImage", { id: atlas.fileID });
