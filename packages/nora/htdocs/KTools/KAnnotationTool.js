function KMarkerTool(master, toolname) {
    ...
    var that = new KToolWindow(
        master,
        $("<div class='KView_tool '><i class='fa fa-comment-o fa-1x'></i></div>").append(
            $("<ul class='KView_tool_menu'></ul>").append($("<li>Markers</li>"))
        )
    );
    ...
    if (markerProxy == undefined) markerProxy = new KMarkerProxy();

    // choose the default markerPanel
    //  var markerPanel = KMarkerPanel();

    function createMarkerSet(type, name) {
        if (name == undefined) name = "undefined " + type;
        var n = that.newSet(name, type);
        if (type != "electrode") n.setType(type);
        return n;
    }
    ...
    $("<li><a>pointset</a></li>")
        .click(function () {
            createMarkerSet("pointset");
        })
        .appendTo($menu);
    $("<li><a>freeline</a></li>")
        .click(function () {
            createMarkerSet("freeline");
        })
        .appendTo($menu);
    $("<li><a>correspondence</a></li>")
        .click(function () {
            createMarkerSet("correspondence");
        })
        .appendTo($menu);
    $("<li><a>surface</a></li>")
        .click(function () {
            createMarkerSet("surface");
        })
        .appendTo($menu);

    $("<li><a>electrode</a></li>")
        .click(function () {
            createMarkerSet("electrode");
        })
        .appendTo($menu);
    $("<li><a>pointROI</a></li>")
        .click(function () {
            createMarkerSet("pointROI");
        })
        .appendTo($menu);
    $("<li><a>rectangles</a></li>")
        .click(function () {
            createMarkerSet("rectangles");
        })
        .appendTo($menu);
    $("<li><a>2diameter</a></li>")
        .click(function () {
            createMarkerSet("2diameter");
        })
        .appendTo($menu);
    ...
    that.$topRow.append(
        $("<li><a><i class='fa fa-save'></i> Save</a></li>").click(function () {
    ...
    that.$topRow.append(
        $("<li><a><i class='fa fa-trash'></i> Clear</a></li>").click(function () {
            function clear() {
                markerProxy.delAll();
                that.update();
            }
            if (markerProxy.modified)
                alertify.confirm("Delete all modifcations on current annotations?", function (e) {
                    if (e) {
                        clear();
                    }
                });
    ...
    btns.$showall = $("<i class='KViewPort_tool fa fa-refresh'></i>")
        .appendTooltip("show all markers")
        .click(showAll)
        .appendTo($toolsRow);
    btns.$locked = $("<i class='KViewPort_tool fa fa-lock'></i>")
        .appendTooltip("lockmarkers")
        .click(function () {
            var dest = (markerProxy.currentSet.state.locked + 1) % 3;
            for (var s in markerProxy.markersets) markerProxy.markersets[s].toggleStateVar("locked", dest);
        })
        .appendTo($toolsRow);
    btns.$createonclick = $("<i class='KViewPort_tool fa fa-pencil'></i>")
        .appendTooltip("markercreateonclick")
        .click(function () {
            markerProxy.currentSet.toggleStateVar("createonclick");
        })
        .appendTo($toolsRow);
    btns.$cyclecolors = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-recycle'></i>")
        .appendTooltip("cyclecolors")
        .click(function () {
            markerProxy.currentSet.toggleStateVar("cyclecolors");
        })
        .appendTo($toolsRow);
    btns.$hoverdetails = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-info'></i>")
        .click(function () {
            markerProxy.currentSet.toggleStateVar("hoverdetails");
        })
        .appendTooltip("markerdetailsonhover")
        .appendTo($toolsRow);
    btns.$follow = $("<i class='KViewPort_tool fa fa-crosshairs'></i>")
        .click(function () {
            markerProxy.currentSet.toggleStateVar("follow");
        })
        .appendTooltip("follow marker position")
        .appendTo($toolsRow);
    ...
    that.newSet = function (name, type) {
        var opts = { name: name, type: type, showPanel: false, hideOtherPanels: true };
    ...
    that.update = function () {
        ...
        for (var i = 0; i < ks.length; i++) {
            ...
            $titlerow.on("drop", function (e) {
                if (!isDragFromMarkertool(e.originalEvent)) return;
                that.dragged.removeClass("isDragged");
            ...
            $titlerow.on(
                ...
                (function (uid) {
                    return function (e) {
                        ...
                        if (tid != uid) moveObjKey(tid, uid, markersets);
            ...
            $name.on(
                ...
                (function (k) {
                    return function () {
                        ...
                        if (markersets[k].markerPanel) markersets[k].markerPanel.updateName();
                    };
                })(k)
            );

            $titlerow.append(
                $("<i class='fa fa-trash'></i> ").click(
                    (function (k) {
                        return function () {
            ...
            $titlerow.append(
                $("<i class='fa fa-dot-circle-o'></i> ").click(
                    (function (k) {
                        return function () {
                            var set = markersets[k];
                            if (set.pickpanel == undefined) set.pickpanel = KPickingPanel(set);
                            else set.pickpanel.toggle();
                        };
                    })(k)
                )
            );
            ...
            $titlerow.append(
                $("<i class='fa fa-copy'></i> ").click(
                    (function (k) {
                        return function () {
                            var content = { annotations: [markerProxy.markersets[k].objectify()] };
                            var $temp = $(
                                "<textarea style='position:absolute; display:block;z-index:99999;top:0px'>" +
		   ...
		   */
            ...
            $titlerow.append(
                $("<i class='fa fa-save'></i> ").click(
                    (function (k) {
                        return function () {
                            markerProxy.save(undefined, k);
                            that.update();
                        };
                    })(k)
                )
            ...
            if (markersets[k].type != "electrode")
                $titlerow.append(
                    $("<i class='fa fa-plus'></i> ").click(
                        (function (k) {
                            return function () {
                                markerProxy.createMarker(undefined, markersets[k]);
                    ...
                    $("<i class='fa fa-arrows-v'></i> ")
                        .click(
                            (function (k) {
                                return function () {
                                    alertify.prompt(
                                        "Move electrode along trajectory by (mm)",
                                        function (e, str) {
                                            if (e) {
                                                ...
                                                for (var j = 0; j < arr.length; j++)
                                                    for (var i = 0; i < 3; i++) arr[j][i] += (d[i] / dnorm) * le;
                        ...
                        .appendTooltip("move electrode along trajectory")
                );
            }
            if ((markersets[k].type == "electrode") | (markersets[k].type == "2diameter"))
                $titlerow.append(
                    $("<i class='fa fa-mars-stroke-v'></i> ")
                        .appendTooltip("view images along trajectory")
                        .click(
                            (function (k) {
                                return function () {
                                    markerProxy.markersets[k].Rectifytransform();
                                };
                            })(k)
                        )
                );
            ...
            if (markersets[k].type == "pointset") {
                $titlerow.append(
                    $("<i class='fa fa-cube'></i> ")
                        .appendTooltip("defineMCPsystem")
                        .click(
                            (function (k) {
                                return function () {
                                    markerProxy.markersets[k].MCPtransform();
                                };
                            })(k)
                        )
                );
                $titlerow.append(
                    $("<i class='fa fa-cubes'></i> ")
                        .appendTooltip("defineRECTsystem")
                        .click(
                            (function (k) {
                                return function () {
                                    markerProxy.markersets[k].RECTtransform();
                                };
                            })(k)
                        )
                );
            ...
            var $colselector = KColorSelector(
                colors,
                function (c) {
                    if (c.getCSS) return "background:" + c.getCSS() + "";
                },
                (function (k) {
                    return function (col, colidx) {
                        ...
                        if (x.type != "electrode")
                            for (var j in x.markerpoints) {
                                ...
                                } else x.markerpoints[j].setcolor(col);
                            }
                        x.color = col.color;
                        x.updateLine();
                    };
...
*/
            ...
            $titlerow.append(
                $("<i class='fa fa-building-o'></i> ").click(
                    (function (k) {
                        return function () {
                            markersets[k].showPanel();
                        };
                    })(k)
                )
            );
            ...
            var $typecombo = $(
                "<select> <option value='pointset'> point set </option>" +
            ...
            var $setinfo;
            if (
                markersets[k].type == "freeline" ||
                markersets[k].type == "surface" ||
                markersets[k].type == "2diameter" ||
                markersets[k].type == "pointset"
            ) {
                var $info = $("<i class='fa fa-info KannoInfo'></i> ");
                $titlerow.append($info);
                $setinfo = $("<div class='annotation_info'> ???: </div>").appendTo($info);
                markersets[k].setInfo = (function ($setinfo) {
                    ...
                    .click(
                        (function (set) {
                            return function (e) {
                                ...
                                e.preventDefault();
                                var sets = [set];
                                if (e.shiftKey) sets = markerProxy.markersets;
                                for (var k in sets) {
                                    var s = sets[k];
                                    markerProxy.updateElectrode(s, e.target.value);
                                    if (s.markerPanel) s.markerPanel.update();
                                    s.pointChanged();
                                }
                ...
                $ohm.append(
                    $("<input> ")
                        .val(markersets[k].electrode_properties.current)
                        .on(
                            "change",
                            (function (set) {
                                return function (e) {
                ...
                var $simpanel = $("<button> Simulation Panel </button>")
                    .appendTo($setinfo)
                    .click(
                        (function (set) {
                            return function (e) {
                                if (set.simpanel == undefined) set.simpanel = KSimulationPanel(set);
                                else set.simpanel.toggle();
                            };
                        })(markersets[k])
                    );
                ...
                (function (k, $name, $titlerow) {
                    return function (e) {
                        if (e.ctrlKey) {
                            ...
                            setTimeout(function () {
                                if ($name.attr("contentEditable")) return;
                                else {
            ...
            for (var j = 0; j < ps.length; j++) {
                ...
                if (markersets[k].type == "electrode") {
                    var offs = 2;
                    if (markersets[k].electrode_properties.directional) offs = 3;
                    thingstoshow.active = j >= offs;
                    thingstoshow.radius = j >= offs;
                    if (ps[j].p.name.substring(0, 2) == "xx") thingstoshow.hidden = true;
                }
                if (thingstoshow.hidden);
                else {
                    var $div = p.createMarkerRepresentation("tool", thingstoshow);
                    $setrow.append($div);
                    if (markersets[k].type == "pointset") $div.append($("<hr width='100%'> "));
                }
            }
    ...
    that.customToggle = function (enabled) {
        ...
        if (mset != undefined) {
            if (enabled) mset.drawAllPoints();
        }

        if (!enabled) markerProxy.hideAll();
    };

    that.$container.on("dragover", function (ev) {
        ev.preventDefault();
    });
...
KMarkerProxy = function () {
	...
	*******************************************************************************/
    that.updateElectrode = function (set, model) {
        var directional = 0;
        if (model == "BostonOctopolarDir") directional = 1;
        ...
        while (Object.keys(set.markerpoints).length < contacts.length + 2 + directional) {
            ...
            point.visible = true;
            for (x in point.glmesh) point.glmesh[x].isPickable = false;
        ...
        var ps = set.getPoints();
        for (var j = 0; j < ps.length; j++) ps[j].active = false;
        ps[2].active = true;
	...
	*******************************************************************************/
    that.objectify = function (key) {
        var content = [];
        for (var k in markersets) {
            if (key == undefined || key == k) content.push(markerProxy.markersets[k].objectify());
        }
        return content;
    };
	...
	*******************************************************************************/
    that.save = function (name, key, finfo) {
        ...
        if (typeof key == "object") key = key.uuid;
	...
	*******************************************************************************/
    that.import = function (a, replaceIntoSet) {
        var current = 0;
        if (a.length == 0) return replaceIntoSet;
        for (var k = 0; k < a.length; k++) {
            ...
            if (n.electrode_properties != undefined && a[k].electrode_properties != undefined) {
                ...
                if (a[k].electrode_properties.color != undefined) n.color = a[k].electrode_properties.color;
            }

            if (a[k].state != undefined) {
                if (a[k].state.keepalive != undefined) a[k].state.keepalive = false;
            ...
            for (var p = 0; p < a[k].points.length; p++) {
                ...
                if (1) {
                    ...
                    if (a[k].color) point.setcolor(new KColor(a[k].color));
                    if (a[k].points[p].color) point.setcolor(new KColor(a[k].points[p].color));
                    if (a[k].points[p].visible != undefined && a[k].points[p].visible == false) point.togglepoint();
                    if (a[k].points[p].dir) point.setdir(a[k].points[p].dir);

                    if (a[k].points[p].size) point.setsize(a[k].points[p].size);
                    else point.setsize(n.state.defaultradius);

                    if (a[k].points[p].subpoints) point.subpoints = a[k].points[p].subpoints;
                    ...
                    if (n.electrode_properties != undefined) {
                        ...
                        } else point.active = undefined;
                    }
                    if (a[k].points[p].voltage != undefined) point.p.voltage = a[k].points[p].voltage;
                    if (a[k].points[p].isContact != undefined) point.isContact = a[k].points[p].isContact;
                    if (a[k].points[p].pickable != undefined) point.pickable = a[k].points[p].pickable;
                    ...
                    if (point.roinii && point.roinii.fileObject) point.roinii.fileObject.modified = false;

                    if (a[k].points[p].formcontent) point.formcontent = a[k].points[p].formcontent;
                    if (a[k].points[p].thresh != undefined) {
                        point.thresh = a[k].points[p].thresh;
                        if (point.onupdate.pointROI != undefined) point.onupdate.pointROI();
                    }
                }
            }
            /*
			...
			*/
            ...
            if (a[k].type == "electrode") {
                if (a[k].elecmodel == undefined) {
                    ...
                    var pts = n.getPoints();
                    if (pts.length == 2) that.updateElectrode(n, a[k].elecmodel);
                    else {
                        n.electrode_properties = $.extend(true, n.electrode_properties, electrodes[a[k].elecmodel]);
                        n.elecmodel = a[k].elecmodel;
                        if (n.$elecmodel_combo) n.$elecmodel_combo.val(a[k].elecmodel);
                        n.pointChanged();
                        var alpha = 10;
                        for (var j in n.markerpoints) n.markerpoints[j].p.color.color[3] = alpha;
            ...
            if (n && n.state && n.state.panel_visible == true) n.showPanel();

            if (n.markerPanel) n.markerPanel.update();
        }
        markerProxy.modified = false;
	...
	*******************************************************************************/
    that.loadAnnotations = function (fileObject, replaceIntoSet, params, quiet) {
            ...
            if (that.markersetIDs.length > 0)
                alertify.confirm("Append to existing annotations?", function (e) {
                    ...
                    if (!append & markerProxy.modified)
                        alertify.confirm("Are you sure to delete all annotations?", function (e) {
                            if (e) {
                                load();
                            }
                        });
        ...
        function load() {
            ...
            if (a.content != undefined) a = a.content;
            if (a.annotations != undefined) a = a.annotations;

            if (!append) markerProxy.delAll();
            ...
            KViewer.markerTool.lastMarkerCollName =
                subfolder + fileObject.filename.replace(".json", "").replace(".ano", "");
            ...
            if (params.intent && params.intent.intent && params.intent.intent.state) {
                ...
                for (var k in state) sets.toggleStateVar(k, state[k]);
            }
            ...
            if (params.intent && params.intent.color != undefined) {
                ...
                for (var j in sets.markerpoints) sets.markerpoints[j].setcolor(col);
            }
        ...
        function import_fcsv(fobj) {
            ...
            for (var k = 0; k < ls.length; k++) {
                if (ls[k].search("columns") > -1) {
                    ...
                    var toks = s.split(",");
                    hdr = {};
                    for (var j = 0; j < toks.length; j++) hdr[toks[j]] = j;
                    continue;
                }
                if (ls[k].trim()[0] == "#") continue;
                var toks = ls[k].split(",");
                content.push(toks);
            }
            var points = [];
    ...
    /******************************************************************************
	  newSet
	*******************************************************************************/
    that.newSet = function (
        args,
        donotsetcurrentset //  name, type, uuid, options)
    ) {
        // uuid already exists, try to take from list of markerset
        if (args && args.uuid != undefined && that.markersets[args.uuid]) {
            if (args.showPanel) that.markersets[args.uuid].showPanel();
            return that.markersets[args.uuid];
        }

        // markerset already exists. Keep
        if (args && args.markerpoints) var mset = args;
        else var mset = new KMarkerset(args || {});
...
*/
        ...
        if (mset.type == "2diameter") {
            ...
            mset.addpoint(math.add(p, [-3 * s, +3 * s, 0, 0])._data, undefined, undefined, {
                size: s,
                color: 2,
                name: "source",
            });
	...
	*******************************************************************************/
    that.delSet = function (key) {
        if (that.markersets[key] == that.currentSet) that.currentSet = undefined;

        if (that.markersets[key] == undefined) return;
	...
	*******************************************************************************/
    that.appendSet = function (
        mset,
        makecurrent //  name, type, uuid, options)
    ) {
        that.markersets[mset.uuid] = mset;
        that.markersetIDs = Object.getOwnPropertyNames(that.markersets);
        if (makecurrent) that.currentSet = mset;
        return mset;
    };
	...
	*******************************************************************************/
    that.getSets = function () {
        ...
        var p = [];
        for (var k = 0; k < keys.length; k++) p.push(that.markersets[keys[k]]);
        return p;
    };
    /******************************************************************************
	 get a set by name
	*******************************************************************************/
    that.getSetByName = function (name) {
        for (var k in that.markersets) if (that.markersets[k].name == name) return that.markersets[k];
    };

    that.getSetByType = function (type) {
        var keys = [];
        for (var k in that.markersets) {
            if (that.markersets[k].type == type) keys.push(k);
        }
        return keys;
    };
 ...
 */
        if (
            ...
            (ev.button != 0) | !($(ev.target).hasClass("KViewPort_canvas") | $(ev.target).hasClass("markerpoint"))
        )
            return;
        ...
        if (medviewer == undefined) {
            ...
            KViewer.iterateMedViewers(function (mv) {
                if (mv.currentFileinfo) tlist.push(mv);
            });
            if (tlist.length != 1)
                //alertify.error("could not find a referenced file image");
                var dummy = 1;
            else tviewer = tlist[0];
        }

        if (tviewer && tviewer.currentFileinfo) {
            if (tviewer.currentFileinfo.SubFolder !== "")
                point.referencedImageFilename =
                    tviewer.currentFileinfo.SubFolder + "/" + tviewer.currentFileinfo.Filename;
            else point.referencedImageFilename = tviewer.currentFileinfo.Filename;

            point.referencedImageFileID = tviewer.currentFileID;
        }

        var colors = KColorSelectorSimple("getcolors");
        if (mset.state.cyclecolors) point.setcolor(colors[runnigPointID % colors.length]);
        else point.setcolor(colors[0]);

        point.setsize(mset.state.defaultradius);
        point.sethover({ locked: mset.state.locked, hoverdetails: mset.state.hoverdetails });

        if (mset.markerPanel) mset.markerPanel.update();

        if (KViewer.markerTool.isinstance) KViewer.markerTool.update();
	...
	*******************************************************************************/
    that.setCurrentSet = function (mset, hideOtherPanels, dontupdate) {
        ...
        if (typeof mset !== "object") {
            if (typeof mset == "number") mset = markersets[Object.keys(markersets)[mset]];
            else mset = markersets[mset];
        }
        ...
        if (hideOtherPanels) {
            for (var k in markersets) {
                ...
                if (cset != mset) {
                    cset.visible = false;
                    if (cset.markerPanel) cset.markerPanel.$container.hide();
                    cset.clearAllPoints();
                }
            }
        }
        ...
        if (mset.markerPanel) {
            if (mset.markerPanel.panelvisible) mset.markerPanel.show();
    ...
    that.getMarkersetsByName = function () {
        var sets = {};
        for (var k in that.markersets) sets[that.markersets[k].name] = that.markersets[k];
        return sets;
    };
    ...
    that.hideAll = function () {
        for (var k in markersets) {
            cset = markersets[k];
            if (cset != markerProxy.currentSet) cset.visible = false;
            cset.clearAllPoints();
        }
    };
	...
	*******************************************************************************/
    ...
    signalhandler.attach("canvasLayoutChanged", function () {
        for (var k in markersets) markersets[k].drawAllPoints();
    });
...
KPointRoiTool = function () {
    return markerProxy.newSet({
        name: "pointROI",
        type: "pointROI",
        showPanel: 1,
        state: { delSetOnPanelClose: 1, showThroughSlice: false, hoverdetails: false },
    });
};
...
KMarkerPanel_lines = function () {
    ...
    return markerProxy.newSet({
        ...
        showPanel: 1,
        templates: templates,
        state: { hoverdetails: false, createonclick: true, delSetOnPanelClose: true, showOnAllImages: 1 },
    });
};
    ...
    var mset = markerProxy.newSet({
        ...
        optionalFunction: {
            ...
            operation: function (mset) {
                ...
                } else alertify.error("Select some ROI imprint poylgon");
            },
        },
        state: {
            hoverdetails: false,
...
`;
    ...
    var $middlebar = $("<div class='' style='margin:10px;line-height:20px;'></div>")
...
templates: 				predefined markers (for dragdrop usage)`;
    ...
    var $preview = $(
        "<div class='modernbutton small green' style='position:absolute;top:0;right:10px'> preview <i class='fa fa-arrow-right'></i></div>"
    );
    ...
    var $textarea = $(
        "<textarea readonly autocorrect='off' autocapitalize='off' spellcheck='false' style='height:auto'></textarea>"
    )
        .appendTo($xbar)
        .val(docutxt);
...
KMarkerPanel = function (markerset_in) {
    ...
    if (markerset_in != undefined) var panel_id = markerset_in.uuid;
    else markerset_in = markerProxy.newSet();
	...
	*******************************************************************************/
    ...
    that.close = function () {
        ...
        if (markerset.state.delSetOnPanelClose) {
            if (markerset.modified)
                alertify.confirm(
                    "This markerset was modfied, but not saved.<br> Are you sure to close it without saving? ",
                    function (a) {
                        if (a) markerProxy.delSet(markerset.uuid);
                    }
                );
            else {
    ...
    that.deletePanel = function () {
        ...
        if (that.freeView) that.freeView.kill();
    ...
    that.toggle = function (force) {
        ...
        if (that.panelvisible) that.show();
        else that.hide();
    };
	...
	*******************************************************************************/
    ...
    var $container = $(
        "<div id='markerPanel" + panel_id + "' class='markerPanel movableWindows panel_floatable__ panel__' ></div>"
    );
    $container.click(function (ev) {
        if (ev.originalEvent && $(ev.originalEvent.target).attr("contentEditable")) return;
        if (markerProxy.currentSet != markerset) markerProxy.setCurrentSet(markerset);
    });
	...
	*******************************************************************************/
    ...
    var $newMarker = $(
        "<a class='KViewPort_tool' draggable=true><i class='fa fa-plus' ></i><span> New Marker </span></a>"
    )
        .appendTooltip("createnNewMarker")
        .click(function () {
            if (that.state.showOnAllImages == 0) {
                alertify.alert(
                    "'showOnAllImages' is disabled. Enable, or drag this button into a viewer to create marker."
                );
            } else markerProxy.createMarker(undefined, markerset);
        });

    // allow to create marker on drag / drop
    $newMarker[0].ondragstart = dragstarter(
        (function (x, name) {
            return function () {
                return {
                    type: "markertemplate",
    ...
    var $saveMarkers = (that.$saveMarkers = $(
        "<a class='KViewPort_tool'><i class='fa fa-save'></i><span> Save </span></a>"
    )
        .appendTooltip("savemarkerset")
        .click(function () {
            ...
            else markerProxy.save(undefined, markerset);
        }));
	...
	*******************************************************************************/
    function switch_enabled(prop, force, invert) {
        ...
        if (that.markerPanel) btns["$" + prop][togglefcn]("KViewPort_tool_enabled");
	...
	*******************************************************************************/
    ...
    btns.$locked = $("<i class='KViewPort_tool fa fa-lock'></i>")
        .appendTooltip("lockmarkers")
        .click(function () {
            markerset.toggleStateVar("locked");
        })
        .appendTo($toolsRow);
    btns.$createonclick = $("<i class='KViewPort_tool fa fa-pencil'></i>")
        .appendTooltip("markercreateonclick")
        .click(function () {
            markerset.toggleStateVar("createonclick");
        })
        .appendTo($toolsRow);
    ...
    btns.$cyclecolors = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-recycle'></i>")
        .appendTooltip("cyclecolors")
        .click(function () {
            markerset.toggleStateVar("cyclecolors");
        })
        .appendTo($toolsRow);
    btns.$hoverdetails = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-info'></i>")
        .click(function () {
            markerset.toggleStateVar("hoverdetails");
        })
        .appendTooltip("markerdetailsonhover")
        .appendTo($toolsRow);
    btns.$showOnAllImages = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-adn'></i>")
        .click(function () {
            markerset.toggleStateVar("showOnAllImages");
        })
        .appendTooltip("markershowOnAllImages")
        .appendTo($toolsRow);
    btns.$showThroughSlice = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-lightbulb-o'></i>")
        .click(function () {
            markerProxy.currentSet.toggleStateVar("showThroughSlice");
            markerProxy.currentSet.drawAllPoints();
        })
        .appendTooltip("showThroughSlice")
        .appendTo($toolsRow);
    btns.$visible = $("<i class='KViewPort_tool KViewPort_tool_enabled fa fa-eye'></i>")
        .click(function () {
            markerset.togglePointsVisibility();
        })
        .appendTooltip("showhideallmarkers")
        .appendTo($toolsRow);

    $toolsRow.append($("<i class='flexspacer'></i>"));
    btns.$defaultradius = $(" <input type = 'text' min='0' max='100' value='" + markerset.state.defaultradius + "' /> ")
        .on("change", changedefaultsize)
        .appendTo($toolsRow);
    function changedefaultsize(ev) {
        markerset.toggleStateVar("defaultradius", parseFloat(btns.$defaultradius.val()));
    }

    KMouseSlider(btns.$defaultradius, { min: 0, incrementPerPixel: 0.2 });
	...
	*******************************************************************************/
    ...
    that.pointROITool.$thresh = $(
        "<input type = 'text' min='0' step=1 value='" + markerset.state.pointROIthresh + "' /> "
    )
    ...
    KMouseSlider(that.pointROITool.$thresh, {
        min: -Infinity,
        incrementPerPixel: 2,
        updateonmove: 0,
        updateonrelease: 1,
    ...
    that.pointROITool.$thresBtn = $("<i class='RoiPen KViewPort_tool fa fa-arrow-circle-up'></i>")
        .appendTo($pointROIRow)
        .click(function (ev) {
    ...
    that.pointROITool.$regionGrowRestrict = $(
        "<i class='RoiPen KViewPort_tool KViewPort_tool_enabled fa fa-fw fa-map-marker'></i>"
    )
        .click(function (ev) {
            selectPen("regionGrowRestrict", "toggle");
        })
    ...
    function selectPen(pen, toggle) {
        if (pen == undefined) pen = "default";

        if ((toggle != undefined) & (pen == that.pointROITool.pen)) pen = "default";
    ...
    var $pointROI = $("<i class='fa KViewPort_tool'>3D</i>")
        .appendTooltip("showpointroias3D")
        .appendTo($pointROIRow)
        .click(function () {
            if (that.freeView) {
	...
	*******************************************************************************/
    ...
    that.scribbleTool.$lps = $(
        "<input type = 'text' min='1' step=1 value='" + that.scribbleTool.lengthPerSegment + "' /> "
    )
        .on("change", function (ev) {
            that.scribbleTool.lengthPerSegment = parseFloat(that.scribbleTool.$lps.val());
        })
        .appendTo($scribbleRow);
    KMouseSlider(that.scribbleTool.$lps, {
        min: 0,
        incrementPerPixel: 0.1,
        updateonmove: 0,
        updateonrelease: 1,
        logScaling: 10,
    });
    ...
    if (that.markerset.type == "scribble") {
        ...
        var $dummy = $("<i class='KViewPort_tool fa fa-fw fa-trash'></i>")
            .click(markerset.deleteAllPoints)
            .appendTooltip("delete all points")
            .appendTo($scribbleRow);
        var $dummy = $("<i class='KViewPort_tool fa fa-fw fa-clone'></i>")
            .click(markerset.interplate_scribbles)
            .appendTooltip("interpolate")
            .appendTo($scribbleRow);
        var $dummy = $("<i class='KViewPort_tool fa fa-fw fa-pencil'></i>")
            .click(markerset.map_to_ROI)
            .appendTooltip("mapToActiveROI")
            .appendTo($scribbleRow);
		...
		******************************************************************/
        ...
        function toggleModifyScribble(force) {
            ...
            function mousedown(ev) {
                isMouseDown = 1;
                $mmt.on("mouseleave mouseup", function () {
                    isMouseDown = 0;
                    $mmt.off("mouseleave mouseup");
                });
                mousemove(ev);
                ev.stopPropagation();
                return false;
            ...
            var mousemove = moveUnlagger(function mousemove(ev) {
				...
				*/
                ...
                if (isMouseDown) KMarkerScribble_modify_with_pen(ev, medViewer, r / 2);
            ...
            if (spen_enabled) {
                var $spen = $("<div class='scribble_pencil roiTool_pencil'></div>")
        ...
        KMarkerScribble_modify_with_pen = function (ev, medViewer, r) {
			...
			*/
            ...
            for (var k = 0; k < points.length; k++) {
                ...
                if (kmath.abs(voxoords[medViewer.getSlicingDimOfArray] - medViewer.getCurrentSlice()) > 0.1) continue;

                for (
                    var s = 0;
                    s < point.subpoints.length;
                    s++ // exclude last point? no.
                ) {
                ...
                for (var x = 0; x < point.subpoints.length; x++) {
                    ...
                    if (to_rem.indexOf(x) == -1) newpoints.push(pp);
                }

                point.subpoints = newpoints;
                if (newpoints.length > 0) point.coords = point.subpoints[0];
                else point.deletepoint();
            }
            markerset.drawAllPoints();
        };
	...
	*******************************************************************************/
    ...
    var $templatesButton = $("<div class='roiTool_panel_flex' style='align-items:center;'></div>")
        .appendTo($container)
        .hide();
    ...
    that.setTemplates = function (items) {
        ...
        for (var k in items) {
            ...
            if (items[k].color != undefined) $temp.css("background", items[k].color);

            items[k].name = k;

            $temp[0].ondragstart = dragstarter(
                (function (x, name) {
                    return function () {
                        return {
                            type: "markertemplate",
    ...
    function dropCallback(ev, params, viewport) {
        ...
        if (v && v.viewerType == "medViewer") {
            if (v.isGLenabled()) ev.coords = v.gl.getRealWorldCoordinateFromEvent(ev)._data;
            else ev.coords = v.getRealWorldCoordinatesFromMouseEvent(ev.clientX, ev.clientY)._data;
    ...
    that.template_setpros = function (obj, point) {
        if (obj.name != undefined) point.p.name = obj.name;
        ...
        if (obj.radius != undefined) point.setsize(obj.radius);
    };
    ...
    that.autocreate_templates = function (ev) {
        for (var k in that.templates) {
            ...
            if (obj.viewport != undefined) {
                ...
                if (v && v.viewerType == "medViewer") medViewer = v;
            }
    ...
    that.update = function () {
        ...
        if (markerset != undefined) {
            ...
            for (var k = 0; k < pts.length; k++) {
                var point = pts[k];
                if (point.type == "pointROI") thingstoshow.pointROI = 1;
                else thingstoshow.pointROI = 0;
	...
	*******************************************************************************/
    function setState(state_new) {
        if (markerset.type == "pointROI") $pointROIRow.show();
        else $pointROIRow.hide();
...
var electrodes = {
    Medtronic3387: {
        ...
        approx_maedler: [0.2991, -1.2654, 0.000341, 0.2784, 0.001217, -6.49e-7],
    },
    Medtronic3389: {
...
function KMarkerset(args) {
    ...
    that.deletePanel = function () {
        if (that.markerPanel) that.markerPanel.deletePanel();
    };

    var defaultradius = parseFloat(ViewerSettings.defaultsizemarker);
    if (defaultradius == undefined) defaultradius = 5;
	...
	*******************************************************************************/
    that.setType = function (type) {
        ...
        for (var x in that.onupdate) that.onupdate[x]();

        //that.cleanFiberAssoc();
        if (that.type == "electrode") markerProxy.updateElectrode(that, that.elecmodel);
        that.pointChanged();

        if (that.type == "pointROI") $.extend(true, state, { hoverdetails: false });
        ...
        if (that.updateLine_sid != undefined) signalhandler.detach("positionChange", that.updateLine_sid);
        if (
            (that.type == "electrode") |
            (that.type == "freeline") |
            (that.type == "2diameter") |
	...
	*******************************************************************************/
    that.toggleStateVar = function (x, force, donotsethover) {
        if (x == "locked") state[x] = force == undefined ? (state[x] + 1) % 3 : force;
        else state[x] = force == undefined ? (state[x] ? false : true) : force;

        var togglefcn = state[x] ? "addClass" : "removeClass";
        var btns = [];
        if (KViewer.markerTool && KViewer.markerTool.isinstance) btns.push(KViewer.markerTool.btns);
        if (that.markerPanel) btns.push(that.markerPanel.btns);

        for (var k = 0; k < btns.length; k++) {
            var btn = btns[k];
            if (btn["$" + x]) {
                if (btn["$" + x].is("input")) btn["$" + x].val(state[x]);
                else {
                    btn["$" + x].removeClass("KViewPort_tool_enabled").removeClass("KViewPort_tool_enabled_alt");
                    if (state[x] > 0) {
                        if (state[x] == 1) btn["$" + x].addClass("KViewPort_tool_enabled");
                        if (state[x] == 2) btn["$" + x].addClass("KViewPort_tool_enabled_alt");
                    }
                }
            }
        }
        ...
        if (x == "locked" && state.locked) $(".markerpoint_trash").hide();
        if (x == "locked" && !state.locked) $(".markerpoint_trash").show();
    ...
    function toggleSomeToMain() {
        KViewer.iterateMedViewers(function (m) {
            if (KViewer.mainViewport == -1) KViewer.toggleMainViewport(m.viewport.viewPortID);
        });

        if (KViewer.mainViewport == -1) KViewer.navigationTool.worldMaster();
    }
    ...
    that.Rectifytransform = function () {
        ...
        if (that.type == "electrode") {
            ...
            var T = math.matrix([
                [1, 0, 0, origin._data[0]],
                [0, 1, 0, origin._data[1]],
                [0, 0, 1, origin._data[2]],
                [0, 0, 0, 1],
            ]);
            pcenter = p1;
            ...
            var rotmat;
            if (w._data[i] > 0) rotmat = calcRotmatForVectors(iedges[i], n._data);
            else rotmat = calcRotmatForVectors(n._data, iedges[i]);
        }
        ...
        if (that.type == "2diameter") {
			...
			*/
            ...
            for (var k = 0; k < 6; k++) {
                var p = perms[k];
                var c = 0;
                for (var j = 0; j < 3; j++) c += math.abs(math.dot(sys[p[j]], iedgesT[j]));
                if (mc < c) {
                    mc = c;
                    mi = k;
                }
            ...
            sys = [
                math.multiply(sys[perms[mi][0]], math.sign(math.dot(sys[perms[mi][0]], iedgesT[0])))._data,
            ...
            pcenter = [0, 0, 0, 1];
            for (var k = 0; k < points.length; k++) pcenter = math.add(pcenter, points[k].coords, 1 / points.length);
            pcenter._data[3] = 1;
            var T = math.matrix([
                [1, 0, 0, pcenter._data[0]],
                [0, 1, 0, pcenter._data[1]],
                [0, 0, 1, pcenter._data[2]],
                [0, 0, 0, 1],
            ]);
        }
        ...
        if (KViewer.mainViewport == -1) KViewer.currentPoint = math.multiply(math.inv(E), math.matrix(pcenter));
        else KViewer.currentPoint = math.matrix(pcenter);

        KViewer.reorientationMatrix.notID = true;
        KViewer.reorientationMatrix.matrix = E;

        if (KViewer.navigationMode == 0) KViewer.navigationTool.switchToNavimode(2);
        if (KViewer.mainViewport == -1) toggleSomeToMain();
    ...
    that.RECTtransform = function () {
        var points = that.getPoints();
        if (points.length !== 3) {
            alertify.alert("Must select 2 or 3 points");
            return;
        }
    ...
    that.MCPtransform = function () {
        ...
        if (KViewer.mainViewPort == -1) {
            alertify.alert("To see the ACPC Line horizontally, go into master mode.");
        }
        ...
        // 2 point mode
        if (points.length == 2) {
            if (points[1].coords[1] > points[0].coords[1]) {
                var p0 = points[0].coords;
                var p1 = points[1].coords;
            } else {
                var p0 = points[1].coords;
                var p1 = points[0].coords;
            }
            ...
            order.splice(gz, 1);
            // greatest X --> AP
            if (points[order[1]].coords[1] > points[order[0]].coords[1]) {
                var p0 = points[order[0]].coords;
                var p1 = points[order[1]].coords;
            } else {
                var p0 = points[order[1]].coords;
                var p1 = points[order[0]].coords;
            }
            ...
            var newY = math.cross(diffZ, diffX);
            newY = math.multiply(newY, 1 / math.norm(newY._data));
            var newZ = math.cross(newY, newX);
            newZ = math.multiply(newZ, 1 / math.norm(newZ._data));
            newX = newX._data;
            newY = newY._data;
            newZ = newZ._data;
            newX.push(0);
        ...
        if (origin._data) origin = origin._data;
    ...
    that.objectify = function () {
        ...
        for (var k = 0; k < pts.length; k++) {
            ...
            if (1) {
                ...
                if (point.referencedImageFileID !== undefined) {
                    ...
                    if (nii != undefined) {
                        nii = nii.content;
                        if (nii.invedges == undefined) nii.invedges = math.inv(nii.edges);
        ...
        if (that.markerPanel && that.markerPanel.panelvisible) ret.state.panel_visible = true;
        else ret.state.panel_visible = false;
    ...
    that.getPointsByName = function () {
        ...
        var p = {};
        for (var k = 0; k < keys.length; k++) p[that.markerpoints[keys[k]].p.name] = that.markerpoints[keys[k]];
        return p;
    };
    ...
    that.getPoints = function () {
        var keys = Object.keys(that.markerpoints).sort();
        var p = [];
        for (var k = 0; k < keys.length; k++) p.push(that.markerpoints[keys[k]]);
        return p;
    };
    ...
    that.insertpoint = function (coords, type, after, params) {
        ...
        var i = 8;
        while (that.markerpoints[id] != undefined) id = after.uuid + "_" + i--;

        return that.addpoint(coords, type, id, params);
    };

    that.addpoint = function (coords, type, id, params) {
        if (type == undefined) type = that.type;

        if (id == undefined) id = that.uuid + "P" + ("00000" + that.runningID++).slice(-5);
        ...
        if (type == "ruler") {
            ...
            x.getLength = function () {
                return Math.sqrt(this.dir[0] * this.dir[0] + this.dir[1] * this.dir[1]);
            };
        ...
        if (that.type == "pointROI") {
            ...
            if (coords == undefined) x.roinii.setThisPointInto3DView();
        ...
        if (x.shape && x.shape == "box") x.p.size = [x.p.size, x.p.size, x.p.size, 0]; // box has 3D size

        if (params) {
            if (params.name) x.p.name = params.name;
            if (params.color) x.p.color = new KColor(KColor.list[params.color]);
            if (params.size) x.p.size = params.size;
            if (params.master) x.master = true;
            if (params.hideMarkerRepresentation) x.hideMarkerRepresentation = 1;
        }

        if (coords) x.movepoint([coords[0], coords[1], coords[2], 1]);
        else {
            {
                if (KViewer.currentPoint) x.movepoint(KViewer.currentPoint._data);
            }
        }

        if (type == "ruler") {
            // || type =='rectangles')
            if (x.$markers[Object.keys(x.$markers)[0]] == undefined) return that.markerpoints[id];
    ...
    that.drawAllPoints = function () {
        if (KViewer.markerTool && that.state.visible && that.visible) {
            //|| KViewer.markerTool.enabled != 0))
            iteratePoints(function (x) {
                x.drawpoint();
            });
            that.drawLine();
        }
    ...
    that.togglePointsVisibility = function (state) {
        if (state == undefined) state = !that.state.visible;

        that.toggleStateVar("visible", state);

        iteratePoints(function (x) {
            x.togglepoint(state);
            if (!state) that.disposeLine();
            else that.drawLine();
        });
        ...
        KViewer.iterateMedViewers(function (medViewer) {
            ...
            if (
                ...
                if (that.color != undefined) {
                    if ((that.color.length == 3) | (that.color.length == 4)) col = new KColor(that.color);
                    else col = colors[that.color];
                }

                var lid = "LINE_" + viewport.viewPortID + "_" + that.uuid;
                if (medViewer.gl != undefined && medViewer.isGLenabled()) {
                    if (that.glmesh[lid] != undefined) that.glmesh[lid].dispose();
                    if (that.type == "electrode") {
                        that.glmesh[lid] = medViewer.gl.createTrace(that, {
                            width: 1,
                            color: new KColor([col.color[0] / 10, col.color[1] / 10, col.color[2] / 5]),
                        });
                    }
                    if ((that.type == "freeline") | (that.type == "correspondence")) {
                        var wid = 1;
                        if (KViewer.defaultFOV_mm != "") wid = KViewer.defaultFOV_mm / 100;
                    ...
                    if (that.mesh2d[lid] != undefined) that.mesh2d[lid].remove();
                    if (
                        (that.type == "electrode") |
                            (that.type == "freeline") |
                            (that.type == "2diameter") |
                            (that.type == "correspondence") &&
                        medViewer.nii != undefined
                    )
    ...
    that.computeInfo = function () {
        if (that.type == "pointset") {
            ...
            for (var i = 0; i < 3; i++) mean[i] /= N;
            for (var i = 0; i < 6; i++) sq[i] /= N;

            var EV = kmath.maxEV3(
                math.matrix([
                    [sq[0] - mean[0] * mean[0], sq[3] - mean[0] * mean[1], sq[4] - mean[0] * mean[2]],
            ...
            for (var k = 0; k < points.length - 1; k++) {
                var n = [
                    points[k].p.coords[0] - points[k + 1].p.coords[0],
                ...
                if (lastdir != undefined)
                    ang +=
                        180 -
                        Math.round(
                            (Math.acos(lastdir[0] * n[0] + lastdir[1] * n[1] + lastdir[2] * n[2]) / Math.PI) * 180
                        ) +
                        ";";
                lastdir = n;
            }
            if (that.setInfo) that.setInfo("length: " + dist.toFixed(1) + " mm  degs:" + ang);
        } else if (that.type == "2diameter") {
            var points = that.getPoints();
            if (points.length == 4) {
                ...
                var len = 0;
                if (that.circumference) len = that.circumference;
            ...
            if (that.setInfo) that.setInfo("area: " + (that.surfacearea / 100).toFixed(4) + " cm^2");
        }
    };
    ...
    that.pointChanged = function (thepoint) {
        ...
        if (that.type == "electrode" && that.electrode_properties != undefined) {
            if (that.electrode_properties.deformed | that.electrode_properties.custom);
                ...
                if (keys.length >= 2) {
                    ...
                    if (that.electrode_properties.directional & (keys.length >= 3)) {
                        ...
                        kmath.normalize(d2);
                        var f = 5;
                        that.markerpoints[keys[2]].p.coords = [
                            p2[0] + f * d2[0],
                            p2[1] + f * d2[1],
                            p2[2] + f * d2[2],
                            1,
                    ...
                    for (var k = 2 + offs; k < keys.length; k++) {
                        ...
                        that.markerpoints[keys[k]].p.coords = [
                            p2[0] + n[0] * dist,
                            p2[1] + n[1] * dist,
                            p2[2] + n[2] * dist,
                            1,
            ...
            var funs = [];
            for (var k = 0; k < points.length; k++) {
                for (j in points[k].onupdate) funs.push(points[k].onupdate[j]);
            }

            iterateSync2(0, funs.length, function (j, ondone) {
                funs[j](ondone);
            });
            ...
            if (keys.length == 4) {
                ...
                if (isNaN(normal._data[0])) return;
    ...
    that.setActive = function () {
        ...
        for (var j = 0; j < ps.length; j++) if (ps[j].active) ps[j].select();
    };

    that.getActive = function () {
        if (that.type != "electrode") return;
    ...
    that.map_to_ROI = function () {
        ...
        setTimeout(function () {
            ...
            if (roi == undefined) {
                ...
                if (roi != undefined) roi = KViewer.dataManager.getFile(roi);
            }
            if (roi == undefined) {
            ...
            if (points.length > 0) {
                ...
                var poly = [];
                for (var k = 0; k < points.length; k++) poly.push({ subpoints: points[k].subpoints });
	...
	*/
    ...
    if (args.delSetOnPanelClose != undefined) state.delSetOnPanelClose = args.delSetOnPanelClose;
    if (args.showThroughSlice != undefined) state.showThroughSlice = args.showThroughSlice;
...
function iterateSync2(current, end, fun) {
    if (current < end)
        fun(current, function () {
            iterateSync2(current + 1, end, fun);
        });
}
...
function KMarkerpoint(parentmarkerset, id) {
    ...
    if (that.shape == "sphere") that.p.size = 5;
    // size in mm. single number for spheres, 3 numbers for box
    else if (that.shape == "box") that.p.size = [5, 5, 5, 0]; // size in mm. single number for spheres, 3 numbers for box

    that.__defineGetter__("uuid", function () {
        return this.p.uuid;
    });
    that.__defineGetter__("coords", function () {
        return this.p.coords;
    });
    that.__defineGetter__("size", function () {
        return this.p.size;
    });
    ...
    that.movepoint = function (coords, medViewer, ev, moveset) {
        if (moveset) {
            ...
            for (var k = 0; k < ps.length; k++) {
                for (var j = 0; j < 3; j++) ps[k].p.coords[j] += dif._data[j];
                ps[k].drawpoint();
            }
        } else {
        ...
        if (that.parentmarkerset.pointChanged)
            if (!that.parentmarkerset.pointChanged(that)) {
                for (var x in that.onupdate) that.onupdate[x](medViewer, ev);
            }
    ...
    that.incsize = function (amount, medViewer) {
        if (that.shape == "sphere") var newsize = that.p.size + amount;
        else if (that.shape == "box") {
            var newsize = [0, 0, 0, 0];
            newsize[0] = that.p.size[0] + amount;
            newsize[1] = that.p.size[1] + amount;
            newsize[2] = that.p.size[2] + amount;
        }
    ...
    that.getsubpoint = function (index, t) {
        if (index < 0) index = that.subpoints.length + index;
    ...
    that.setsizeFromMouseEvent = function (edgeindex, refpoint, newpoint, medViewer, donotrecalcflag) {
        if (that.parentmarkerset.state.fixedsize) return;
...
*/
            ...
            if (1 || that.type == "3druler") var posshift = math.multiply(rw_diff, -0.5);
            else var posshift = math.multiply(rw_diff, 0);

            var newpos = math.add(refpos, posshift);

            // check if lower right or upper left resizer
            if (edgeindex == 0) var dir = 0.5;
            else if (edgeindex == 1) var dir = -0.5;
        ...
        if (that.shape == "scribble") {
            ...
            if (tlen == 0) {
                ...
                if (!that.isclosed) var lastpoint = that.getsubpoint(tlen - 1);
                // .length ist costly, save in future!
                else var lastpoint = that.getsubpoint(that.lastInsertedPointInd); // .length ist costly, save in future!
                    ...
                    if (that.isclosed) {
						...
						*/
                        ...
                        if (that.movedirection == 0) {
                            ...
                            var dLastL = math.norm(
                                kp_vsum(
                                    that.getsubpoint(that.lastInsertedPointInd - 1),
                                    that.subpoints[that.lastInsertedPointInd],
                                    -1
                                )
                            );
                            var dLastR = math.norm(
                                kp_vsum(
                                    that.getsubpoint(that.lastInsertedPointInd + 1),
                                    that.subpoints[that.lastInsertedPointInd],
                                    -1
                            ...
                            var dNewL = math.norm(
                                kp_vsum(that.getsubpoint(that.lastInsertedPointInd - 1), newcoords, -1)
                            );
                            var dNewR = math.norm(
                                kp_vsum(that.getsubpoint(that.lastInsertedPointInd + 1), newcoords, -1)
                            );
                            ...
                            if (dLastL - dNewL > dLastR - dNewR) that.movedirection = -1;
                            else that.movedirection = 1;
                        }
                        // insert new point
    ...
    that.setcolor = function (color, alpha) {
        ...
        } else if (color != undefined) that.p.color = color;

        if (alpha != undefined) that.p.color.color[3] = alpha;
        ...
        for (var mid in $markers) {
            ...
            if (that.shape == "box") $markers[mid].find("line").css("stroke", css);
            ...
            if (that.shape == "scribble") {
                ...
                $markers[mid]
                    .find("polyline")
                    .css("stroke", "rgba(" + that.p.color.color.slice(0, 3).join(",") + "," + opac + ")");
            }
        }
        that.drawpoint();
    ...
    that.togglepoint = function (state) {
        if (state !== undefined) that.visible = state;
        else that.visible = that.visible ? false : true;

        if (that.visible) that.drawpoint();
        else that.clearpoint();
    ...
    that.select = function (idx) {
        ...
        if (idx == undefined) {
            ...
            if (that.active[idx] == undefined) that.active = [0, 0, 0];
            that.active[idx] = ((that.active[idx] + 2) % 3) - 1;
        }
        that.runcallbacks("active");

        if (parentmarkerset.simpanel) parentmarkerset.simpanel.updateActive();

        for (var x in parentmarkerset.onupdate) parentmarkerset.onupdate[x]();
        for (var x in that.onupdate) that.onupdate[x]();
    };
    ...
    that.drawpoint = function () {
        ...
        function draw() {
            if (KViewer.viewports == undefined) return;
            if (that.parentmarkerset.visible == false) return;
            ...
            KViewer.iterateMedViewers(function (medViewer) {
                if (
                    (medViewer.nii !== undefined &&
                        ...
                        that.visible &&
                        (KViewer.markerTool.enabled != 0 ||
                            (that.parentmarkerset.markerPanel && that.parentmarkerset.markerPanel.panelvisible))) ||
                    that.type == "ruler" ||
    ...
    that.changeName = function ($e) {
        that.p.name = $e.html();
        if (that.p.name.replace(/\s*[<br\s*/>]/g, "") == "") that.p.name = "noname";
    ...
    function drawpoint_2D(medViewer, mid) {
        var x = that.p.coords;
        if (KViewer.navigationMode == 0) x = math.multiply(KViewer.reorientationMatrix.matrix, x)._data;
        ...
        if ($markers[mid] === undefined) {
            ...
            if ((that.type == "ruler") | (that.type == "rectangles")) {
                if (that.type == "ruler") {
                    ...
                    $a.$text = $(
                        "<div class='markerruler_text' > <span></span> <i class='fa fa-trash'></i></div>"
                    ).appendTo($a);
                    ...
                    $a.$term_a = $(
                        "<div class='markerruler_terminal markerruler_terminal_a rect_term' ></div>"
                    ).appendTo($a);
                    $a.$term_b = $(
                        "<div class='markerruler_terminal markerruler_terminal_b rect_term' ></div>"
                    ).appendTo($a);
                    $a.$rect = $("<div class='markerrectangle_rectborder' ></div>").appendTo($a);
                }

                attachRulerHandlers($a);
            } // the normal generic class
            else {
                if (that.shape == "sphere") {
                    $a.addClass("markerpoint");
                    $a.append(
                        $(
                            "<div class='markerpoint_haircross_x' style='background:" +
                                that.p.color.getCSS() +
                                "'></div>"
                        )
                    );
                    $a.append(
                        $(
                            "<div class='markerpoint_haircross_y' style='background:" +
                                that.p.color.getCSS() +
                                "; '></div>"
                        )
                    );
                    ...
                    if (that.master && that.parentmarkerset.showTrash) {
                        if (that.parentmarkerset.showTrash)
                            $a.append(
                                $("<span><i class='markerpoint_trash fa fa-trash'></i></span>").click(function () {
                    ...
                    if (that.parentmarkerset.optionalFunction) {
                        $a.append(
                            $(
                                "<span><i class='markerpoint_trash fa " +
                                    that.parentmarkerset.optionalFunction.icon +
                                    "'></i></span>"
                            ).click(function () {
                        ...
                        attachMouseSlider($a.$resizerZ, {
                            ...
                            mouseup: function () {
                                ...
                                if (that.parentmarkerset.markerPanel) {
                                    var dummy = that.parentmarkerset.markerPanel.$markerListTable.children(
                                        '[marker_uuid="' + that.uuid + '"]'
                                    );
                ...
                if (that.type == "3druler") {
                    ...
                    $a.$resizerA = $("<div class='markerpoint_resizer markerpoint_3Druler_resizer'></div>").appendTo(
                        $a
                    );
                    $a.$resizerB = $(
                        "<div class='markerpoint_resizer markerpoint_3Druler_resizer markerpoint_resizer_other'></div>"
                    ).appendTo($a);

                    $a.$rulertext = $("<div class='markerpoint_3Druler_text'></div>").appendTo($a);
                    $a.$rulertextInner = $("<span class=''></span>").appendTo($a.$rulertext);
                    $a.$delete = $("<span><i class='markerpoint_trash fa fa-trash'></i></span>")
                        .appendTo($a.$rulertext)
                        .click(function () {
                            that.deletepoint();
                        });
                    if (that.parentmarkerset.state.locked) $a.$delete.hide();
                    ...
                    $a.$rulersvg = $(
                        "<svg version='1.1' viewBox='-1 -1  1 1' preserveAspectRatio='none'><line x1='-1' x2='1' y1='-1' y2='1' style=''></svg>"
                    ).appendTo($a);
                }
                ...
                if (that.type == "scribble") {
                    ...
                    $a.$closescribble = $("<span><i class='markerpoint_trash fa fa-circle-o'></i></span>")
                        .appendTo($a)
                        .click(function () {
                            that.closescribble();
                        });
                    ...
                    var gaussblur = 1;
                    var defs =
                        "<defs><filter id='svg_blur'> <feGaussianBlur stdDeviation='" +
                        gaussblur +
                        "' /> </filter> </defs>";
                    //$a.$scribblesvg = $("<svg version='1.1' viewBox='-1 -1  1 1' preserveAspectRatio='none'>" + defs + childstr + "</svg>").appendTo($a);
                    $a.$scribblesvg = $(
                        "<svg version='1.1' _viewBox='-1 -1  1 1' _preserveAspectRatio='none'>" +
                            defs +
                            childstr +
                            "</svg>"
                    ).appendTo($a);
                ...
                sethover_singleVP($a, {
                    locked: that.parentmarkerset.state.locked,
                    hoverdetails: that.parentmarkerset.state.hoverdetails,
                });
            }
        ...
        if (that.shape == "sphere") {
            ...
            var size_apparent_x =
                math.sqrt(math.pow(size_true_x, 2) - math.pow(canvascoords.z_mm - medViewer.getCurrentSliceInMM(), 2)) *
                canvascoords.x_pixPerMM *
                2;
            ...
            $markers[mid].css(
                "transform",
                "scale(" + xf + "," + yf + ") translate(" + -xf * ss + "%," + -yf * ss + "%)"
            );
            if (that.type == "3druler") {
                if ($markers[mid].$rulertext) {
                    $markers[mid].$rulertext.css(
                        "transform",
                        "scale(" + xf + "," + yf + ") translate(" + -xf * ss + "%," + -yf * 0 + "%)"
                    );
            ...
            var isInside = test < medViewer.nii.voxSize[medViewer.getSlicingDimOfArray()] * 0.3;
            size_apparent_x = 3;
            size_apparent_y = 3;
        }
        ...
        if (that.parentmarkerset.type == "scribble") {
            ...
            function cmouseover(ev) {
                ev.target.setAttribute("r", circleradius * 1.3);
            }
            function cmouseout(ev) {
            ...
            function cmousemove(ev, pindex, startpos) {
                ...
                for (var i = 0; i < tlen; i++) {
                    ...
                    dx = cc.x_pix - canvascoords.x_pix;
                    var dy = cc.y_pix - canvascoords.y_pix;
                    pstr += dx.toFixed(2) + "," + dy.toFixed(2) + " ";
                }
                // add very first point to close scribble
                ...
                if (pindex == that.subpoints.length - 1) cmousemove(ev, 0, startpos);
                ev.stopPropagation();
                return false;
            }

            function cmouseup(ev) {
        ...
        if (
            that.parentmarkerset.state.showOnAllImages == 0 &&
            that.referencedImageFilename != medViewer.getFullFilename()
        )
        ...
        if (firstcondition && isInside | that.parentmarkerset.state.showThroughSlice) {
            ...
            if ($markers[mid].$resizerA) $markers[mid].$resizerA.css("background", that.p.color.getCSS());
            if ($markers[mid].$resizerB) $markers[mid].$resizerB.css("background", that.p.color.getCSS());
            ...
            if ((that.type == "ruler") | (that.type == "rectangles")) {
                ...
                $markers[mid].css({
                    width: l * canvascoords.x_pixPerMM + "px",
                    transform: "translate(-50%,-50%) rotate(" + deg + "deg)",
                });
                if (that.type == "ruler") {
                    $markers[mid].$textspan.text(l.toFixed(1) + "mm");
                    $markers[mid].css({ background: that.p.color.getCSS(), borderColor: that.p.color.getCSS() });
                    $markers[mid].$term_a.css({
                        background: that.p.color.getCSS(),
                        borderColor: that.p.color.getCSS(),
                    });
                    $markers[mid].$term_b.css({
                        background: that.p.color.getCSS(),
                        borderColor: that.p.color.getCSS(),
                    });
                } else if (that.type == "rectangles") {
                    deg = 90 - deg;
                    $markers[mid].$rect.css({
                        width: Math.abs(canvascoords.x_pixPerMM * dir[1]) + "px",
            ...
            var siz = -2;
            var opac = 0;
            canvascoords.x_norm = -1000;
            canvascoords.y_norm = -1000;
            $markers[mid].css({
                left: canvascoords.x_norm * 100 + "%",
                top: canvascoords.y_norm * 100 + "%",
                width: siz,
                height: siz,
                borderColor: that.p.color.getCSS(),
            });
    ...
    that.createMarkerRepresentation = function (targetid, thingstoshow) {
        ...
        var $box = $("<div class='markerpointrow'></div>")
            .on("onmouseenter", point.onHoverEnter)
            .on("onmouseleave", point.onHoverLeave);
        ...
        if (fields.coords) {
            ...
            var $row2;
            if (fields.economy) $row2 = $row1;
            else $row2 = $("<div class='markerpointrow_coords'></div>").appendTo($box);
            ...
            $innercords.append(
                $("<div><input type = 'number' value='" + point.p.coords[0].toFixed(1) + "' /></div>").on(
                    "change",
                    updateInputsC(0)
                )
            );
            $innercords.append(
                $("<div><input type = 'number' value='" + point.p.coords[1].toFixed(1) + "' /></div>").on(
                    "change",
                    updateInputsC(0)
                )
            );
            $innercords.append(
                $("<div><input type = 'number' value='" + point.p.coords[2].toFixed(1) + "' /></div>").on(
                    "change",
                    updateInputsC(0)
                )
            );
            if (fields.radius) {
                if (that.shape == "sphere") {
                    ...
                    $innercords
                        .append(
                            $("<div><input type = 'number' value='" + sizestring + "' /></div>").on(
                                "change",
                                updateInputsC(3)
                            )
                        )
                        .on("keydown", function (ev) {
                            if ((ev.keyCode == 13) & ev.shiftKey) {
                                point.parentmarkerset.getPoints().map((x) => x.setsize(parseFloat(ev.target.value)));
                                point.parentmarkerset.pointChanged();
                            }
                        });
                } else if (that.shape == "box") {
                    var sizestring =
                        point.p.size[0].toFixed(1) +
                        "," +
                        point.p.size[1].toFixed(1) +
                        "," +
                        point.p.size[2].toFixed(1);
                    $innercords.append(
                        $("<div><input type = 'text' value='" + sizestring + "' /></div>").on(
                            "change",
                            updateInputsC(3)
                        )
                    );
            ...
            if (fields.coords_transformed && KViewer.reorientationMatrix.notID) {
                ...
                var $inner = $coords;
                $inner.append(
                    $("<div><input type = 'number' value='" + coords[0].toFixed(1) + "' /></div>").on(
                        "change",
                        updateInputsC(1)
                    )
                );
                $inner.append(
                    $("<div><input type = 'number' value='" + coords[1].toFixed(1) + "' /></div>").on(
                        "change",
                        updateInputsC(1)
                    )
                );
                $inner.append(
                    $("<div><input type = 'number' value='" + coords[2].toFixed(1) + "' /></div>").on(
                        "change",
                        updateInputsC(1)
                    )
                );
            ...
            if (fields.comment) {
                ...
                $inner.append(
                    $(
                        "<div><input class='pointcomment' type = 'text' placeholder='a comment' value='" +
                            point.p.comment +
                            "' /></div>"
                    ).on("change", updateInputsC(2))
                );
            }
            ...
            function updateInputs(which) {
                ...
                if (which == 0) {
                    ...
                    point.movepoint(coords);
                } else if (which == 3) point.setsize(parseFloat($inputs[3].value));
                else if (which == 2) {
            ...
            point.callbacks.move[targetid] = function () {
                if (that.shape == "sphere") var sizestring = point.p.size.toFixed(1);
                else if (that.shape == "box")
                    var sizestring =
                        point.p.size[0].toFixed(1) +
                        "," +
            ...
            if (that.parentmarkerset.type == "electrode" && that.isContact) {
                ...
                $coords.append(
                    $(
                        "<div class='DBS_simindicator'> simulation </div><input class='DBS_nosimindicator' value='" +
                            voltage +
                            "' type = 'number' placeholder='voltage' min=0 max=50 />"
                        ...
                        if (isNaN(voltage)) {
                            ...
                            function approx(r, imp) {
                                return (
                                    k[0] * 0 + k[1] * r + k[4] * r * imp + k[3] * r * r + k[2] * imp + k[5] * imp * imp
                                );
                            }
                            if (voltage > 0 && voltage < 20)
                                var radius = binsearch(
                                    function (x) {
                                        return approx(x, impedance);
                                    },
                                    Math.abs(voltage),
                                    0,
                                    20,
        ...
        if (fields.colorsel)
            var $colorselector = KColorSelectorSimple(
                $("<div class='markerpointrow_colorselector'></div>"),
                point.setcolor,
                point.p
            ).appendTo($row1);
        ...
        if (fields.active) {
            if (parentmarkerset.electrode_properties.directional) {
                ...
                for (var t = 0; t < 3; t++) {
                    var deg = 120 * t;
                    $active[t] = $("<i style='transform: rotate(" + deg + "deg)' class='fa fa-arrow-circle-o-up'></i>")
                        .appendTooltip("activepoint")
                        .click(
                            (function (t) {
                                return function () {
                                    point.select(t);
                ...
                var $active = $("<i class='fa fa-circle-o'></i>")
                    .appendTooltip("activepoint")
                    .click(function () {
                        point.select();
                        point.parentmarkerset.drawLine();
                ...
                point.callbacks.active[targetid] = function (point) {
                    ...
                    if (point.active == 1) $active.removeClass("fa-circle-o").addClass("fa-circle").css("color", "red");
                };
                point.callbacks.active[targetid](point);
            }
        }
        ...
        if (fields.toggle) {
            var $toggle = $("<i class='fa fa-eye'></i>")
                .appendTooltip("showhidemarkerpoint")
                .click(function () {
                    point.togglepoint();
                })
                .appendTo($tools);
            point.callbacks.toggle[targetid] = function (point) {
                if (point.visible) $toggle.addClass("fa-eye").removeClass("fa-eye-slash").css("color", "");
                else $toggle.removeClass("fa-eye").addClass("fa-eye-slash").css("color", "");
            };
            point.callbacks.toggle[targetid](point);
        }

        if (fields.search)
            var $search = $("<i class='fa fa-binoculars'' style='font-size:12px'></i>")
                .appendTooltip("jumptopoint")
                .mousedown(point.jumpToPoint)
                .appendTo($tools);
        if (fields.delete) {
            var $delete = $("<i class ='fa fa-trash '></i>")
                .appendTooltip("deletemarkerpoint")
                .click(function () {
                    if (that.parentmarkerset.state.locked == 0) point.deletepoint();
                    else alertify.error("you have to unlock the set to delete points");
                })
                .appendTo($tools);
        }

        if (point.type == "pointROI") {
            if (point.thresh != undefined) var thresh = point.thresh;
            else var thresh = that.parentmarkerset.state.pointROIthresh;

            var $thresh = $("<input style='display:inline' type = 'text' min='0' step=1 value='" + thresh + "' /> ")
                .appendTo($tools)
                .on("change", function (ev) {
                    point.onupdate.pointROI(undefined, undefined, parseFloat($thresh.val()));
                });
            KMouseSlider($thresh, {
                min: -Infinity,
                incrementPerPixel: 1,
    ...
    that.jumpToPoint = function (ev) {
        ...
        if (ev.button != 0) signalhandler.send("positionChange");
        else signalhandler.send("positionChange centralize");
    ...
    that.sethover = function (state) {
        if (state.locked != undefined) that.locked = state.locked;
        for (var mid in $markers) {
    ...
    function sethover_singleVP($a, state) {
        ...
        if ($a.$scribblesvg != undefined) return;

        if (that.type == "3druler") var $target = $a.$rulertextInner;
        else var $target = $a;
        ...
        if (state.locked <= 1) {
            function MouseWheelHandler(ev) {
                ...
                if (handler.speed == undefined) handler.speed = 0;

                ev = ev.originalEvent || ev;
                if (!ev.shiftKey | that.parentmarkerset.state.fixedsize) return;

                var incdir = (ev.wheelDelta || -ev.detail) > 0 ? 1 : -1;
                if (handler.cid != undefined) clearTimeout(handler.cid);
                handler.cid = setTimeout(function () {
                    handler.speed = 0;
                }, 250);

                if (math.sign(handler.speed) != incdir) handler.speed = incdir;
                else handler.speed += incdir;
            ...
            function mousedown(ev) {
                ...
                if (that.parentmarkerset.type == "pointROI" && that.roinii && that.roinii.fileObject) {
                    if (ev.which == 1) that.roinii.setThisPointInto3DView();
                }

                var startdiff = [
                    ev.clientX - $a.offset().left - $a.width() / 2 - 4,
                    ev.clientY - $a.offset().top - $a.height() / 2 - 4,
                ];

                var worlddiff = math.add(
                    math.multiply(
                        $a.medViewer.getRealWorldCoordinatesFromMouseEvent(
                            $a.offset().left + $a.width() / 2 + 4,
                            +$a.offset().top + $a.height() / 2 + 4
                        ),
                        -1
                    ),
                    that.p.coords
                ...
                if (ev.shiftKey & !that.parentmarkerset.state.fixedsize) {
                    ...
                    $(document.body).on("mouseup", function (ev2) {
                        $(document.body).off("mouseup mousemove mouseleave", mousemovehandler);
                    });
                    $(document.body).on("mouseleave", function (ev2) {
                        $(document.body).off("mouseup mousemove mouseleave", mousemovehandler);
                    });
                    return false;
                }
                ...
                if (ev.buttons == 2 && state.locked == false) {
                    if (that.parentmarkerset.type == "correspondence") markerProxy.delSet(that.parentmarkerset.uuid);
                    else that.deletepoint();
                    ev.stopPropagation();
                    return false;
                }
                //left click
                    ...
                    var mousemovehandler = moveUnlagger(function (ev2) {
                        if ($a.medViewer.nii) {
                            var p = $a.medViewer.getRealWorldCoordinatesFromMouseEvent(
                                ev2.clientX - startdiff[0],
                                ev2.clientY - startdiff[1]
                            );
            ...
            $target.on("dblclick", function (ev2) {
                if (ev2.target.nodeName == "INPUT") return;
            ...
            if ($a.$resizerA != undefined) attachResizeHandlers(that, $a, $a.$resizerA, 1);
            if ($a.$resizerB != undefined) attachResizeHandlers(that, $a, $a.$resizerB, 0);
        ...
        for (var mid in $markers) {
            ...
            if (that.parentmarkerset.state.hoverdetails) {
                ...
                var thingstoshow = {
                    ...
                    toggle: 0,
                    search: 0,
                    delete: !that.parentmarkerset.state.locked,
                    coords: 1,
                    comment: 0,
                };
                $props
                    .append(that.createMarkerRepresentation("self" + mid, thingstoshow))
                    .hide()
                    .fadeIn(100);
    ...
    that.onHoverLeave = function (e) {
        for (var mid in $markers) {
            ...
            setTimeout(function () {
                markerProxy.HoverCnt = 0;
                if ($markers[mid] != undefined) $markers[mid].css("z-index", 5);
            }, 1500);
        }
    };
    ...
    function attachRulerHandlers($a) {
        ...
        if ($a.$text) $a.$text.on("mousedown", mousedownbody);
        ...
        function mousedownbody(e) {
            ...
            function mousemove(e2) {
                ...
                that.movepoint(
                    medViewer.getRealWorldCoordinatesFromMouseEvent(
                        e2.clientX - startMouse[0] + abspos[0],
                        e2.clientY - startMouse[1] + abspos[1]
                    )
                );
            }
    ...
    that.clearpoint = function () {
        ...
        for (var mid in glmesh) {
            glmesh[mid].dispose();
            if (glmesh[mid].viewer.gl) glmesh[mid].viewer.gl.activateRenderLoop();
            delete glmesh[mid];
        }
        ...
        if (that.type == "pointROI" && that.roinii && that.roinii.fileObject) {
            ...
            KViewer.iterateMedViewers(function (medViewer) {
                for (var k = 0; k < medViewer.ROIs.length; k++) {
                    if (
                        medViewer.viewport.pointROIviewer == undefined &&
                        medViewer.ROIs[k].roi.fileID == that.roinii.fileObject.fileID
                    ) {
    ...
    that.deletepoint = function () {
        ...
        for (var x in that.removeupdater) that.removeupdater[x]();
...
function createPointROI(p) {
    ...
    if (markerPanel.freeView == undefined) {
        var fvpanel = KPanel(
            $(document.body),
            "viewportpanel" + KPanelView.runningID++,
            "<b>" + p.parentmarkerset.name + "</b>   3D view"
        );
        ...
        $colsel.click(function () {
            ...
            $colsel.css(
                "background",
                "rgb(" + clearColor[0] * 255 + "," + clearColor[1] * 255 + "," + clearColor[2] * 255 + ")"
            );
            freeView.medViewer.gl.engine.scenes[0].clearColor = new BABYLON.Color3(
                clearColor[0],
                clearColor[1],
                clearColor[2]
            );
            freeView.medViewer.gl.engine.scenes[0].render();
            if (clearColor[0] == 0) freeView.$infobar.css("color", "", "background", "");
            else freeView.$infobar.css("color", "black", "background", "red");
    ...
    p.callbacks.delete.pointROI = function () {
        KViewer.iterateMedViewers(function (medViewer) {
            for (var k = 0; k < medViewer.ROIs.length; k++) {
                if (
                    medViewer.viewport.pointROIviewer == undefined &&
                    medViewer.ROIs[k].roi.fileID == that.fileObject.fileID
                ) {
                    medViewer.ROIs.splice(k, 1);
                    signalhandler.send("positionChange");
                    break;
            ...
            if (markerPanel.freeView.medViewer) {
                if (
                    markerPanel.freeView.medViewer.ROIs.length > 0 &&
                    markerPanel.freeView.medViewer.ROIs[0].roi.fileID == that.fileObject.fileID
                ) {
    ...
    function createROINii() {
        ...
        var vv = [
            math.ceil(radius / voxSize[0]) + nr,
            math.ceil(radius / voxSize[1]) + nr,
            math.ceil(radius / voxSize[2]) + nr,
            0,
        ];
        ...
        var fobj = {
            content: {
                ...
                voxSize: voxSize,
                sizes: sizes,
                pixdim: [3, voxSize[0], voxSize[1], voxSize[2]],
            },
        };
    ...
    function updateROI(medViewer_clicked, ev, thresh) {
        if (thresh != undefined) p.thresh = thresh;
        if (thresh == undefined && p.thresh == undefined) p.thresh = parseFloat(markerPanel.pointROITool.$thresh.val());
        ...
        if (
            medViewer_clicked != undefined &&
            medViewer_clicked.nii != undefined &&
            niitemplate != medViewer_clicked.nii
        ) {
        ...
        if (p.parentmarkerset.markerPanel.pointROITool.pen == "regionGrowRestrict") state.regionGrowRestric = true;
        ...
        function updateDistanceLine() {
            if (markerPanel.freeView.medViewer && markerPanel.freeView.medViewer.gl) {
                ...
                if (that.stats.firstPoint == undefined || that.stats.secondPointPoint == undefined) return;
                ...
                var ssb = 1 + ssa;
                var p2_2 = [0, 0, 0];
                var vec1 = [p2[0] - p1[0], p2[1] - p1[1], p2[2] - p1[2], 0];
                p2_2[0] = p1[0] + ssb * vec1[0];
                p2_2[1] = p1[1] + ssb * vec1[1];
                p2_2[2] = p1[2] + ssb * vec1[2];

                var p1_1 = [0, 0, 0];
                p1_1[0] = p1[0] - ssa * vec1[0];
                p1_1[1] = p1[1] - ssa * vec1[1];
                p1_1[2] = p1[2] - ssa * vec1[2];
                var myPath = [
                    new BABYLON.Vector3(p1_1[0], p1_1[1], -p1_1[2]),
                    new BABYLON.Vector3(p2_2[0], p2_2[1], -p2_2[2]),
                ];
                ...
                markerPanel.freeView.medViewer.distanceLine = BABYLON.MeshBuilder.CreateTube(null, {
                    path: myPath,
                    radius: 0.01 * that.stats.diameter,
                    instance: markerPanel.freeView.medViewer.distanceLine,
                });
            }
        }
        //////////////////////////////////////////
        ...
        roiTool.modifyRoiInternal(points_wc, valtoset, undefined, medViewer, state, function (changedPoints) {
            ...
            KViewer.iterateMedViewers(function (medViewer) {
                if (medViewer.viewport.pointROIviewer != undefined) return;

                var found = false;

                for (var k = 0; k < medViewer.ROIs.length; k++)
                    if (medViewer.ROIs[k].roi.fileID == that.fileObject.fileID) found = true;
                if (!found) {
    ...
    function calcAndSetStatistics() {
        ...
        //************** report  *********************
        // mL or microliters ??
        if (stats.volume < 1) var volumestr = (stats.volume * 1000).toFixed(1) + " microL";
        else var volumestr = stats.volume.toFixed(1) + " mL";
        ...
        if (that.fileObject.fileinfo.surfreference != undefined) {
            ...
            var surf_of_sphere_with_same_vol =
                4 * Math.PI * radius_of_sphere__with_same_vol * radius_of_sphere__with_same_vol;
        ...
        if (markerPanel.freeView.medViewer) markerPanel.freeView.$infobar.html(str);
    }

    that.setThisPointInto3DView = function (forceshow) {
        if (!that.fileObject) return false;

        if (
            !markerPanel.freeView.medViewer ||
            that.fileObject.fileID !== markerPanel.freeView.medViewer.currentFileID
        ) {
            if (markerPanel.freeView.medViewer) {
                // return if already in view
                if (markerPanel.freeView.medViewer.nii == that.fileObject.content) return false;
        ...
        if (forceshow) markerPanel.freeView.$container.show();
    };
...
function calcInertiaTensor(nii, img) {
    ...
    if (img != undefined) var A = math.multiply(math.inv(img.edges), nii.edges);
            ...
            for (var z = 0; z < sz; z++) {
                ...
                if (val > 0 && calcMaxDiameter) {
                    ...
                    if (
                        ...
                        var vec = [
                            border[count][0] * nii.voxSize[0],
                            border[count][1] * nii.voxSize[1],
                            border[count][2] * nii.voxSize[2],
                        ];
                        for (var k = 0; k < count; k++) {
                            var vec2 = [
                                border[k][0] * nii.voxSize[0],
                                border[k][1] * nii.voxSize[1],
                                border[k][2] * nii.voxSize[2],
                            ];
                            var distance = math.sqrt(
                                (vec[0] - vec2[0]) * (vec[0] - vec2[0]) +
                                    (vec[1] - vec2[1]) * (vec[1] - vec2[1]) +
                                    (vec[2] - vec2[2]) * (vec[2] - vec2[2])
                            );
    ...
    if (calcMaxDiameter) diameter = biggestDistance;
    else diameter = false;
...
function kp_vsum(a, b, s) {
    ...
    if (a == undefined || b == undefined) console.log(a);
    if (s == undefined) s = 1;
...
KMarkerScribble_interpolate_slices = function (mset) {
	...
	*******************************************************/
    ...
    for (var k = 0; k < points.length; k++) {
        if (k == 0)
            // re-use the first normal vec of first slice, otherwise direction might change...
            var temp = calc_center_of_mass(points[k].subpoints);
        else var temp = calc_center_of_mass(points[k].subpoints, tlist[k - 1].normalvec);

        tlist.push(temp);
        dlist.push(temp.dist_to_origin);
    }
    var sortind = Array.from(Array(dlist.length).keys()).sort((a, b) =>
        dlist[a] < dlist[b] ? -1 : (dlist[b] < dlist[a]) | 0
    );
    ...
    function do_it(ind1, ind2) {
		...
		*/
        ...
        if (method == "closest") {
        ...
        } // method 2: intercect with plane defined by point=>centerofmass
        else {
            var equalize_com = 1;
            ...
            for (var k = 0; k < poly0cc.length; k++) {
                ...
                for (var u = 1; u < poly1.length; u++) {
                    ...
                    if (d0 * d1 <= 0) {
                        // intersection means: both points are on different sides of plane
                ...
                if (isec.secpoints.length == 0) continue;
		...
		************************************/
        ...
        for (var s = 0; s < nsteps; s++) {
            ...
            for (var k = 0; k < targetpoly.length; k++) {
                if (debug) temppoly.push(poly0[k]);
...
function createRectNiiFromPoly(polygons, inplaneres) {
    ...
    for (var j = 0; j < polygons.length; j++) {
        ...
        for (var k = 0; k < poly.length; k++) {
            var c = [
                poly[k][0] * nx[0] + poly[k][1] * nx[1] + poly[k][2] * nx[2],
        ...
        if (j > 0) mindist = math.min(mindist, math.abs(aligned[j][0][2] - aligned[j - 1][0][2]));
    }
    mini;
    maxi;
    var or = [
        mini[0] * nx[0] + mini[1] * ny[0] + mini[2] * nz[0],
    ...
    var edges = [
        [vsz[0] * nx[0], vsz[1] * ny[0], vsz[2] * nz[0], or[0]],
    ...
    var shape = [];
    for (var k = 0; k < 3; k++) shape[k] = math.ceil((maxi[k] - mini[k]) / vsz[k]) + 1;

    var nii = {
        edges: math.matrix(edges),
        bbox: undefined,
        voxSize: vsz,
        sizes: shape,
        pixdim: [3, vsz[0], vsz[1], vsz[2]],
        wid: shape[0],
        widhei: shape[1] * shape[0],
        ...
        for (var y = 0; y < nii.sizes[1]; y++)
            for (var x = 0; x < nii.sizes[0]; x++) {
                ...
                if (
                    ...
                    (j < s1) &
                    (k < s2) &
                    (polymask.data[i + j * polymask.wid + k * polymask.widhei] > 0)
                ) {
...
function fillPolygon(point, nii, addToHistory) {
    ...
    if (point.$markers) var medViewer = point.$markers[Object.getOwnPropertyNames(point.$markers)[0]].medViewer;
    // 	else
    ...
    //return false;
    var tOffset = 0;
    if (nii.currentTimePoint) tOffset = nii.currentTimePoint.t * nii.widheidep;
    ...
    for (var x = minX; x < maxX; x += 1) {
        for (var y = minY; y < maxY; y += 1) {
            var isin = f(x, y);
            if (isin) {
                if (minind == 0) var currentVoxel = [slicepos, x, y, 1];
                else if (minind == 1) var currentVoxel = [x, slicepos, y, 1];
                else if (minind == 2) var currentVoxel = [x, y, slicepos, 1];
