...
kmath.matrices_equal = function (x, y, eps) {
    ...
    var diff = 0;
    for (var k = 0; k < x.length; k++) for (var j = 0; j < x[k].length; j++) diff += Math.abs(x[k][j] - y[k][j]);
...
kmath.matrix = function (v) {
    ...
    if (typeof v[0] == "number") {
        ...
        for (var k = 0; k < v.length; k++) data[k] = v[k].slice(0);
    }
    return {
        _data: data,
        _size: sizes,
...
kmath.Transpose = function (v) {
    if (v._data == undefined) v = kmath.matrix(v);
    else v = clone(v);
...
kmath.diag = function (v) {
    ...
    for (var i = 0; i < n; i++) data[i][i] = v[i];
...
kmath.setTranslation = function (m, t) {
    ...
    for (var i = 0; i < n; i++) {
        data[i] = [];
        for (var j = 0; j < n; j++) data[i][j] = m._data[i][j];
    }
    for (var i = 0; i < n - 1; i++) data[i][n - 1] = t[i];
...
kmath.cross = function (a, b, donormalize) {
    ...
    if (y._data) y = y._data;

    var z = [x[1] * y[2] - x[2] * y[1], x[2] * y[0] - x[0] * y[2], x[0] * y[1] - x[1] * y[0]];

    if (donormalize == 1) {
        var n = 0;
        for (var k = 0; k < z.length; k++) n += z[k] * z[k];
...
kmath.dot = function (a, b) {
    if (a._data != undefined) a = a._data;
    if (b._data != undefined) b = b._data;
...
kmath.dots = function (a, b) {
    ...
    var out = 0;
    for (var k = 0; k < a.length; k++) out += a[k] * b[k];
    return out;
};
...
kmath.multiply = function (a, b) {
    if (Array.isArray(a)) a = kmath.matrix(a);
    if (Array.isArray(b)) b = kmath.matrix(b);

    if (typeof a == "number") {
        var r = kmath.matrix(b._data);
        if (r._size.length == 1) for (var k = 0; k < r._size[0]; k++) r._data[k] = r._data[k] * a;
        else {
            for (var k = 0; k < r._size[0]; k++) for (var j = 0; j < r._size[1]; j++) r._data[k][j] = r._data[k][j] * a;
        }
        return r;
    } else if (typeof b == "number") return kmath.multiply(b, a);
    else {
        var data = [];
        var trans = false;
        if (b._size.length == 1) {
        ...
        for (var k = 0; k < a._size[0]; k++) {
            ...
            for (var j = 0; j < b._size[1]; j++) {
                var v = 0;
                for (var i = 0; i < a._size[1]; i++) v += a._data[k][i] * b._data[i][j];
                data[k][j] = v;
            }
        }
...
kmath.add = function (a, b, f) {
    ...
    if (typeof a == "number") return kmath.add(b, a, f);

    a = kmath.matrix(a);

    if (typeof b == "number") {
        if (a._size.length == 1) {
            for (var k = 0; k < a._size[0]; k++) a._data[k] += f * b;
        } else {
            for (var k = 0; k < a._size[0]; k++) for (var j = 0; j < a._size[1]; j++) a._data[k][j] += f * b;
        }
        return a;
    } else {
        b = kmath.matrix(b);
        if (a._size.length == 1) {
            for (var k = 0; k < a._size[0]; k++) a._data[k] += f * b._data[k];
        } else {
            for (var k = 0; k < a._size[0]; k++)
                for (var j = 0; j < a._size[1]; j++) a._data[k][j] += f * b._data[k][j];
        }
...
kmath.maxEV = function (M) {
    ...
    for (var k = 0; k < 20; k++) {
        ...
        var dif = 0;
        for (var j = 0; j < n; j++) dif += (v._data[j] - old._data[j]) * (v._data[j] - old._data[j]);
        old = v;
        if (dif < 0.00000001) break;
    }
...
kmath.normalize = function (v) {
    if (v._data) v = v._data;

    var n = 0;
    for (var k = 0; k < v.length; k++) n += v[k] * v[k];
    n = kmath.sqrt(n);
    for (var k = 0; k < v.length; k++) v[k] /= n;
    return n;
};
...
kmath.median = function (values, iqr) {
    ...
    if (iqr == undefined) {
        ...
        var q1 = Math.floor(values.length * 0.25);
        var q2 = Math.floor(values.length * 0.75);
