...
self.addEventListener(
    ...
    function (e) {
        ...
        if (execObj.func == "createFiberVisitMap") {
            if (execObj.lines == undefined) execObj.lines = self.lines;
            else self.lines = execObj.lines;
            createFiberVisitMap(execObj, function (perc) {
                self.postMessage({ msg: "rendering visits " + (100 * perc).toFixed(0) + "%" });
            });
            self.postMessage({ msg: "done", execObj: execObj }, [execObj.buffer]);
        } else if (execObj.func == "createISOSurf") {
            var res = computeIsoSurf(execObj, function (perc) {
                self.postMessage({ msg: "rendering isosurface " + (100 * perc).toFixed(0) + "%" });
            });
            self.postMessage({ msg: "done", execObj: res }, []);
        } else if (execObj.func == "atlasToRoi") {
            atlasToRoi(execObj, function (perc) {
                self.postMessage({ msg: "rendering " + (100 * perc).toFixed(0) + "%" });
            });
            self.postMessage({ msg: "done", execObj: { buffer: execObj.img.buffer } }, [execObj.img.buffer]);
        } else if (execObj.func == "RoiToAtlas") {
            RoiToAtlas(execObj, function (perc) {
                self.postMessage({ msg: "rendering " + (100 * perc).toFixed(0) + "%" });
            });
            ...
            self.postMessage({ msg: "done", execObj: { buf: execObj.buf, changedPoints: res.changedPoints } }, [
                execObj.buf,
            ]);
        } else if (execObj.func == "removesalt") {
            var changedPoints = removesalt(execObj.data, execObj.size, execObj.threshold);
            self.postMessage({ msg: "done", execObj: { buf: execObj.buf, changedPoints: changedPoints } }, [
                execObj.buf,
            ]);
        } else if (execObj.func == "invert") {
            var tot = 1;
            for (var k = 0; k < execObj.size.length; k++) tot *= execObj.size[k];
            var changedPoints = [];
            for (var k = 0; k < tot; k++) {
                if (execObj.data[k] > 0) execObj.data[k] = 0;
                else execObj.data[k] = 1;
            ...
            var tot = 1;
            for (var k = 0; k < execObj.size.length; k++) tot *= execObj.size[k];
            for (var k = 0; k < tot; k++) if (execObj.data[k] > 0) execObj.data[k] = 1;
            ...
            self.postMessage(
                {
                    msg: "done",
                    execObj: { buf: execObj.nii.buffer, data: execObj.nii.data, changedPoints: changedPoints },
                },
                [execObj.nii.buffer]
            );
        }

        if (!execObj.keepOpen) self.close();
    },
    false
);
    ...
    if (typeof data.key == "object") {
        {
            ...
            for (var z = 0; z < sizes[2]; z++)
                for (var y = 0; y < sizes[1]; y++)
                    for (var x = 0; x < sizes[0]; x++) {
                        if (data.key[w2key(x, y, z)]) data.img.data[x + sizes[0] * y + sizes[0] * sizes[1] * z] = 1;
                        else data.img.data[x + sizes[0] * y + sizes[0] * sizes[1] * z] = 0;
                    }
        }
    } else {
        if (sameGeometry(data.img, data.atlas) && data.deffield == undefined) {
            ...
            for (var z = 0; z < sizes[2]; z++)
                for (var y = 0; y < sizes[1]; y++)
                    for (var x = 0; x < sizes[0]; x++) {
                        if (w2key(x, y, z) > 0) data.img.data[x + sizes[0] * y + sizes[0] * sizes[1] * z] = 1;
                        else data.img.data[x + sizes[0] * y + sizes[0] * sizes[1] * z] = 0;
                    }
        }
    }
            ...
            for (var y = 0; y < sizes[1]; y++)
                for (var x = 0; x < sizes[0]; x++) {
                    ...
                    if (data.img.data[idx] > 0) {
                        if (data.atlas.data[idx] <= 0) data.atlas.data[idx] = data.key;
                    } else if (data.atlas.data[idx] == data.key) data.atlas.data[idx] = 0;
                }
    } else {
        for (var z = 0; z < sizes[2]; z++)
            for (var y = 0; y < sizes[1]; y++)
                for (var x = 0; x < sizes[0]; x++) {
                    var idx = x + sizes[0] * y + sizes[0] * sizes[1] * z;
                    if (data.img.data[idx] > 0) data.atlas.data[idx] = data.key;
                    else if (data.atlas.data[idx] == data.key) data.atlas.data[idx] = 0;
                }
    }
}
...
function createFiberVisitMap(data, progress) {
        ...
        for (var index = 0; index < plen; index++) {
            for (var k = 0; k < osamp; k++) {
                ...
                var weight = Math.sqrt(
                    (last0 - v0) * (last0 - v0) + (last1 - v1) * (last1 - v1) + (last2 - v2) * (last2 - v2)
                );
                last0 = v0;
                last1 = v1;
                last2 = v2;
    ...
    for (var l = 0; l < subsetlen; l++) {
        if (l % steps == 0) progress(l / subsetlen);
...
function computeIsoSurf(nii) {
    ...
    var offset = 0;
    if (timepoint != undefined) offset = timepoint.t * whd;
