...
var RNAInteractionViewer = (function (riv) {
    ...
    riv.filterCombinedRegions = (combineRegions) => {
        ...
        for (let key in combineRegions) {
            ...
            if (revKey in combineRegions) {
                slicedRegionPie[key] = parseInt(combineRegions[key]) + parseInt(combineRegions[revKey]);
    ...
    riv.loadRNATypes = (records) => {
        ...
        if (recordsData && recordsData.length > 1) {
            ...
            _.each(recordsData, (item) => {
                ...
                symbolGeneExp = riv.collectTopSymGeneExp(symbolGeneExp, sym1, sym2, tpm1, tpm2);
            });
            // get unique types
            ...
            riv.plotPieChart(typ2Pie, "type2-piechart", "Right chimeric arm biotypes");
            riv.plotPieChart(regionPie, "type1-type2-piechart", "Types of interactions");
            riv.plotHistogram(
    ...
    riv.setDisplayRecordsMessage = () => {
        ...
        if (modelLen === 0) {
            ...
            textMessage = "Showing <b>" + (ctr + 1) + "-" + toRecord + " of " + modelLen + " records";
        }
        return textMessage;
    };
    ...
    riv.buildInteractionsPanel = () => {
        ...
        if (model && model.length > 1) {
            ...
            let showModel = model.slice(riv.counterRecords, riv.counterRecords + riv.nIncrement);
    ...
    riv.fetchSymbolsInteractions = (e) => {
        ...
        if (isOpen === 0) {
            ...
            tableName += ".";
            dbQuery +=
                tableName + "symbol1 = '" + riv.symbol1 + "' AND " + tableName + "symbol2 = '" + riv.symbol2 + "'";
    ...
    riv.fetchSingleSummary = (e) => {
        ...
        tableName += ".";
        dbQuery += tableName + "symbol1 = '" + riv.symbol1 + "' AND " + tableName + "symbol2 = '" + riv.symbol2 + "'";
        dbQuery +=
            " AND " +
            tableName +
            "txid1 = '" +
    ...
    riv.buildRNAPairInformation = (item) => {
        ...
        alignment =
            sequenceInfo.dotbrackets.indexOf(")") === -1 ? noAlignmentTemplate : riv.fetchAlignment(sequenceInfo);
    ...
    riv.makeRegionQ = (baseQ) => {
        ...
        if (riv.type1 === "all" && riv.type2 !== "all") {
            ...
            query = " WHERE " + col1Name + " = '" + riv.type1 + "' AND " + col2Name + " = '" + riv.type2 + "'";
        }
        return baseQ + query;
    };
    ...
    riv.registerPlotClick = (elemId) => {
        ...
        myPlot.on("plotly_click", (data) => {
            ...
            dbQuery += " WHERE symbol1 = " + "'" + plotSelectedOpt + "' OR symbol2 = " + "'" + plotSelectedOpt + "'";
            riv.resetSummary();
            riv.resetFilters();
            riv.resetCheckAll();
    ...
    riv.getSymbolsBaseQuery = (tableName) => {
        return "SELECT DISTINCT (symbol1 || ':' || symbol2) as symbols FROM " + tableName;
    };
    ...
    riv.makeSearchURL = () => {
        ...
        if (filterType === "score") {
            ...
            filterClause = tableName + "hybrid" + " " + filterOperator + " " + "'" + filterQuery + "'";
        } else if (filterType === "mfe") {
            filterClause = tableName + "mfe" + " " + filterOperator + " " + filterQuery;
        } else {
    ...
    riv.createSummary = (summaryItems) => {
        ...
        _.each(summaryItems, (item) => {
            ...
            summaryRegionTypes[combinedRegions] = (summaryRegionTypes[combinedRegions] || 0) + 1;
        ...
        plottingData.region1 = riv.mergeFamiliesToOthers(plottingData.region1, summaryResultRegion1.length);
    ...
    riv.plotInteractions = (data) => {
        ...
        riv.plotPieChart(data.summaryRegionTypes, "selected-types", "Types of interactions");
        riv.plotHistogram(data.score, "rna-score", "Overall score for the chimeras", "Score", "# Interactions");
        riv.plotHistogram(data.score1, "rna-score1", "Score for left interacting partners", "Score", "# Interactions");
        riv.plotHistogram(data.score2, "rna-score2", "Score for right interacting partners", "Score", "# Interactions");
        riv.plotHistogram(
    ...
    riv.makeAlignmentSummary = (alignment1, alignment2) => {
        ...
        $elAlignmentGraphics1.append("<p>Alignment positions on " + alignment1.length + " transcripts<p>");
        $elAlignmentGraphics1.append(riv.buildSVGgraphics(alignment1, "gene1", "green"));
        $elAlignmentGraphics2.empty();
        $elAlignmentGraphics2.append("<p>Alignment positions on " + alignment2.length + " transcripts<p>");
        $elAlignmentGraphics2.append(riv.buildSVGgraphics(alignment2, "gene2", "blue"));
    };
