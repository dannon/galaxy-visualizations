var calculateCoords = (module.exports = function (seq, dotbr, links) {
    ...
    if (2 > 1) {
        ...
        while (i < seq.length) {
            ...
            if (j > i) {
                drawLoop(i, j, x + (65 * vx) / 2.0, y + (65 * vy) / 2.0, dirAngle, coords, centers, angles, seq, links);
...
function drawLoop(i, j, x, y, dirAngle, coords, centers, angles, seq, links) {
    ...
    if (getPartner(i, links) === j) {
        ...
        drawLoop(
            i + 1,
            j - 1,
            x + 40 * Math.cos(dirAngle),
            y + 40 * Math.sin(dirAngle),
            dirAngle,
            coords,
            centers,
            angles,
        ...
        var mlCenter = { x: x + centerDist * Math.cos(dirAngle), y: y + centerDist * Math.sin(dirAngle) };
        ...
        for (k = 0; k < helices.length; k++) {
            ...
            drawLoop(
                m + 1,
                n - 1,
                40 * Math.cos(newAngle) + (coords[m].x + coords[n].x) / 2.0,
                40 * Math.sin(newAngle) + (coords[m].y + coords[n].y) / 2.0,
                newAngle,
                coords,
                centers,
                angles,
